...
foreach ($f in $files) {
    ...
    foreach ($m in ([regex]::Matches($text, '(?m)^\s+((?:msc|gfat|xhci|db3|drec)-cell-[\w-]*|usb-hid-cell-base)\s*:\s*Integer\s*=\s*(\d+)\s*$'))) {
