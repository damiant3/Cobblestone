...
$blocks = @(
    @{ Name = 'xhci scalars (GopXhci private map)'; Lo =  0; Hi = 19 },
    @{ Name = 'xhci port cells (20 + port, xhci-port-cells=8)'; Lo = 20; Hi = 27 },
    @{ Name = 'xhci ownership/BAR scalars'; Lo = 28; Hi = 46 },
    @{ Name = 'usb-hid-cell-count (the HID bind counter)'; Lo = 47; Hi = 47 },
    @{ Name = 'xhci controller table (48 + i*4, 4 controllers)'; Lo = 48; Hi = 63 },
    @{ Name = 'xhci-ep-base block (6 words)'; Lo = 64; Hi = 69 },
    @{ Name = 'msc cells'; Lo = 70; Hi = 87 },
    @{ Name = 'usb-hid-note block (usb-hid-cell-base + idx*4, 4 devices)'; Lo = 96; Hi = 111 }
)
...
$named = @{}
$files = Get-ChildItem -Recurse -Path apps, codex, build/boot -Filter *.codex -ErrorAction SilentlyContinue
foreach ($f in $files) {
    $text = Get-Content -Raw -LiteralPath $f.FullName -ErrorAction SilentlyContinue
    if (-not $text) { continue }
    # Only chapters that actually touch xdiag can own an xdiag cell.
    if ($text -notmatch 'xdiag-(put|get)') { continue }
    ...
    foreach ($m in [regex]::Matches($text, '(?m)^\s+((?:msc|gfat|xhci)-cell-[\w-]*|usb-hid-cell-base)\s*:\s*Integer\s*=\s*(\d+)\s*$')) {
        ...
        if (-not $named.ContainsKey($name)) { $named[$name] = @{ Cell = $val; File = $f.Name } }
    }
}
...
foreach ($k in $named.Keys) {
    if ($exclusiveBounds -contains $k) { continue }
    $c = $named[$k].Cell
    if (-not $byCell.ContainsKey($c)) { $byCell[$c] = @() }
    $byCell[$c] += "$k ($($named[$k].File))"
}
foreach ($c in ($byCell.Keys | Sort-Object)) {
    if ($byCell[$c].Count -gt 1) {
        $problems += "cell ${c}: claimed by " + ($byCell[$c] -join ' AND ')
    }
}
...
for ($i = 0; $i -lt $blocks.Count; $i++) {
    for ($j = $i + 1; $j -lt $blocks.Count; $j++) {
        ...
        if ($a.Lo -le $b.Hi -and $b.Lo -le $a.Hi) {
            ...
            $problems += "cells ${lo}..${hi}: block '$($a.Name)' overlaps block '$($b.Name)'"
        }
    }
}

# A named cell must fall inside the block its subsystem declared, or it is
# a constant nobody reserved room for.
foreach ($k in $named.Keys) {
    if ($exclusiveBounds -contains $k) { continue }
    $c = $named[$k].Cell
    $inSome = $false
    foreach ($b in $blocks) { if ($c -ge $b.Lo -and $c -le $b.Hi) { $inSome = $true; break } }
    if (-not $inSome) {
        $problems += "cell ${c}: '$k' ($($named[$k].File)) is outside every declared block -- add a block row or move it"
    }
}

if (-not $Quiet) {
    Write-Host "xdiag cell map ($($named.Count) named constants, $($blocks.Count) declared blocks):"
