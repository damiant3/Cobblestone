# Memory watchpoint / breakpoint debugger for bare-metal Codex.
#
...
if (-not $Kernel) { $Kernel = 'build-output\bare-metal\Codex.cdx' }
if (-not (Test-Path -PathType Leaf $Kernel)) {
    Write-Error "MISSING: $Kernel"
    exit 2
}
if (-not $Watch -and -not $Break -and -not $ReadWatch) {
    Write-Error "Specify -Watch, -Break, or -ReadWatch with an address"
    exit 2
}
if ($Accel -eq 'whpx' -and ($Watch -or $ReadWatch)) {
    Write-Error "WHPX does not support watchpoints. Use -Accel tcg or use -Break instead."
    exit 2
}

$gdbCheck = wsl -- which gdb 2>&1
if ($LASTEXITCODE -ne 0) { Write-Error "gdb not found in WSL. Install: sudo apt install gdb"; exit 2 }

$srcBytes = [System.IO.File]::ReadAllBytes($Src)
$addr = if ($Watch) { $Watch } elseif ($Break) { $Break } else { $ReadWatch }

# Build GDB script
$bpCmd = if ($Watch) { "watch *(unsigned long long *)$addr" }
         elseif ($ReadWatch) { "rwatch *(unsigned long long *)$addr" }
         else { "hbreak *$addr" }
...
kill
quit
"@ | Set-Content $gdbFile -Encoding UTF8 -NoNewline
...
try {
    ...
    $gdbProc = Start-Process -FilePath 'wsl' -ArgumentList @(
        '--', 'timeout', "$TimeoutSec", '/usr/bin/gdb', '-batch', '-nx', '-x', $wslGdbPath
    ) -PassThru -WindowStyle Hidden -RedirectStandardOutput $gdbOut -RedirectStandardError $gdbErr
    ...
    Write-Host "==========================================="
    if (Test-Path $gdbOut) { Get-Content $gdbOut }
    $errTxt = if (Test-Path $gdbErr) { Get-Content $gdbErr -Raw } else { '' }
    if ($errTxt.Trim() -and $errTxt -notmatch '^warning:') {
        Write-Host ""
