# Helper invoked by test.ps1. Boots a CDX/ELF kernel under a VM, captures
# serial output between READY and HEAP, writes to the output file.
#
# Three modes:
#   CODEX-VM MODE (default): codex-vm.exe via TCP sockets, all cases.
#   FILE MODE (QEMU, no stdin): chardev file backend. No TCP, no race.
#     QEMU writes serial output to host files; script reads them after exit.
#   TCP MODE (QEMU + stdin): chardev socket backend. Needed to pump stdin
#     bytes to the guest via COM1.
[CmdletBinding()]
param(
...
$needsTcp = ($StdinFile -and (Test-Path -PathType Leaf $StdinFile)) -or $script:UseCodexVm

# ═══════════════════════════════════════════════════════════════════════
# FILE MODE — QEMU only, no sockets, no race
# ═══════════════════════════════════════════════════════════════════════
if (-not $needsTcp) {
    ...
    try {
        Write-SweepLog "$sample run-start pcore=$PCore qemu-file-mode"
        $diskArgs = @()
        ...
        Write-SweepLog "$sample vm-pid=$($proc.Id) qemu-file-mode"
...
try {
    ...
    Write-SweepLog "$sample vm-pid=$($run.Process.Id) connected"
