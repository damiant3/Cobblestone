...
| `lease <state> ip=... gw=... dns=... lease=N gw-mac=... hop=... leased-gw-is-hop=same|differs|unresolved|none cfg-gw=absent|<addr>` (stage 17, DiagLease.codex) | NIC-6's question asked on the driver `b3` bound: a DHCP lease is requested under `b3`'s own budget (4 s, 16 DISCOVERs), the leased gateway is ARPed for, and its MAC is set beside the hop the record channel resolved. Nothing is re-addressed: the channel and `b3` keep the address DIAG.CFG gave them. `ok`, `no-lease` (no server answered), `no-arp` (the leased gateway did not answer), `no-card` (nothing ahead bound the driver), `no-hpet`. codex-vm's NAT answers every ARP with one MAC, so the bed reads `same` on every flight; the board is the separating reading |
| `rtcw <state> b=N binary=y/n before=S wrote=S+30 back=S' restored=S` (stage 18, DiagRtcW.codex) | WORKS-24's question: a seconds value thirty away from the reading is written to the MC146818 inside a Status B SET window and read back, then the reading is written back the same way. `accepted` (the write landed; a board loses under a second), `ignored` (the value from before came back: codex-vm drops every CMOS write, so this is the bed's reading on every arm and the board's is the one that counts), `unreadable`, `other` |
