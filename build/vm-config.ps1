...
# Default VM is codex-vm; set $env:USE_QEMU=1 to force QEMU.
# Dot-source from test-compile.ps1, test-run.ps1, test.ps1, build.ps1.

Set-StrictMode -Version Latest
$ErrorActionPreference = 'Stop'

# Locate codex-vm or QEMU. Default is codex-vm; set $env:USE_QEMU=1 to
# force QEMU (needed for GDB watchpoints / TCG-only debugging).
...
if (-not $script:UseCodexVm -and (-not $script:QemuBin -or -not (Test-Path -PathType Leaf $script:QemuBin))) {
    throw "Neither codex-vm nor QEMU found. Build tools/codex-vm.c or install QEMU."
}
...
# Launch VM (codex-vm or QEMU) with the standard dual-socket layout.
