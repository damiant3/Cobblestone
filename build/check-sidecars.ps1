...
$exts = @('.skip', '.slow', '.fatal', '.expected', '.expected-arm64', '.expected-riscv64', '.stdin', '.keys', '.disk', '.disk2', '.disk-src', '.disk-mint', '.disk2-mint', '.failing', '.diag', '.smp', '.vmargs', '.no-cross', '.arch-only', '.covers', '.cross-refusal', '.cross-budget', '.message')
$orphans = @()
$checked = 0
$noeol = @()
$expChecked = 0
