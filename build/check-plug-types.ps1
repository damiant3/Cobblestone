...
# the IR text wire is a closed contract
# 
...
foreach ($f in @($EmitFile, $ParseFile)) {
    if ((-not (Test-Path -PathType Leaf $f))) {
        Write-Host ([string]'check-plug-types: missing ' + $f)
        exit 1
    }
}
...
foreach ($line in ([System.IO.File]::ReadAllLines($ParseFile))) {
    foreach ($m in ([regex]::Matches($line, '(?:head|a)\s*==\s*"(?<h>[a-z][a-z0-9\-]*)"'))) {
        $accepted[$m.Groups['h'].Value] = $true
    }
...
if ((($emitted.Count -eq 0) -or ($accepted.Count -eq 0))) {
    ...
    Write-Host ([string]'check-plug-types: FAILED to extract (emitted=' + ([string]$emitted.Count + ([string]' accepted=' + ([string]$accepted.Count + ')'))))
...
$positional = @('chapter', 'def', 'strs', 'args', 'params', 'sum-ctors', 'record-fields', 'tparams', 'ctors', 'fields', 'effs', 'scopes')
$divergent = @(($emitted.Keys | Where-Object { (-not $accepted.ContainsKey($_)) } | Where-Object { (-not ($positional -contains $_)) } | Sort-Object))


if ($Update) {
    $header = @('# plug-wire-baseline.txt -- generated by build/check-plug-types.ps1 -Update', '#', '# Forms codex/compiler/Emit/IRTextEmitter.codex can WRITE that', '# codex/plugs/common/IRTextParser.codex cannot READ. Each reads back as a', '# plausible default, not an error, so nothing downstream notices.', '#', '# This file is the KNOWN residue. The check fails on anything not listed', '# here. Shrinking it is the work -- give each form a parser arm. Growing it', '# needs a reason in the CL description.', '')
    Set-Content -Path $BaseFile -Value ($header + $divergent) -Encoding UTF8
    Write-Host ([string]'baseline updated: ' + ([string]@($divergent).Count + ' known divergence(s)'))
...
if ((-not (Test-Path -PathType Leaf $BaseFile))) {
    Write-Host ([string]([string]'check-plug-types: no baseline at ' + $BaseFile) + ' -- run with -Update')
    exit 1
}
...
if ((@($new).Count -gt 0)) {
    Write-Host ([string]'check-plug-types: FAIL -- ' + ([string]@($new).Count + ' new wire divergence(s)'))
    foreach ($h in $new) {
        Write-Host ([string]'  (' + ([string]$h + ' ...) is emitted by IRTextEmitter and read by no parser arm'))
    }
    Write-Host ''
...
if ((@($fixed).Count -gt 0)) {
    Write-Host ([string]'check-plug-types: OK -- and ' + ([string]@($fixed).Count + ' baselined divergence(s) are gone:'))
    foreach ($h in $fixed) {
        Write-Host ([string]([string]'  ' + $h) + ' (now parsed -- drop it from the baseline)')
    }
    exit 0
}

Write-Host ([string]'check-plug-types: OK (' + ([string]$emitted.Count + ([string]' emitted, ' + ([string]$accepted.Count + ([string]' accepted, ' + ([string]@($divergent).Count + ' known)'))))))
exit 0
