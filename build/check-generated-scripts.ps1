...
#   check-generated-scripts.ps1 -Update         # rewrite the drift baseline
#
...
if ($Update) {
    ...
    Set-Content -Path $BaseFile -Value ($header + $driftedNow) -Encoding utf8
    Write-Host ""
    Write-Host "baseline updated: $($driftedNow.Count) known drift(s)"
    exit 0
}
