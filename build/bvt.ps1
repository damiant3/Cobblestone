# bvt.ps1 -- Build Verification Test - minimal confidence gate
...
if ((Test-Path -PathType Container $ResultsDir)) {
...
if ($totalFail -gt 0) {
    ...
    foreach ($f in $compileFails) {
        Write-Host "  $f" -ForegroundColor Red
    }
    foreach ($f in $runFails) {
        Write-Host "  $f" -ForegroundColor Red
    }
    exit 1
}
