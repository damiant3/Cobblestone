# Build seed/Codex.img, the bootable UEFI dev-console image.
#
...
if (-not (Test-Path -PathType Leaf $BootSourcePath)) { Write-Host "FAIL: boot source $BootSourcePath missing"; exit 1 }
...
if (-not (Test-Path -PathType Leaf $KernelArg)) { Write-Host "FAIL: kernel $KernelArg missing"; exit 1 }
...
if ($Agent) {
    ...
    if ($LASTEXITCODE -ne 0) { Write-Host "FAIL: agent bundle failed"; exit 1 }
...
if (-not (Test-Path -PathType Leaf $SourceFile)) {
    ...
    if ($LASTEXITCODE -ne 0 -or -not (Test-Path -PathType Leaf $SourceFile)) {
        Write-Host "FAIL: could not produce the compiler source concat"; exit 1
    }
}
...
& pwsh -NoProfile -File $ImgScript -PeInput $BootPe -Out $ImgOut `
    -Seed $Seed -Source $SourceFile -TotalSectors 32768 @AgentArgs
