...
date
echo ""
echo "Phase 1: Cleaning codex intermediates..."
# Codex stage outputs and caches regenerated every run from .codex source.
# Deliberately NOT touching .NET bin/obj directories — dotnet incremental
# compilation catches any C# changes, and wiping bin/obj collides with
# any concurrent `dotnet test` run that has its assemblies loaded.
