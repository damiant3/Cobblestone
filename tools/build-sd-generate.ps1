...
$src = Join-Path $PSScriptRoot "sd-generate.cu"
$out = Join-Path $PSScriptRoot "sd-generate.exe"
if (-not (Test-Path (Join-Path $PSScriptRoot "..\build\output"))) {
    New-Item -ItemType Directory (Join-Path $PSScriptRoot "..\build\output") -Force | Out-Null
}
cmd /c "`"$vcvars`" >nul 2>&1 && `"$nvcc`" -O2 -lcublas -o `"$out`" `"$src`""
if ($LASTEXITCODE -ne 0) { Write-Host "BUILD FAILED"; exit 1 }
Write-Host "Built: $out"
Write-Host "Run: .\tools\sd-generate.exe"
