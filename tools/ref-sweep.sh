#!/bin/bash
# Sample-sweep harness. Runs every samples/*.codex through one compiler and
# verifies the outcome against its sidecar convention.
#
# Usage:
#   tools/ref-sweep.sh [--compiler=ref|selfhost|selfhost-cs|selfhost-netbin|il] [--jobs=N]
#
# Default is --compiler=ref (REF C# CLI, fast — ~0.3s per sample).
# --compiler=selfhost boots build-output/bare-metal/Codex.Codex.elf in QEMU
# per sample (~2-5s per sample, needs stage-0 ELF present).
# --compiler=il builds samples via REF --target il (managed .dll) and runs
# them under `dotnet`. Measures IL-backend coverage/regressions.
...
OUTROOT=build-output/probes/samples-$COMPILER
RESULTS_DIR="$OUTROOT/_results"
mkdir -p "$OUTROOT"
rm -rf "$RESULTS_DIR"
mkdir -p "$RESULTS_DIR"

echo "sample-sweep: compiler=$COMPILER jobs=$JOBS outdir=$OUTROOT"

# compile_sample <src.codex> <outdir> → sets $COMPILE_OK (0/1) and writes
# $outdir/$name.{elf,cs} and $outdir/build.log.
compile_sample() {
    ...
    local cs="$outdir/$name.cs"

    case "$COMPILER" in
        ref)
            if "$DOTNET" "$CLI" build "$src" --target x86-64-bare --exit-mode=qemu-exit \
        ...
        selfhost)
            ...
            ;;
        selfhost-cs)
            if "$DOTNET" "$BOOTSTRAP" --emit-sample "$src" "$cs" > "$log" 2>&1; then
                COMPILE_OK=1
            else
                COMPILE_OK=0
            fi
            ;;
        selfhost-netbin)
            if "$DOTNET" "$BOOTSTRAP" --binary-sample "$src" "$elf" --exit-mode=qemu-exit > "$log" 2>&1; then
                COMPILE_OK=1
            else
                COMPILE_OK=0
            fi
            ;;
        il)
            if "$DOTNET" "$CLI" build "$src" --target il --output-dir "$outdir" > "$log" 2>&1; then
                COMPILE_OK=1
            else
                COMPILE_OK=0
            fi
...
# run_sample <src.codex> <outdir> <actual-out> → runs compiled artifact,
# writes stdout to <actual-out>. Returns 0 on success.
run_sample() {
    local src=$1 outdir=$2 actual=$3
    local name
    name=$(basename "$src" .codex)
    case "$COMPILER" in
        ref|selfhost|selfhost-netbin)
            wsl bash tools/ref-run-for-sweep.sh "$outdir/$name.elf" "$actual" 2>/dev/null
            ;;
        selfhost-cs)
            local cs="$outdir/$name.cs"
            local proj="$outdir/proj"
            mkdir -p "$proj"
            cp "$cs" "$proj/Program.cs"
            cat > "$proj/Program.csproj" <<'EOF'
...
EOF
            if "$DOTNET" build "$proj/Program.csproj" --nologo -v quiet > "$outdir/csbuild.log" 2>&1; then
                "$DOTNET" "$proj/bin/Debug/net8.0/Program.dll" 2>&1 | tr -d '\r' > "$actual"
            else
                echo "CSBUILD_FAIL" > "$actual"
                return 1
            fi
            ;;
        il)
            "$DOTNET" "$outdir/$name.dll" 2>&1 | tr -d '\r' > "$actual"
            ;;
    esac
}

# process_sample <src.codex> → classifies one sample, writes result line to
# $RESULTS_DIR/<name>. Format: "STATUS\t<name>\t<detail>".
process_sample() {
    local src=$1
    local name
    name=$(basename "$src" .codex)
    ...
    # Backend-specific .expected override lives in samples/_<compiler>/.
    # Presence of an override also serves as an explicit opt-in: a backend
    # that can run a sample the root .skip marks as unsupported elsewhere
    # (e.g. effect handlers work on IL but not on X86_64 bare-metal) sets
    # an override sidecar and the skip is ignored for that backend only.
    local expected_file="$dir/$name.expected"
    local override_file="$dir/_$COMPILER/$name.expected"
    if [ -f "$override_file" ]; then
        expected_file="$override_file"
    ...
    if [ -f "$failing_file" ]; then
        ...
        local codes_ok=1
        # Strip CR via tr — `${var%$'\r'}` ANSI-C quoting doesn't survive
        # the export -f / xargs -I {} bash -c subshell round-trip, leaving
        # a trailing \r on expected_code that makes the grep miss a
        # present "error CODE:" line every time. tr is byte-level and
        # robust regardless of how the function body round-trips.
...
export COMPILER DOTNET CLI BOOTSTRAP OUTROOT RESULTS_DIR

chmod +x tools/ref-run-for-sweep.sh 2>/dev/null || true

# Enumerate samples — intentionally flat, samples/MM4-Deferred/ excluded.
samples=()
...
failed=$(( ${#FAIL_COMPILE[@]} + ${#FAIL_EXPECTED_BUT_COMPILED[@]} \
           + ${#FAIL_WRONG_DIAGNOSTIC[@]} + ${#FAIL_OUTPUT[@]} + ${#FAIL_RUNTIME[@]} ))
echo "compiler=$COMPILER  total=$total  pass=$passed  fail=$failed  skip=${#SKIPPED[@]}"
