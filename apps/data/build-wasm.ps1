# Compile the relational engine to a wasm module for the landing site.
#
#   DataWasm.codex -> IR-CCE -> codex/plugs/wasm -> WAT -> wat2wasm -> .wasm
#
...
# is integers. A query answers a TABLE, and a Codex `Text` is a pointer to a
# four-byte length followed by CCE bytes, which a page cannot read without a
# CCE table. So this takes the compiler page's shape, the same one
# apps/mathbook uses: a WASI program, the plan name in on `fd_read`, the
# rendered table out on `fd_write`, driven by the `_start` the emitter already
# exports. Both directions are plain UTF-8 because the foreword converts at
# the I/O boundary (R-CCE), so the page needs no codec. The working precedent
# is codex/plugs/wasm/page-workspace-arm.js.
#
...
#
# Usage: pwsh apps/data/build-wasm.ps1 [-WatOnly] [-Kernel <cdx>]
[CmdletBinding()]
param(
    [switch]$WatOnly,
    [string]$Kernel = ''
...
$OutDir  = Join-Path $Repo 'apps\landing\web\data'
$WorkDir = Join-Path $PSScriptRoot 'build-output'
$Chapter = Join-Path $PSScriptRoot 'DataWasm.codex'
...
$irFile  = Join-Path $WorkDir 'data.ir'
$logFile = Join-Path $WorkDir 'data-compile.log'
Write-Host '[data-wasm] compiling DataWasm.codex to IR ...'
...
Write-Host "[data-wasm] IR: $((Get-Item $irFile).Length) bytes (CCE)"
...
Write-Host '[data-wasm] running the wasm plug ...'
