...
// -- The result is an identity --------------------------------------------
{
  ...
  for (let seed = 1; seed <= 60; seed++) {
    ...
    const want = melds[0] + melds[2] + melds[1] + melds[3] + 240;
    const got = e.pn_t0(r) + e.pn_t1(r);
    if (want !== got) bad.push(`seed ${seed}: scores sum to ${got}, meld plus 240 is ${want}`);
  ...
  ok('the two team scores sum to the meld plus the deck\'s 240 trick points',
