...
| STARMAP-10 | **`scene-add-objects` mutates the scene its caller handed it** | `StarMapScene.codex`: `scene-append-list` builds with `list-push`, which extends in place, and `__record-set` stores through the caller's pointer and returns the same record, so `scene-add-objects ss objs` and `ss` are one value afterwards. A caller holding the old scene to compare against, which is exactly what an arm wants to do, sees no difference at all (L-ALIAS). Found by `codex/test/apps/starmap-suite.codex` on its first run; the arm was changed to capture the count first, so this is registered and NOT fixed. Whether the app wants a copying `scene-add-objects` depends on whether any caller relies on the mutation, and no caller was audited. |
