...
| PRISM-11 | **A hosted Codex server speaks HTTP and not HTTPS, and the TLS to fix that already exists** (red, opened 2026-09-08) | Stage 5d gave a hosted target an HTTP serve loop over `host-socket` (main 23477), and it is plaintext. **The blocker is wiring, not cryptography, and not the hosted boundary.** Measured 2026-09-08: `codex/foreword/core/Tls.codex` cites only CCE, AesGcm, Sha256, Hmac, Hkdf, DiffieHellman, Maybe and ListUtils; `codex/foreword/encode/TlsEndpoint.codex` cites only Encode and Foreword chapters. **Neither names a Net chapter**, so nothing in the TLS 1.3 stack is below the hosted boundary. The API is a byte pump: `tls-ep-server-new`, then `tls-ep-recv : TlsEp, List Integer -> TlsEpStep` where `TlsEpStep = { tes-ep, tes-out : List (List Integer) }`, with `tls-ep-app-in` for the decrypted application bytes. A caller feeds ciphertext read from any stream and writes back what it is handed, which is what `host-socket` recv and send already provide, and `DtlsEndpoint` and `CoapsEndpoint` drive that same API over a non-TCP transport today. What is missing is a serve loop in `apps/works/HostedServe.codex` that pumps the endpoint between the socket and `http-parse-request`/`http-encode`, plus a server certificate. **The certificate is the blocker and it is measured: we can read one and cannot write one.** `Asn1.codex` is a pure reader and every `x509-*` name parses or verifies; there is no DER writer in the tree, and the certificates we hold are minted by `openssl` in `build/mint-tls-fixtures.ps1` for fixtures. Damian ruled 2026-09-08 that self-signing is the direction and a stand-in is needed now, registered as `codex/foreword/core/core-backlog.md` CORE-9 (DER encoder, then a dev-only self-signed certificate, then a CA). The server key is generated per deployment and is NOT the seed signing key. **What does NOT transfer:** `web-serve-concurrent` multiplexes through `WebConn = record { transport : TcpTransportState, ... }`, so its sequencing, reassembly and idle sweep are the NIC stack. A hosted target needs none of that work, because the host kernel sequenced every accepted stream, so hosted concurrency is a set of accepted descriptors rather than a translation of `WebMux`. |
