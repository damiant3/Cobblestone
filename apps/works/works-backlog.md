...
## WORKS-63: `DiskTestRunner` has an arm, and no gate runs the arm

CLOSED as written, 2026-09-09 (fester): `build/disk-runner-arm.ps1` builds the
FAT16 fixture at test time and flies two arms. Root ruled against a `.disk`
sidecar in the depot (8 MB of binary per arm, because the sidecar IS the image),
therefore the arm compiles the sample, runs the PE and IMG plugs and writes
`build-output/disk-runner-arm/`, which is untracked. Arm 1 asserts the guest
compiled the chapter ON THE VOLUME to the same byte count the host's own compile
produced (89,714 for `codex/test/field-range-proven.codex`, 2026-09-09); arm 2,
the control, runs with no disk attached and must print `bps=0 ok=no` and
`REFUSED` and must compile nothing. `-Compiler <kernel>` grades a compiler
change. Measured green on seed 77F6A5D09CFF6BD5.

**RULED by the commander 2026-09-09: the arm is NOT wired into a gate.** Whoever
changes the disk path runs the arm as a lane arm and says so; gate membership is
reviewed at the next release against the cost below.

**Cost, measured 2026-09-09 on the depot seed:** 84.5 seconds wall, 6 guests one
at a time (sample compile, PE plug, IMG plug, runner compile, then the two run
arms), therefore a peak of one guest rather than a fan-out: 1 GiB while a run
arm is up, 0.25 GiB during a compile guest. The 8 MB fixture is written under
`build-output/` on every run and Perforce tracks none of it.
