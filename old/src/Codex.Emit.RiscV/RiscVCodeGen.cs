...
sealed class RiscVConfig : RegisterAllocatorConfig
{
    public override string TargetName => "riscv";
    public override uint HeapReg => Reg.S1;
}
...
sealed class RiscVCodeGen(RiscVTarget target = RiscVTarget.LinuxUser)
{
    ...
    // S1 = working-space heap pointer.  Not saved/restored by functions — global state.
    static readonly RiscVConfig s_config = new();
    ...
    void EmitFunction(IRDefinition def)
    {
        ...
        // Prologue: base frame = 80 bytes (ra + s0 + s2-s9).
    ...
    void EmitStart(IRChapter module)
    {
        ...
        {
            // Linux: heap via brk syscall (214)
            foreach (uint insn in RiscVEncoder.Li(Reg.A0, 0)) Emit(insn);
            foreach (uint insn in RiscVEncoder.Li(Reg.A7, 214)) Emit(insn);
            Emit(RiscVEncoder.Ecall());
            Emit(RiscVEncoder.Mv(Reg.S1, Reg.A0)); // heap starts at brk base
