...
Active design. Implementation in `codex/foreword/engine/EdgeMesh.codex`.
Builds on existing mesh networking (GroupMembership, MeshRoles,
EdgeRouter, RaftConsensus, GossipProtocol), trust lattice (TrustNode,
TrustLattice, PolicyEngine), ManaCoin blockchain (ChainCore,
MintAuthority), Helm communication (HelmBridge, voice hierarchy), and
the Codex game engine (Scene3D, Renderer3D, Physics, Netcode).
...
### Phase 1: Core Orchestration (this CL)
