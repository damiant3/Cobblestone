...
Active design. Implementation in progress. Phase 1 targets a complete
3D pipeline from mesh to framebuffer, running on bare metal with no OS,
no libc, no external dependencies. Every byte is Codex.
...
### Phase 1: Software 3D Pipeline (this CL)
...
### Phase 2: SIMD + Shadows + Skeletal Animation

- SIMD math (Vector 4 Real approximate) for transforms
- Shadow mapping (directional light depth pass)
