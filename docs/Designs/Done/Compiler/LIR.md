...
**Status: DONE-PILED 2026-08-05, Damian's ruling.** What landed stays and is
live; the ARM64 retarget this doc schedules is deferred, not queued, and the doc
comes back off the pile when ARM is revisited. Nothing here is a live commitment
until then -- read the tree, not this file, for what the LIR does today.

The codegen-quality campaign is **closed** -- see §12 for the closing
...
had been false for weeks. §12 closes the *quality* frontier and not the three
open coverage and verification items it names; those go unowned onto the pile
with the rest.

**`IR/Lir.codex` is on the shipping emission path.** Its own chapter prose said
otherwise -- "deliberately dead to the shipping compiler", "still analysis only"
-- until main 13293 deleted those blocks. `emit-function` calls `lir-emit-try`
first (`Emit/X86_64.codex:724`) and takes its bytes whenever the whitelist
admits the definition. Anyone returning to this doc for the ARM work should
start from that fact rather than from the increment narrative below. Satisfies entry criterion **E3**
