...
**Origin:** val's adversarial review of the demand-paged arena (main CL
7202, seed DDAB0BD2). Companion to
`docs/Designs/Done/Compiler/DemandPagedArena.md` (the design) and
`DemandPagingVictory.md` (the ship story). This document records what the
...
(blu, 2026-07-13): every core now has its own TSS and IST1 emergency
stack, so an AP fault produces an `!EXC` dump instead of a silent triple
fault. What remains of SMP is `docs/PM/BACKLOG.md` 4.11.
...
- BSP only at the time. **Superseded 2026-07-13 (blu):** the GDT now
  carries one TSS descriptor per core at selector `24 + core * 16`, and
  each core's IST1 points at its own 2 KB stack in [0x15000, 0x1D000).
  An AP fault is no longer a silent triple fault.
...
**It did NOT retire the survey prose in PHASE-ARCHITECTURE.md.** That
