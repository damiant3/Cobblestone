...
**Moved to Done 2026-08-05 (Damian's ruling).** Everything compile-time is shipped and pinned; the step-6 remainder (runtime scope enforcement) is backlogged as COMPILER-1 in `codex/compiler/compiler-backlog.md`.
...
the indirect route through an unscoped helper is CLOSED (2026-07-13) --
see below.
...
| **2. Scope** (path/host parameterization) | **RING 1 SHIPPED.** CDX4002 fires on a direct call outside its grant, on a computed path, and on a callee declaring a wider scope. The indirect route through an unscoped helper is CLOSED (2026-07-13). Network/channel scope enforcement is wired too: pinned by `codex/test/errors/scope-network-port.codex`. See below. |
...
| 4 | Compile-time scope checking (CDX4002) | **RING 1 SHIPPED 2026-07-13.** Direct calls, computed paths, and callee narrowing are enforced. The indirect route through an unscoped helper is closed (2026-07-13): the builtins carry honest effect rows. |
