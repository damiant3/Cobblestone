...
Status: **DONE. Mechanism and migration both complete.** Archived
2026-07-26.

The `grounds` keyword works end to end (parse -> Document/AChapter
`ground-effects` -> `UnificationState.chapter-grounds` -> `check-def-normal`,
scoped, one-pass; pinned by `grounds-port` and `errors/grounds-scope`).

**This header used to say "What remains is the migration campaign below",
while the campaign section below said "The migration is complete." The
document contradicted itself, and the stale half is what `CurrentPlan.md`
and two agents' workplans were quoting as live work.** Re-verified against
the source 2026-07-26:

- `quire-effect-exempt` (`TypeChecker.codex`) holds thirteen entries: the
  eight compiler quires and the five plug backends. **No `codex/os/` quire
  is exempt.** Not `Kernel`, `Dev`, `Net`, `Sched`, `Trust`, `Observe`,
  `Replay`, `Verify`, or `OS`.
- Fourteen driver chapters carry `grounds`, including three that ground
  `Device.Mmio` (`HdaAudio`, `Lapic`, `MicInput`). The Kernel entry below
  says "No `Device.Mmio` grounding", so **work continued past the last
  update to this file** -- it is stale in the direction of more complete,
  not less.
- Enforcement is live rather than merely unlisted: deleting
  `grounds Device.Port` from `Pci.codex` makes a real Kernel chapter fail
  CDX2031 and CDX2033. That is the check worth repeating if anyone doubts
  it, because an empty exemption list would look identical to a working one
  if the chapters simply performed no effects.
- The three pinned tests pass: `grounds-port` runs and matches, and both
  `errors/grounds-scope` and `errors/board-mmio-pure` are correctly
  REJECTED (CDX2031, and CDX2031+CDX2033).

What remains is only the "Open question for later" at the foot of this
document, which is explicitly deferred: whether the compiler's own serial
chapters should ground too. The compiler is the trusted base and
self-compiles, so that is polish, not a hole.
