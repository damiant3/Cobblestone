...
**Test coverage is thinner than "with tests" implies.** In
`codex/test/apps/`, only three annotation-layer tests actually run in
the battery: `signed-annotation-test`, `historian-test`, and
`annotation-trust-filter-test`. Five are **skipped** —
`annotation-author-test`, `annotation-driver-test`,
`annotation-migrate-test`, `annotation-reader-test`, and
`annotation-transport-test` each carry a `.skip` sidecar reading
"stub: test body not yet written". (The `.codex` bodies were in fact
written later; the skip reason is stale, but the skip is real and the
tests do not run.) `historian-test-full` is marked `.failing`. So the
modules ship, and most of them ship unexercised. Un-skipping those five
— confirming each passes, or fixing what it finds — is real outstanding
...
`codex.annotations/` (six sidecar files, ~50 annotations across five
chapters; freshly renamed from `annotations/` in CL 1201) and this
document (413 lines through the original).
...
## B. The rename — `annotations/` → `codex.annotations/`

Done in CL 1201. Reasons:

- Every other top-level Codex tree follows the `codex.<quire>/`
  shape: `codex.foreword/`, `codex.foreword.{game,ai,ui,...}/`,
  `codex.kernel/`, `codex.os.{trust,net,verify,...}/`, `codex.works/`,
  `codex.test/`, `codex.build/`. Bare `annotations/` was the only
  exception, and it bracketed itself away from the rest of the
  tree visually.
- The lowercase form (`codex.annotations`, not `codex.Annotations`)
  matches every existing directory. Quire references in `cites` use
  `Annotations chapter X` (PascalCase) the same way `Foreword
  chapter Hamt` already works. Directory and quire name are
  separate decisions.
- The rename is **structural**: `p4 move` preserved file history,
  no content edits beyond the README updating its own paths.
- Inbound references checked and updated in the same CL to point
  at this addendum.
...
The system today annotates exactly five compiler chapters:
`codex/Emit/PeWriter`, `codex/Emit/X86_64Chapter`, `codex/IR/
LambdaLifting`, `codex/IR/Lowering`, `codex/Semantics/ChapterScoper`.
Roughly 50 annotation records total. The other ~340 chapters in
the depot have zero. The README diagrammed
`codex.foreword/codex.kernel/codex.os` subdirectories that don't
exist.
...
| CL | What |
|---|---|
| **A0** | Done in CL 1201 — rename `annotations/` → `codex.annotations/`, update README + Build.md, this addendum. |
...
|---|---|---|
| **A0** ✓ | `annotations/` → `codex.annotations/` rename, addendum | structural |
...
- Does **not** propose breaking the existing `codex.annotations/`
...
The current sidecar storage (`codex.annotations/*.json`) is a mutable
...
codex.annotations/
  ...
  codex/Emit/X86_64Chapter.json   — materialized view (unchanged format)
  codex/Emit/PeWriter.json
  ...
```
