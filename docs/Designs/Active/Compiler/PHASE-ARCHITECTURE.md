...
**Remaining work (the whole of it):**

1. **Precise escape roots for CHECK and LOWER.** PARSE (Document) and
   SCOPE (AChapter) are pinned and walk clean. CHECK and LOWER outputs
   have no typed root walk, so the conservative scan alone cannot
   separate their real escapes from false positives. The copying
   compactor stays blocked on this, and only on this.
2. **TCO-reset removal.** The phase boundary is the reclamation point;
   the within-phase compacting reset is redundant under the discipline
   and is still in the emitter.
...
6. 🟡 **Escape invariant enforcement.** Live and clean for the pinned PARSE and SCOPE roots. CHECK and LOWER still have no typed root walk. **Open work.**
7. ⬜ **Remove TCO reset.** Not yet addressed. **Open work.**
