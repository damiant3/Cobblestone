...
6. **Windows. IN PROGRESS.** Damian's ask, 2026-08-24, in his words:
   ...
   **What the arm found on the way, and it reorders the work.** Two of the
   ...
   | D.4 | **MEASURED AND RULED: the check, and the tree already ships one.** This row said `__alloc` checks nothing and that a check must be proven affordable against a self-compile. The first half is false at head and the second is now measured. **`heap-bump-reg` and `heap-bump-imm` already append `deck-guard-code` after EVERY heap bump**, not merely inside `__alloc`, behind `deck-guard-enabled` in `Core/BuildSettings.codex`, **and that flag is 1**: four instructions, a disarmed-cell test that skips, a `cmp r10, ceiling` and a `UD2`. **The cost, ablated at the flag and measured 2026-09-08 on seed 27D2386F7AF76F0C** (val, no seed landed): two compilers built from the same source by the same seed, one emitting guards and one not, each then used to compile the 3,154,973-byte self-concat. **Code size is +120,864 bytes on 3,102,429, which is +3.9% and exact.** **Time is BELOW THE NOISE**: ten runs each, both arms share a 5.08 s minimum, medians 5.23 against 5.20 and means 5.283 against 5.212 (1.4%), against a within-arm spread of 5.08 to 5.88. A difference smaller than the spread of repeats of the same point is not a reading (the sample of three that preceded it said 1.9% and could not be quoted either). **So the ruling is the CHECK, and the spawn pool's fixed slot regions are not needed for the bound.** What remains for D.5 is smaller than this row assumed: the shipping guard is a DECK CEILING, armed by `__deck-enter` and zero outside an extent, so D.5 arms the same mechanism on a REUSED HOLE's top rather than inventing one. **The ablation was nearly scored blind**: the first attempt wrote the flag to a depot-READ-ONLY file, the write was silently denied, and both arms compiled identical bytes; the identical hash is what caught it, and any future arm here fails unless the pattern hit count is 1 |
   | D.5 | **Reuse, and D.4's reading says what shape it has to take.** A pane open picks a dead buried entry whose hole fits instead of pushing a new mark; acceptance is unchanged, R6 falling toward zero while R5 stays put. **What D.4 settles is that the bound cannot be armed on the hole alone.** The ceiling the shipping guard compares against is a statement about the DECK CURSOR, not about the frontier: `PhaseAllocator.codex` records the defect measured 2026-08-27, where arming it at the reservation made the guard fire on the first ordinary allocation after every deck and the guarded compiler could not compile itself, `!EXC=06` at R10 0x14ADAECC. The reason is exactly the one D.5 walks into: **outside an extent R10 is the bivy frontier, which lives ABOVE the reservation by construction**, so a ceiling set to a hole's top condemns every later allocation that legitimately sits past it. **Therefore reusing a hole means ENTERING it**, moving R10 into the hole for as long as the pane owns it, which is what `__deck-enter` and `__deck-exit` already do on the nesting counter's zero crossings, with the arming and disarming that go with them. D.5 is then not a new bound but a pane lifetime expressed as an extent, and the hard part moves to where it belongs: **a deck extent is lexical and a pane's life is not.** A pane opens, the desk runs, and the pane closes later and out of order, so the enter and the exit are separated by the whole event loop and by other panes' extents; the LIFO the guard's counter assumes is the same LIFO the mark stack assumes, and both are what the campaign exists to break. **Nothing here needs a new firing arm**: the guard's fire is already recorded, by the sabotage `PhaseAllocator` names (writing `p + 4096` in place of `p + size` dies `!EXC=06` where the good kernel exits 0), and a battery arm cannot express it anyway, its pass condition being a fault |
