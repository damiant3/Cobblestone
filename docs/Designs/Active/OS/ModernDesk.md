...
**`desk-edit` is the known exception and stays exceptional.** The contract
records why it has no heap restore: `ged-init` parks an 8 MB buffer and a 1 MB
index pointer in a persistent `ds` cell. Under this design that stops being a
special case and becomes the NORMAL case -- a long-lived app with a
...
| # | Stage | Arm |
|---|---|---|
| 0 | **PART DONE 2026-08-18.** The frame counter is in and the numbers are below; the stack-depth reading is still missing. | Met, and improved on: `gsc-hud` prints `f<n> <ms>ms`, so ONE capture yields a rate and the two-shot arm becomes a check rather than the method. Control ran exactly as specified -- under a pinned clock the scene is byte-identical across ten seconds while the counter goes 598 to 1196, so frames PRODUCED are separated from picture CHANGED by measurement and not by argument. |
...
Browser and the 3D panes will need this same arm. Edit may not: its 9 MB never
moves out of its `ds` cell.
