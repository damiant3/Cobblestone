...
## Interim CSS Parallax (what we build now)

The landing page (`welcome.html`) uses CSS `perspective` and
`translateZ` on nested divs to simulate the same parallax effect with
...
- **Spark WASM backend** — `docs/Designs/Active/Backends/WasmBackend.md`
- **Codex UI library** — `codex.foreword.ui` (Scene, Animation, Camera)
- **Asset pipeline** — SVG or procedural generation for layer images

## Status

CSS parallax stand-in: building now.
WASM renderer: blocked on Spark WASM backend completion (Gap 9 in CurrentPlan).
