...
**Scope: still outside every sweep** are `apps/` other than `works` and `guios`,
and `codex/os/` other than `net`, `verify`, `trust` and `kernel`. Nothing is
known to hide there and nothing has looked.
...
| 13 | `Toml`, `Yaml`, `Uri`, `Smtp`, `Syslog`, `Tftp`, `Icmp` and `codex/foreword/encode/Markdown.codex` `md-parse` (harness callers only: `md-parse` is reached by `smtp-md-test`; the pages call the ui chapter); `Sntp` and `Uuid` have no parse entry point with a caller | KAT-ONLY / NO-TEST | caller census 2026-09-24; take them in that order. `Csv`, `Base64`, the ui chapter's `ui-md-parse` and `Hex` are guarded (`csv-parse-checked`, `base64-decode-checked`, `markdown-guard`, `hex-decode-checked`; `ExaminersAssay.md`). `KeyManager` imports through `hex-decode-checked`; `BrowserPersist`, `ServicesPersist`, `FactArchive` and `RepoProtocolPersist` still decode stored signatures and public keys through the lenient `hex-decode`, where a malformed field becomes wrong bytes (a signature that fails verification, a key nobody holds) rather than a refusal; `Jwt` still decodes its segments through the lenient `base64-decode-text`, safe for the signature because it signs the raw segment text, but a malformed segment is not refused; bare `csv-parse` still answers every input and builds each field one character at a time with `acc &`, quadratic in the field length. `Ini`: `ini-parse` has no caller, so a row |
| 17 | `codex/foreword/core/Fat32.codex` `fat32-parse-bpb32`, `fat32-resolve-path`, `fat32-read-bytes` (REACHED: `os/kernel/DriveManager.codex`, `apps/guios/FontLoad.codex`) | KAT-ONLY: `install-to-drive` reads a volume `fat32-format` made | the same class as `Fat16` one format over: `:44` divides by BPB byte 13 with no zero check; `:206` the dir entry's `de-size` is the read length bounded only by chain end; `:135`/`:146` the FAT chain walks have no cycle guard, so a volume whose chain loops walks forever |
| 18 | `codex/foreword/core/OtaBoot.codex` `boot-verify-candidate:204` | **LATENT, and the one piece of items 18-19 left open on purpose** | a 32-bit length word off flash reaches `sha256-buf slot-addr 0 len` with only `len <= 0` refused. Deliberately NOT fixed: there is no slot-size constant in the chapter to bound it against, the only honest bound is one the CALLER supplies, and `boot-load` has no production caller to supply it, so inventing a signature for an uncalled function is design work for whoever wires OTA up. `boot-load`'s apparent `FirstBoot.codex:223` cite is a SUBSTRING of `first-boot-load-mode`, not a call |
| 20 | **A CLASS, not a file: a bounds guard that ADDS can be overflowed** | the four sites with a u64 read or a text parse behind them are FIXED; the residue is a survey | `text-to-integer` of a 19-digit field answers i64 max, so `off + n` wraps negative, the sum is under any length, and the guard admits what it exists to refuse. **Refusing a negative length is not enough and is the trap**: the value was bounded, the arithmetic was not. The fix is to SUBTRACT, which cannot wrap. **The site count is not the work; the operand WIDTH is:** a wrap needs an operand that can approach i64 max, and a 1-, 2-, 3- or 4-byte wire field tops out at 255, 65535, 16.7M or 4.3e9. Every named additive guard in `Tls`, `Dtls`, `DtlsMessage`, `DtlsHello`, `TlsCert`, `TlsEndpoint`, `Asn1`, `CCE`, `Pattern` and `TextScan` was read by hand and is safe for exactly that structural reason. **NOT swept: the ~50 other files carrying the same width-capped shape**, which were not read individually and are not claimed. Account in `ExaminersAssay.md` "A Bounds Guard That ADDS Can Be Overflowed" |
...
- **`Rle`, `LoraLoader`, `PngMetadata` and `PromptParser` have no caller at
  ...
  (L-UNCALLED).
- **`fd-max-content-len` ships with NO ARM**, because reaching it needs a store
  image over 4 MB: on anything smaller the medium ceiling refuses the entry
  first.
...
- **`fb-parse-ttf`'s glyph-body counts are still unbounded by `next-off`.**
...
| wire framing+net | codex/foreword/encode/Csv.codex | Csv | csv-parse, csv-parse-row, csv-parse-quoted | NEGATIVE-ARM | codex/test/apps/csv-guard.codex (unterminated quote, text after a closing quote, quote inside an unquoted field) |
...
| storage | codex/foreword/core/OtaBoot.codex | OtaBoot | boot-load, boot-verify-candidate, boot-run | NEGATIVE-ARM | ota-boot-rollback.codex run-corrupt, run-no-flag; ALL THREE UNCALLED outside that test. This row said `boot-load` REACHED FirstBoot.codex:223 and that was a substring, not a caller: the name there is `first-boot-load-mode`, and FirstBoot does not cite Foreword chapter OtaBoot at all, so `boot-load` could not resolve in it (corrected fester 2026-08-21). The correction runs toward the bigger gap, which is the direction a caller claim is least often re-checked; :204-206 a 32-bit flash length word drives sha256-buf with only len <= 0 refused |
