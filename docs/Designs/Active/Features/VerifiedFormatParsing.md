...
| 9 | `apps/works/AgentBundle.codex` `gguf-parse-header`, digest and manifest-size refusals | ARMED 2026-08-28 (root, main 20464) | every refusal site in the chain has an arm in `build/agent-bundle-test.ps1`, each bundle honest at every link but its own and asserted not to refuse at a neighbouring link; 16/16 green. The manifest-absent branch alone stays untested: `build-img` refuses a model with no manifest (both or neither), so that image is not expressible with the shipping builder |
| 10 | `codex/foreword/encode/Modbus.codex`, `Coap` consumers `Lwm2mClient`, `Lwm2mFirmware` | **SURVEYED (reek, 2026-08-20). The three split three ways and only one is work.** `Modbus` is LATENT: `modbus-parse-registers`, `modbus-parse-coils`, `modbus-is-exception` and `modbus-exception-code` have NO production caller anywhere in the tree -- the only callers are `codex/test/modbus-encode.codex` itself -- so by this design's own rule an arm waits for a caller. Recorded for whoever writes that caller, because all four are genuinely unguarded: `modbus-parse-registers` reads `list-at pdu 1` without checking the PDU has two bytes, then trusts that byte as `count` and reads `2 + count` bytes with no check against the PDU length; `modbus-parse-coils-loop` indexes `2 + (i >> 3)` for a caller-supplied `quantity` with no length check; `modbus-is-exception` and `modbus-exception-code` index 0 and 1 unguarded. `Lwm2mClient` is GUARDED, measured rather than assumed: `lwm2m-server-request` refuses a path under two segments, `lwm2m-do-read`/`do-write` require three before indexing, and `idx` comes from `lwm2m-find`, which returns `0 - 1` or a `k` already less than `list-length (c.lc-res)`, so every `list-at (c.lc-res) idx` is in range by construction; `lwm2m-store` never indexes by `idx` at all, it walks and compares. `lwm2m-digits` IS an unbounded decimal accumulator (`acc * 10 + d`) off the wire, but its value only ever reaches equality tests and TLV fields, never an index or an allocation. **`Lwm2mFirmware` had a real UNBOUNDED WRITE and it is FIXED (reek, 2026-08-20, red's ruling).** `fw-stage-block` guards block ORDER (`num /= c.fw-next-block` refuses) but nothing bounds total BYTES: `fw-write` writes at `c.fw-stage + c.fw-written` and then grows `fw-written` by `list-length payload`, so a server sending in-order blocks with `more == 1` marches the write past the bank. Nothing downstream bounds it either -- `FlashBank` carries only a base and a fault count (`Board.codex:557`) and `flash-write-page` takes an ABSOLUTE address. RULED by red 2026-08-20 and built the same day: the BANK SIZE enforces (`Lwm2mFw` carries `fw-capacity`, `fw-write` refuses subtractively, and the check sits in `fw-write` because that is the only place a byte reaches the flash) and CoAP `Size2` is an EARLY refusal only, so a hint that is absent or lying low never weakens the bound. Four arms in `ota-lwm2m-loopback`, each isolated by ablation; account in `OTAFirmwareUpdate.md` "The staging bound" | industrial: actuation from a frame; `Coap` itself is guarded, its consumers are not |
...
| 20 | **A CLASS, not a file: a bounds guard that ADDS can be overflowed.** `71` sites compare `a + b` against a `list-length` or `text-length`; **34 have a non-constant second operand**. Found by blu in reek's own item-15 `sdw-decode` bound one day after it landed | IN PROGRESS (reek, 2026-08-18), and **the site count is not the work; the operand WIDTH is.** Re-measured with a wider regex the shape appears at ~96 sites in 55 files, and the two counts are different instruments rather than a disagreement (mine also matches prose quoting the old form, `SourceDefWire:162`). Neither count discriminates, because **a wrap needs an operand that can approach i64 max, and a field's width decides that**: a 2-, 3- or 4-byte length off a wire tops out at 16.7M or 4.3e9, and no offset bounded by a real buffer pushes that to 9.2e18. The sites that matter take a length or offset from a **u64 read or a text parse**. Screening every file with both an additive length guard and such a read leaves **four**: `SourceDefWire` (already subtractive, blu 15641), `TypeChecker:2380` (internal indices, not foreign bytes), `SpirvBinary:345` (`wc` is the high 16 bits of a word, at most 65535, and the input is our own emitter's output), and `Gguf`. **`Gguf` was live and is FIXED (main 16693)**: `gguf-fits`, the guard every other guard in that chapter is built on, was `(off + n) <= list-length data` with both operands read as u64s, and asked directly on the seed it answered True for a read of 8 bytes at offset 9223372036854775800 into a ten-byte list. **`MessageFraming.frame-fits` also made subtractive (16695) with no live path**, on the `repo-has` precedent: its two call sites pass a 32-bit length and a narrowed offset, but `frame-read-le64` is in the same chapter and it is the most-trafficked parser in the tree. **The named files are now READ BY HAND and every one is safe, for one structural reason: the addend is a FIXED-WIDTH WIRE FIELD.** `Tls:508` enc-len and `Dtls:266` n are two bytes (65535); `DtlsMessage:124` flen and `TlsEndpoint:199` len are three (16.7M); `DtlsHello:112` n and `TlsCert:243` ctx-len are one (255); `DtlsHello:169` elen is two; `Asn1` is capped by its own `n > 4` refusal one line above the guard, so its long-form length cannot exceed four octets. In `CCE:443` the addend is `utf8-decode-length`, which answers 1 to 4; in `Pattern:126` and `TextScan:55` it is `text-length` of a text that exists. **None of these can approach i64 max and none of them needs changing.** The class is dangerous exactly where a length is NOT width-capped: a u64 read or a text parse. **The parameter-passed residual is closed too**: the tree has three `List Integer, Integer, Integer -> Boolean` bounds predicates, and `lz4-fits` was already subtractive (`off <= list-length input - n`) while the other two are the ones fixed above. What is NOT swept is the ~50 other files carrying the same width-capped shape; they were not read individually and are not claimed. **A wrap probe must be checked for actually wrapping**: the first `frame-fits` probe used 9223372036854775800, whose sum does not reach i64 max, and every row came back green against a guard that was in fact defeated. | `text-to-integer` of a 19-digit field answers i64 max and `46 + i64max` wraps negative, so the sum is under any length and the guard admits what it exists to refuse; the guest died `!EXC=06` against main 15576. The fix is to SUBTRACT (`clen > text-length - (p9 + 1)`), which cannot wrap. **Refusing a negative length is not enough and is the trap**: the value was bounded, the arithmetic was not. `sdw-decode` fixed by blu (15614); `repo-has` made subtractive by reek with an arm, no live path there since every call site passes a constant. The other 33 are per-site judgement in owned files (`Tls`, `Dtls*`, `Asn1`, `TlsCert`, `TlsEndpoint` are the net leg's; `CCE`, `Pattern`, `TextScan` foreword) and most are probably safe. Account: `ExaminersAssay.md` "A Bounds Guard That ADDS Can Be Overflowed" |
