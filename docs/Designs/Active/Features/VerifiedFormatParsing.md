...
**Ruling (Damian, 2026-08-05): stage 0 is APPROVED as schedulable background
work.** It needs no design approval and is not seed-affecting; any lane with
slack may take it. Stages 1-4 remain proposal.

**This design owns Track D of `docs/PM/CurrentPlan.md` ("bytes we did not
produce"), and stage 0 has a CENSUS, section 10. Section 10.1 is the work
queue, in priority order. Take from it; do not fix the nearest parser.**
...
### 10.1 The open gaps, in the order the next agent should take them
...
queue.** Every row closed since the census was written has its account in
`docs/ExaminersAssay.md` under the guard's own name, and the CL that landed it;
nothing about a closed row is repeated here.

**Reachability decides the order.** A parser nothing calls is untested
(L-UNCALLED) but it is also not exposure, and stage 0 is about code that ships.
**RULED (red, 2026-08-16): guard a chapter if it has ANY caller, production or a
harness that RUNS it; a chapter with no caller at all gets a row saying so under
L-UNCALLED and no guard.**

**`codex/test/forewords/` cannot be dismissed as a directory: check the file.**
Of 316 files, 299 are under 400 bytes and print a string, while 16 are over
1,000 bytes and are real harnesses that call the chapter's entry points. A
generalisation over the directory was made once and was false, and it would have
exempted chapters that have a runner. `codex/test/apps/foreword-all-compile.codex`
cites 417 chapters and its entire body is one `print-line-uni`, so a citer is not
a caller.

**Scope:** every tree has had one census (rows 21 and 23). The `apps/` census
found readers by NAME and accumulators by SHAPE; a reader with neither
(L-CENSUS) is not reached.

| # | parser | state | why it ranks here |
|---|---|---|---|
| 7 | `codex/os/trust/PeerDiscovery.codex`, `codex/os/net/OAuthClient.codex`, `codex/os/net/ImapClient.codex` | **no caller of any parse entry point** (2026-09-24), so each is a row under L-UNCALLED, not a guard | `PeerDiscovery` has no byte parser: a `DiscoveryBeacon` is built in-process and nothing deserializes one; **latent:** `discovery-process-beacon` never reads `beacon.signature` and trusts the claimed `sender-key`. `oauth-parse-tokens` is called by nothing (`OAuthProvider` and `ExternalAuthBridge` cite the chapter for other functions); **latent:** a body with no `access_token`, or JSON that is not an object, answers `OAuthSuccess` with an empty token, where RFC 6749 5.1 requires `access_token` and `token_type`. `imap-parse-*` is called by nothing (`imap-wire` exercises the framing only); **latent:** `imap-num-loop`'s `acc * 10` traps on a long digit run, and `imap-parse-response-kind` reads any line containing " OK " as success without checking the tag. The four codecs this row named are guarded (`ExaminersAssay.md`, the CBOR, MessagePack, Protobuf and Bencode guards) |
| 8b | `codex/os/kernel/VirtioBlk.codex`: `peek-16 d0 14` is a used-ring index the DEVICE writes and the driver follows with no bound | NO-TEST, **unowned** | the ARM64/virtio path has no bed that can feed it a hostile ring today, so it waits for one or for the Renode bed |
| 10 | `codex/foreword/encode/Modbus.codex` | **LATENT**: `modbus-parse-registers`, `modbus-parse-coils`, `modbus-is-exception` and `modbus-exception-code` have NO production caller; the only callers are `codex/test/modbus-encode.codex` itself | recorded because all four are genuinely unguarded and the arm waits for a caller: `modbus-parse-registers` reads `list-at pdu 1` without checking the PDU has two bytes, then trusts that byte as `count` and reads `2 + count` bytes with no check against the PDU length; `modbus-parse-coils-loop` indexes `2 + (i >> 3)` for a caller-supplied `quantity` with no length check; `modbus-is-exception` and `modbus-exception-code` index 0 and 1 unguarded |
| 11 | `codex/foreword/encode/VideoCodec.codex` | **no byte parser, and `video-decode-frame` has no caller** (2026-09-24: `MediaPlayer` and `media-codec-test` cite the chapter for other functions), so a row under L-UNCALLED | a `VideoFrame` is an in-process record. **Latent:** `apply-deltas` writes each delta into the caller's `prev` list with `list-set-at`, so decoding a delta frame rewrites the previous frame the caller still holds (L-ALIAS), and `pd-offset` is never bounded against the frame. `Qoi` and `Bmp` from this row are guarded (`ExaminersAssay.md`) |
| 13 | The row's parsers are guarded; `Sntp` and `Uuid` have no parse entry point with a caller (caller census 2026-09-24) | RESIDUE ONLY | `Csv`, `Base64`, the ui chapter's `ui-md-parse`, `Hex`, `Toml`, `Yaml`, `Uri`, `Smtp`, `Syslog`, `Tftp`, `Icmp`, `Jwt` and the encode chapter's `md-parse` are guarded (`csv-parse-checked`, `base64-decode-checked`, `markdown-guard`, `hex-decode-checked`, `toml-guard`, `yaml-guard`, `uri-guard`, `smtp-guard`, `syslog-guard`, `tftp-guard`, `icmp-parse-checked`, `jwt-guard`, `md-encode-guard`; `ExaminersAssay.md`). `KeyManager`, `BrowserPersist`, `ServicesPersist`, `FactArchive` and `RepoProtocolPersist` decode stored keys and signatures through `hex-decode-checked` (`persist-hex-guard`; a tombstone keeps its path and loses a malformed signature, because the index replay reads its path unverified); bare `csv-parse` still answers every input, by design beside `csv-parse-checked`. `Ini`: `ini-parse` has no caller, so a row |
| 18 | `codex/foreword/core/OtaBoot.codex` `boot-verify-candidate:213` | **LATENT, left open on purpose** | a 32-bit length word off flash reaches `sha256-buf slot-addr 0 len` with only `len <= 0` refused. Deliberately NOT fixed: there is no slot-size constant in the chapter to bound it against, the only honest bound is one the CALLER supplies, and `boot-load` has no production caller to supply it, so inventing a signature for an uncalled function is design work for whoever wires OTA up |
| 21 | `codex/os/` `core`, `dev`, `observe`, `replay` and `sched` (census 2026-09-25, by reader name and by indexed-read shape, 63 chapters) | RESIDUE ONLY | `ExprEval` is guarded (`ExaminersAssay.md`, the debugger expression guard). `ProgramRegistry`'s `extract-caps` adds two u64 header fields unchecked, but `registry-install` reaches it only after `verify-cdx-full` accepts, and the integrity phase bounds both fields against the length first. `ReplayCrf`'s `crf-decode` and `QmkProtocol`'s `via-parse-*` are length-checked and have no production caller. `HexFormat`'s `hexfmt-parse-hex` has no caller; **latent:** `acc * 16` traps past 15 digits. Every indexed read in `observe` and `sched` walks an in-process list |
| 23 | `apps/` outside `works` and `guios` (census 2026-09-25: reader names with a caller, then every unbounded `acc * 10` or `acc * 16` accumulator) | **OPEN: nine readers trap on a 20-digit field and have no channel to refuse through** | Guarded: `IdeaEngine` `parse-int-text`, `CellFormula` numbers and row numbers, `CellCsv` (an overlong field stays text), `mathbook` `Parser` (`ParseErr`), `BulkLoader` (`ValNull`); `ExaminersAssay.md`, "App Integer Literals". `AccpJson`'s `aj-int` refuses past 9 digits first; `FullText` reads ids its own index wrote. **Not fixed, because answering a value in place of the refusal is L-BAILVALUE (root, 2026-09-25); each needs the named channel first:** `ChatPage` `text-to-int` (a -1 or Maybe its four callers test), `cvmm` `Calculator` `text-to-int` (an error display state), `ExplorerTheme` `text-to-int` (a Maybe for its DOM callers), `WorkflowEngine` `parse-int` (a condition that fails on a non-number), `PageCompiler` `page-tokenize-number` (an error token), `WaLoad` `wa-scan` (a rejected-line count; the slot is u32). In `apps/games` (val's claim, register only): `GameServer` and `codexmagic` `ServerUtil` `parse-int-loop`, both server-side, and `magic` `OracleParse` `parse-int-prefix`. `IdeaPage` `read-digits` has the shape but compiles to JavaScript, where it cannot trap. **Latent, not a trap:** `ExplorerServer` and `IdeaEngine` `url-decode` read `%zz` as byte 0 where WHATWG keeps the three characters |

**Residues named by closed rows, each one deliberate:**

- **`Brotli` carries no guard and that is the measured answer.** A termination
  arm proved a NEGATIVE worth keeping: deleting the `brotli-valid` gate and
  widening the `dist < 1` refusal each moved ZERO rows, because hostile headers
  and truncated streams never decode far enough to reach a distance command.
  `brotli-valid` is not load-bearing and its `w <= 24` half is unreachable by
  construction. Reaching the real guards needs a crafted bitstream past the
  Huffman tables, which is a FUZZ-CORPUS job and is the honest residue.
- **`Rle`, `LoraLoader`, `PngMetadata` and `PromptParser` have no caller at
  all**, measured rather than assumed, so they carry a row and no guard
  (L-UNCALLED).
- **`apps/works/AgentBundle.codex`'s manifest-absent branch is untested**, because
  `build-img` refuses a model with no manifest (both or neither), so that image
  is not expressible with the shipping builder.
- **`ttf-parse` in `encode/` and `TrueTypeFont` stay latent**, reached only by
  `FontExtract`.

**A wrap probe must be checked for actually wrapping.** The first `frame-fits`
probe used 9223372036854775800, whose sum does not reach i64 max, and every row
came back green against a guard that was in fact defeated (L-VACUOUS).

**Guarding a consumer does nothing for an input its producer cannot survive.**
The `ui/` `upem` guards sit DOWNSTREAM of `ttf-parse`, so a truncated font never
reached them.

**A guard that stops a crash can leave a wrong answer.** With `dec-pow10` made
safe, the unclamped `Decimal` parser does not fault at all and parses
`1.1234...` to `-8.000...`: a fault-hunting pass would have fixed the divisor,
watched the crash stop, and shipped the wrong number. The same shape is
`Pbkdf`'s truncated stored hash, which verified True against the right password
and never faulted, and `Schedule`'s four ACCEPT defects. **Fixing the crash is
not fixing the parse.**

**Fix the shared primitive, not the input path.** `Decimal` had three producers
of an out-of-range scale, not one: the text parser, `dec-mul` (which ADDS the
scales, so two scale-32 values reach 64 with no text involved) and `dec-new`
(which takes a scale from its caller). Guarding only the parser would have been
L-CAPABILITY one level down.

**How to take one.** The pattern is settled and the worked examples are in
`docs/ExaminersAssay.md` ("The UDP Frame Guard", "The CDX Input Guard", "The GPT
Integrity Guard", "The FAT Geometry Guard", "The Config-Descriptor Clamp").
Mutate the KAT vector: truncate at every field boundary, extend, flip each
length field to zero and to maximum, zero the magic, nest to the fuel cap.
Require no crash, no hang, and no accept of a mutant that should be refused.
Keep the well-formed vector as the positive control and a
well-formed-but-refused-for-another-reason arm as the discriminator. **Clamp
where the length decides a slice, refuse where it decides WHERE a read lands.**
Predict every expected value before the arm runs, and ablate each guard
separately to prove the arm moves.

**A chapter on the compiler's own boot or store path is inside the seed's
reachable set** (`Fat16` cites `Gpt`; `opening.codex` cites `FactDisk`), so
whoever takes one takes the token for the gate. Measure the cite rather than
assuming it from the row: `Gguf` was NOT seed-affecting, because only
`AgentBundle` cites it.

### 10.2 The full table

Encode-only modules are listed so nobody re-audits them.
...
| trust decisions | codex/foreword/encode/Jwt.codex | Jwt | jwt-decode, b64url-decode-utf8, jwt-utf8-ok | NEGATIVE-ARM | codex/test/apps/jwt-guard.codex (base64url alphabet, padding, length, strict UTF-8, CCE refusal; RFC 7519 section 3.1 token accepted); jwt-decode-test.codex: two-segments, bad-payload -> JwtMalformed |
...
| loading authority | codex/os/verify/CdxBinary.codex | CdxBinary | cdx-verify-magic, cdx-verify-content-hash, decode-effects, decode-proof-section | NEGATIVE-ARM | codex/test/apps/cdx-binary-test.codex: cdx-verify-magic on [0,0,0,0]; codex/test/apps/cdx-section-guard.codex: a section past the end, an overflowing offset plus size, a negative offset and a 30-byte file each answer `hash-ok=False` (`cdx-section-fits`) |
...
| session establishment | codex/foreword/encode/WebSocket.codex | WebSocket | ws-decode-frame, ws-frame-size, ws-read-length | NEGATIVE-ARM | codex/test/apps/websocket-frame-guard.codex (truncation at every field, 2^32 length, top-bit length) |
...
| media+documents | codex/foreword/encode/Bmp.codex | Bmp | bmp-decode, bmp-decode-rows, bmp-decode-row | NEGATIVE-ARM | codex/test/apps/bmp-guard.codex (row order both ways, header fields, truncation, hostile height, offset) |
...
| media+documents | codex/foreword/encode/Qoi.codex | Qoi | qoi-decode, qoi-decode-pixels, qoi-read-be32 | NEGATIVE-ARM | codex/test/apps/qoi-guard.codex (header, end marker, RGBA, channel wrap, op bounds, pixel count) |
...
| media+documents | codex/foreword/encode/TrueType.codex | TrueType | ttf-parse, ttf-read-dir, ttf-read-glyph | NEGATIVE-ARM | `ttf-resolve-glyph` bounds a glyph body by the next `loca` offset (256 contours, 4096 points, instructions inside the body) and refuses falling contour end-points (codex/test/ttf-contour-order-guard.codex); codex/test/apps/truetype-truncated-guard.codex: eight truncation points each answer `upem=0 render=1x1` (`ttf-byte-at` answers zero past the end and `gr-upem-ok` refuses the zero); `ExaminersAssay.md`, the Truncated Font Guard |
...
| wire framing+net | codex/foreword/encode/Base64.codex | Base64 | base64-decode-checked, base64-valid, base64-decode | NEGATIVE-ARM | codex/test/apps/base64-guard.codex (out-of-alphabet, after the padding, short length, lone padding); bare `base64-decode` still answers every input |
| wire framing+net | codex/foreword/encode/Bencode.codex | Bencode | ben-decode, ben-decode-at, ben-decode-int | NEGATIVE-ARM | codex/test/lib/bencode-guard.codex (integer and length grammar, overflow at both edges, lengths past the end, depth, trailing text) |
| wire framing+net | codex/foreword/encode/Cbor.codex | Cbor | cbor-decode, cbor-decode-at, cbor-read-arg | NEGATIVE-ARM | codex/test/lib/cbor-guard.codex (truncation, reserved and indefinite ai, 8-byte arguments, depth, trailing bytes, floats, UTF-8) |
...
| wire framing+net | codex/foreword/encode/Csv.codex | Csv | csv-parse, csv-parse-row, csv-parse-quoted | NEGATIVE-ARM | codex/test/apps/csv-guard.codex (unterminated quote, text after a closing quote, quote inside an unquoted field; parse and emit scaling linear) |
...
| wire framing+net | codex/foreword/encode/Hex.codex | Hex | hex-decode-checked, hex-well-formed, hex-decode | NEGATIVE-ARM | codex/test/apps/hex-guard.codex (odd length, non-hex character; KeyManager import refusals); bare `hex-decode` still answers every input |
...
| wire framing+net | codex/foreword/encode/Json.codex | Json | json-parse, json-parse-value, json-parse-object | NEGATIVE-ARM | codex/test/forewords/encode-json-numbers.codex: "1.", "1e", "1e+", ".5", "1e99" all refused; codex/test/apps/json-cost-guard.codex: string read and emit scaling linear |
| wire framing+net | codex/foreword/encode/Markdown.codex | Markdown | md-parse, md-parse-heading, md-parse-maybe-blockquote | NEGATIVE-ARM | codex/test/apps/md-encode-guard.codex (bare `#` and `######` trapped on the seed; `#hello` and `>x` lost a character); fenced code content is still parsed line by line as markdown |
| wire framing+net | codex/foreword/encode/MessagePack.codex | MessagePack | msgpack-decode, mp-decode-at, mp-decode-item | NEGATIVE-ARM | codex/test/lib/msgpack-guard.codex (every width truncated, signed widths, unused and ext bytes, depth, trailing bytes, UTF-8) |
...
| wire framing+net | codex/foreword/encode/Protobuf.codex | Protobuf | pb-decode-varint, pb-decode-field, pb-read-le | NEGATIVE-ARM | codex/test/lib/protobuf-guard.codex (cut and overlong varints, lengths, fixed widths, field range, wire types, two's complement) |
| wire framing+net | codex/foreword/encode/Smtp.codex | Smtp | smtp-parse-response, smtp-reply-shape, smtp-session-checked | NEGATIVE-ARM | codex/test/apps/smtp-guard.codex (8 malformed replies, a continuation, dot-stuffing, 5 injections); bare `smtp-build-session` still sends any field unchecked |
...
| wire framing+net | codex/foreword/encode/Toml.codex | Toml | toml-parse, toml-parse-lines, toml-parse-kv | NEGATIVE-ARM | codex/test/lib/toml-guard.codex (a stated subset; 18 refusals, duplicates, tables, 200,000 keys) |
| wire framing+net | codex/foreword/encode/Uri.codex | Uri | uri-parse, uri-parse-relative, uri-parse-authority | NEGATIVE-ARM | codex/test/apps/uri-guard.codex (a stated RFC 3986 subset; 12 refusals including the userinfo host confusion and a 20-digit port) |
...
| wire framing+net | codex/foreword/encode/Yaml.codex | Yaml | yaml-parse, yaml-map-loop, yaml-scalar | NEGATIVE-ARM | codex/test/lib/yaml-guard.codex (a stated subset; 24 refusals, duplicates, 200,000 keys and items) |
...
| wire framing+net | codex/os/net/Tftp.codex | Tftp | tftp-parse, tftp-parse-request | NEGATIVE-ARM | codex/test/apps/tftp-guard.codex (14 malformed packets; transfer arms `dup`, `skip`, `after-last`, `block-0`, `reused`, and `transfer` scaling linear); harness callers only |
| wire framing+net | codex/os/net/Syslog.codex | Syslog | syslog-parse, syslog-pri-ok, syslog-parse-digits | NEGATIVE-ARM | codex/test/apps/syslog-guard.codex (12 malformed PRI or framing arms, UTF-8 both directions); harness callers only |
| wire framing+net | codex/os/net/Icmp.codex | Icmp | icmp-parse-checked, icmp-parse | NEGATIVE-ARM | codex/test/apps/icmp-guard.codex (short, bad checksum, a value above 255); bare `icmp-parse` still answers every input, a short packet as an echo reply; no production caller (CurrentPlan Track B) |
...
| wire framing+net | apps/works/RepoProtocolPersist.codex | RepoProtocolPersist | source-def-from-text, tombstone-from-text, proposal-from-persist-text, verdict-from-persist-text | NEGATIVE-ARM | codex/test/apps/repo-tombstone-replay.codex: replayed and stale-timestamp records must not override newer ones; persist-hex-guard.codex: a malformed hex field refuses the line |
...
| wire framing+net | apps/works/AgentBundle.codex | AgentBundle | ab-parse-model, ab-window-step, gguf-parse-header (via wrapper) | ARMED 2026-08-28 | `build/agent-bundle-test.ps1` (root, main 20464): signature, digest, size, parse and manifest-shape refusals each have an arm with wrong-link discrimination; the digest-tamper arms predate this and the old note here was stale about them |
...
| other | codex/os/trust/FactSync.codex | FactSync | sync-decode-hashes, sync-decode-facts, sync-decode-fact | NEGATIVE-ARM | codex/test/apps/fact-sync-truncated.codex: a truncated reply leaves the store untouched, a truncated offer harvests nothing, a count of 4294967295 over four bytes; codex/test/apps/wire-payload-refuse.codex: a correctly signed envelope over a short payload is refused (`ExaminersAssay.md`, the Wire Payload Refusal) |
...
|---|---|---|---|---|---|
| storage | codex/foreword/core/Gpt.codex | Gpt | gpt-read, gpt-read-after-hdr, gpt-parse-entry | NEGATIVE-ARM | codex/test/apps/gpt-core-read, gpt-core-size-256, gpt-core-size-guard, gpt-core-count-guard: `gpt-header-geom-ok` refuses an entry LBA below 2, a count outside 1..1024, a size outside 128..512 or not a multiple of 128, and an array ending past FirstUsableLBA (`ExaminersAssay.md`, the Foreword GPT Geometry Guard); at every admitted size each entry, its name included, lies inside its 512-byte sector; install-to-drive.codex reads back our own gpt-write-table |
| storage | codex/foreword/core/Fat32.codex | Fat32 | fat32-parse-bpb32, fat32-resolve-path, fat32-read-bytes | NEGATIVE-ARM | fat32-cluster-guard.codex, fat32-read-guard.codex (cycle, short chain, bad first cluster refused; a cycle past the end of the size read); install-to-drive.codex on our own fat32-format volume |
| storage | codex/foreword/core/Fat16.codex | Fat16 | fat16-parse-bpb, fat16-read-dir-entry, fat16-read-bytes, fat16-next-cluster | NEGATIVE-ARM | `fat16-read-guard.codex`: the file read refuses a bad cluster, a short chain and a cycle, as FAT32 does; `fat16-cluster-guard.codex` and `fat16-cycle-guard.codex` (`ExaminersAssay.md`, the Foreword Cluster Guard and the Chain Cycle Guard); the geom-guard refusal lives in Works GopFat16 above it; REACHED from `opening.codex` and `GopFat16.codex`. `fat16-read-text` keeping CR (CCE 255) is an ASSERTED CONTROL, `codex/test/fat16-source-cr.expected` pins `41 58 255 1 50 48` against `fat16-read-source` dropping byte 13; not a finding (blu, 2026-08-16) |
| storage | codex/foreword/core/FactDisk.codex | FactDisk | store-read-bundle, fd-fold, fd-entry-content | NEGATIVE-ARM | factdisk-hostile-head.codex: `fd-fold-entry` refuses an entry over `fd-max-content-len` (4 MB) or running past the store's end, and `fd-head-credible` a log head the device cannot hold (`ExaminersAssay.md`, the Fact Store Length Guards); `build/factdisk-cap-test.ps1` (factdisk-cap.codex) pins the cap itself, one entry at it and one past it; factdisk-read.codex well-formed sidecar; REACHED opening.codex:2101 |
...
| storage | codex/foreword/core/SourceDefWire.codex | SourceDefWire | sdw-decode, sdw-hex-decode | NEGATIVE-ARM | codex/test/apps/source-def-wire-guard.codex: the ninth field's length refused when negative, past the line or overflowed (9 arms; `ExaminersAssay.md`, the Fact Store Length Guards); foreword-source-def-wire.codex: shape; REACHED FactDisk.codex:128, RepoProtocolPersist.codex:146. The same test's arms 10 to 13: a signature field of odd length, a non-hex digit or an uppercase digit refuses the record (`sdw-hex-ok`) |
| storage | codex/foreword/core/OtaBoot.codex | OtaBoot | boot-load, boot-verify-candidate, boot-run | NEGATIVE-ARM | ota-boot-rollback.codex run-corrupt, run-no-flag; ALL THREE UNCALLED outside that test; :212-213 a 32-bit flash length word drives sha256-buf with only len <= 0 refused |
...
| session establishment | codex/foreword/core/Tls.codex | Tls | tls-decode-record, tls13-record-decrypt, tls13-strip-inner | NEGATIVE-ARM | tls13-record.codex tamper, wrong-seq; codex/test/apps/tls-record-guard.codex: a record shorter than its header, and a declared payload longer than the bytes behind it, answer `tls-rec-valid` False (`ExaminersAssay.md`, the TLS Record Header Guard); decrypt REACHED from TlsEndpoint, tls-decode-record has no caller outside the chapter |
...
| trust decisions | codex/foreword/core/Aes256.codex | Aes256 | aes256-pkcs7-unpad, aes256-cbc-decrypt | NEGATIVE-ARM | codex/test/crypto-test.codex: `aes256-cbc-partial` (17 bytes), `unpad-mismatch`, `unpad-zero`, `unpad-too-large`, `unpad-partial`; `aes256-pkcs7-unpad` refuses a length not a multiple of 16, a pad byte outside 1..16 and a padding run that disagrees; REACHED IdentityManager, GopWizard |
| wire framing+net | codex/foreword/core/CCE.codex | CCE | utf8-bytes-to-text, utf8-code-point-at, cce-decode-at | NEGATIVE-ARM (utf8) / KAT-ONLY (cce-decode-at) | utf8-cce-test.codex utf8-truncated-dropped, utf8-stray-cont-dropped; REACHED Validation.codex:164 (`cce-decode-at`), CellCsv.codex:43 (`utf8-bytes-to-text`) and all 6 apps/sheets chapters (2026-09-24); :167-176 cce-decode-at reads offset+1..+3 with no check against the text length |
...
| wire framing+net | codex/foreword/core/Schedule.codex, Pattern.codex, Decimal.codex | Schedule, Pattern, Decimal | schedule, pattern, dec-from-text (user text) | NEGATIVE-ARM | codex/test/apps/schedule-parse-guard.codex (four malformed shapes that answered a wrong schedule now answer `None`), pattern-progress-guard.codex (a zero-width `many` and `repeat`'s absolute position), decimal-scale-guard.codex (a scale past 18 from the parser, `dec-mul` and `dec-new`); `ExaminersAssay.md` under each guard's name; all UNCALLED outside tests |
...
| media+documents | codex/foreword/ai/Gguf.codex | Gguf | gguf-parse-header, gguf-tensor-info-offset, gguf-md-scan, gguf-parse-tensor-info | NEGATIVE-ARM | codex/test/apps/gguf-hostile.codex: `gguf-fits` (subtractive) is asked before every read at a file-supplied offset, including `gguf-md-scan` and the tensor-info walk, and each caller refuses on its own channel (`-1`, `""`, `gh-valid`, `gti-valid`); `ExaminersAssay.md`, the GGUF Bounds Guards; build/gguf-foreign-test.ps1 real llama.cpp models as KAT; REACHED from AgentBundle |
| media+documents | codex/foreword/ai/SafeTensors.codex | SafeTensors | st-parse-file, st-find-tensor, st-load-tensor | NEGATIVE-ARM | foreword-safetensors.codex short file, garbage header; codex/test/apps/safetensors-bounds-guard.codex: a well-formed header describing a tensor the file does not hold (`ExaminersAssay.md`, the SafeTensors Read Bounds); cited for loading by ModelRegistry, LoraLoader, ImageTo3d, TextEncoder, UNetXL and VaeDecoder (2026-09-24) |
...
| media+documents | codex/foreword/ai/GpuProxy.codex | GpuProxy | gpu-parse-result, gpu-f32-bytes-to-tensor | NEGATIVE-ARM | codex/test/apps/gpu-result-guard.codex: three guards (the 40-byte result, the 4-byte status, `rows * cols` against the buffer), each ablation killing only its own row (`ExaminersAssay.md`, the GPU Result Bounds); UNCALLED outside tests |
| media+documents | codex/foreword/ui/TrueTypeFont.codex, GlyphRasterizer.codex | TrueTypeFont, GlyphRasterizer | ttf-load-font (delegates to encode ttf-parse); gr-render-glyph, gr-render-glyph-aa | NEGATIVE-ARM | codex/test/apps/glyph-upem-guard.codex: `unitsPerEm` 0 (divide error) and 1 (double fault from an 11,201 px glyph) refused by `gr-upem-ok`; truetype-truncated-guard.codex feeds it truncated fonts (`ExaminersAssay.md`, the Glyph unitsPerEm Guards); UNCALLED (FontExtract calls encode ttf-parse directly) |
| media+documents | codex/foreword/ui/Markdown.codex | Markdown | ui-md-parse, md-parse-inline | NEGATIVE-ARM | codex/test/lib/markdown-guard.codex (heap linear in line length, code block and run count; 200,000 lines and 100,000 runs in constant stack; a 10-digit list marker is a paragraph); REACHED CallPage, RiverPage, Notebook, WorkflowHtml, DetailPane |
| compression | codex/foreword/compress/Deflate.codex | Deflate | deflate-decompress, deflate-blocks, deflate-dynamic-block | NEGATIVE-ARM | codex/test/apps/deflate-hostile.codex: `deflate-copy` refuses a negative source (all three call sites, Brotli's included), `br-exhausted` stops a truncated dynamic block, `deflate-after-stored` refuses a stored header cut short or whose NLEN is not the complement of LEN (`ExaminersAssay.md`, the Deflate Decoder Guards); UNCALLED outside tests (Brotli reuses BitReader/dh-decode-sym/deflate-copy) |
| compression | codex/foreword/compress/Lz4.codex | Lz4 | lz4-decompress, lz4-decompress-loop, lz4-read-extra-length | NEGATIVE-ARM | codex/test/apps/lz4-hostile.codex: literal run past the stream, offset byte past the end, distance 0, and a back-reference before the output start (which fabricated bytes rather than crashing), all clamped subtractively (`ExaminersAssay.md`, the Lz4 Decompressor Guards); production caller FactArchive only, which checks its hash after decoding |
| compression | codex/foreword/compress/Lz77.codex | Lz77 | lz77-decompress | NEGATIVE-ARM | codex/test/apps/lz77-hostile.codex: `start < 0`, `off <= 0` and `mlen > lz77-max-match` (`ExaminersAssay.md`, the Lz77 Decompressor Guards); run by `lz77-test`, no production caller |
...
| compression | codex/foreword/compress/Brotli.codex | Brotli | brotli-decompress, brotli-blocks, brotli-cmd-loop, brotli-valid | NEGATIVE-ARM (termination only) | brotli-test.codex brotli-invalid (brotli-valid []); codex/test/apps/brotli-hostile.codex: hostile headers and truncated streams terminate, and reach none of the distance guards (`ExaminersAssay.md`, the Brotli Termination Arm); b01-b24, brotli-ctx2-test, brotli-dict-test, build/brotli-read-test.ps1 all KAT; UNCALLED; :2370-2371 bounded by need, :2405 dist < 1 refused, :2409 zero progress refused; :1901 mlen up to 2^24 from a tiny header sizes the output, :2493-2495 fill-lens delta unbounded against n |
...
`AesGcm`); the whole of
`compress/` and most of `ai/` are latent.
...
- **`Gguf`'s llama.cpp measurement is a KAT, not a negative arm**; its
  negative arm is `gguf-hostile` (the second sweep's row).
...
- **One contract note outside the census's subject, recorded here so it is
  not lost:** `KeyInput.codex:191` (`efi-key-decode`) returns the firmware's `UnicodeChar` raw
