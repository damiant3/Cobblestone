...
Status: planned (2026-06-10, reek). Execute phase by phase, one CL at a
time, after a fresh session init. This document is the complete context
for that session — read it before touching apps/erp.

## Where it stands

The comment "it's just a schema, no app there" is half right. The 21
chapters hold 194 records + 108 variants and ~154 functions, of which
~65 are real engines: FinGL posts balanced journal entries with period
control and produces trial balance / statements; PpPlanning explodes
BOMs with scrap and runs single-level MRP; SdSales has a condition
pricing engine; QmQuality computes SPC limits with an integer sqrt;
IsBanking amortizes loans. What makes it not-an-app:

1. **It cannot even be built.** Chapters cite `ERP chapter X` and no
   resolver knows the quire. `build/quire-map.ps1` has no entry;
   `codex/plugs/common/plug-build-lib.ps1` auto-derives `Erp` (case
   mismatch with the `ERP` cites).
2. **Zero tests.** Nothing under codex/test references any ERP chapter.
3. **No entry point, no API, no UI.** No `opening`, no HTTP routes, no
   Page chapter. The suite is a library nobody calls.
4. **Persistence is theater.** `GlState.gl-db : DbServer` is
   constructed and never read or written. Logs accumulate in memory
   and evaporate.
5. **Integration is unproven.** Subledgers (AP/AR, payroll, COGS,
   allocations) each construct JournalEntries, but no test ever runs a
   flow end-to-end into the GL and checks that the books balance.
...
- **Browser UI**: the WebApp quire (apps/webapp — WebRuntime, WebTheme,
  ...
  docs/Designs/Apps/WebApp/Active/BaseTemplate.md.
...
### Phase 0 — Make it buildable and prove one engine (2 CLs)
...
### Phase 1 — Foundation hardening (4-6 CLs)
...
### Phase 2 — The integrated scenario: this is what makes it an app (3-4 CLs)
...
### Phase 3 — Persistence through Codex DB (2-3 CLs)
...
### Phase 5 — Browser dashboard (2 CLs)
