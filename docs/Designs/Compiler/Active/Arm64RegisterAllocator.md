...
**Date:** 2026-06-20
**Agent:** val
**Status:** Design justification. The next agent builds the solution.
...
| File | What |
|------|------|
| `codex/plugs/arm64/Arm64CodeGen.codex` | The allocator lives here. Key functions: `a64-alloc-local`, `a64-alloc-temp`, `a64-save-if-needed`, `a64-emit-binary-reg`, `a64-emit-direct-call`, `a64-emit-record`, `a64-emit-let`, `a64-emit-act-loop`. |
| `codex/plugs/arm64/Arm64Runtime.codex` | Runtime helpers. Already fixed. Stable. |
| `codex/plugs/arm64/Arm64Plug.codex` | Plug entry. Already fixed. Stable. |

---

## Current Workaround State (CL 5046)

| Workaround | Location | Status |
|------------|----------|--------|
| Direct-call local recycling | `a64-emit-direct-call` | Active, works |
| Closure-call local recycling | `a64-emit-closure-call` | Active, works |
| Binary-reg local recycling | `a64-emit-binary-reg` | Active, works |
| Record inline construction | `a64-emit-record` | Active, works |
| Tail-call save-args-all | `a64-emit-tail-call-general` | Active, works |
| Always-save prologue | `a64-emit-prologue` | Active (all 5 pairs) |
| Always-restore epilogue | `a64-emit-epilogue` | Active (all 5 pairs) |
| Retarget disabled | `a64-can-retarget-last` | Returns False |
| Trivial-arg skip | `a64-arg-is-trivial` | Returns False (disabled) |
| Stack guard | Prologue | CMP SP, X28 + B.HI/WFI |
| Spill load via alloc-temp | `a64-load-local` | Uses cycling temps |

These workarounds get simple programs working (boot test prints
"hello world") and the web server through PCI scan + device find.
The web server crashes in `arm64-pci-find-dev-loop` (5 params +
field accesses + comparisons) which is the next function that
exceeds the workarounds' capacity.
...
| Test | Status | Notes |
|------|--------|-------|
| `arm64-boot-test.codex` | PASS | Prints "Codex ARM64 boot OK / hello world" |
| `arm64-encoder.codex` | PASS | x86 battery test for encoding |
| `arm64-web-server.codex` | PARTIAL | Boots to "VirtIO init..." then crashes in find-dev-loop |

The boot test is the regression gate. If it breaks, the allocator
change is wrong.
