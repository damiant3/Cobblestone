...
**Status:** RETIRED (2026-04-22). The v1 watchdog landed on the REF side (CL 244 `--watchdog=progress|pet`), mirrored to self-host (CLs 245, 248), grew tiered escalation at 1s/5s/30s with a 4-slot ring buffer in self-host (CL 251), pets `wd-stale-tick-addr` inside idle-wait loops so `read-line` no longer false-positives (CL 251), and got the richer tier-3 panic dump — `HWM=<hex>` on both emitters plus a 4-qword stack walk from the most-recent ring slot on self-host (CL 253). Items 4 (runtime disable flag) and 5 (configurable threshold) from the follow-up list below were explicitly declined as not-critical; the `progress-tick` primitive remained a design-only artifact and was subsumed by the in-loop pet pattern documented in `memory/feedback_idle_loops_pet_watchdog.md`. Doc kept as historical record of the design conversation.
