...
**Two traps in the per-definition shape, both paid for (red, 2026-09-02).** A `PhaseStart` from `phase-start` is a bivy record, so a `__heap-restore` to a mark taken BEFORE it destroys `deck-origin` and the overflow test fires on garbage: take the mark after the phase-start. Every nullary definition (`no-overflow-bag` included) allocates per use on the bivy at the use site, so a bag built before a compact and merged after it must be `deck-record`ed at the use site; the focused proof did not catch that, `-PoisonCompact` did. **DESUGAR was measured and not taken:** its ~50 MB deck is all survivors (the keep grows ~54 MB at the boundary), so the only prize there is the ~46 MB double residence, at the price of making the desugarer's whole-chapter rewrite passes per-definition. Claim a host saving only from a head-against-candidate measurement, never intermediate-against-intermediate.

**DESUGAR re-measured by address (red, 2026-09-02, seed F134E3E7, `-Measure`), and the prize is real.** The union rule above decides it: DECK-3 DESUGAR sits at 302,075,254..351,979,838 (49.9 MB) and nothing after it touches that range (SCOPE deck 92..98 MB and bivy 201..236 MB, CHECK deck 199..227 MB and bivy 937..986 MB, LOWER deck 138..160 MB and bivy 826..840 MB), so those pages are paid once by DESUGAR alone, where LOWER's old deck sat on pages the frontend had already touched. The whole unique frontend set is LEX deck 40 MB and bivy 23 MB, PARSE deck 31 MB, PARSE-KEEP 14 MB and DESUGAR 50 MB, which is the "about 150 MB baseline" measured from the other side. The DESUGAR-KEEP trace agrees with the morning's run to within 0.4 per cent (deck 49,904,584 against 50,081,624; keep used 54,247,792 against 54,452,008; bivy 309,232). The choice that follows: do not make the desugarer per-definition; desugar straight into the frontend keep (set the keep deck before `desugar-document` and let the chapter pass through where `copy-as-chapter-guarded` would otherwise copy what already lies above the reservation base). That removes the 50 MB range and the 54 MB copy at once, with a 309 KB bivy the only scratch. Acceptance: host peak working set head against candidate, and the keep's growth at the boundary staying at about 54 MB.

**Stage 3 LANDED (red, 2026-09-02): DESUGAR desugars straight into the frontend keep.** The whole `desugar-document` runs inside one `deck-record` extent with the deck cursor at `fk-base`, so the desugared chapter is built where it will live; `relocate-chapter-tail` copies only the six parse pass-through fields. Measured at seed F134E3E7 on the selfhost: host peak working set 550.2 / 550.2 MB to 505.8 / 505.6 MB (two runs each), the DESUGAR deck now 50.57 MB at the keep's own address with a 24 B bivy, one-pass fixed point. **The trap it paid for, kept because the shape recurs:** the desugar compact now poisons the LEX deck, the PARSE deck and the PARSE keep, so every parse-side pointer the desugarer had shared by address became dangling, and the pointer-map walk over the AChapter reported zero escapes while five existed (L-FALSIF). The census that found them was by SHAPE against the parse copier (`copy-sx-expr`, `copy-sx-typeexpr`, `copy-sx-citesdecl` name every non-token heap field a parse node carries): the ListExpr and PropEqType spans, the BoundedIntType `OverflowMode` cell (a nullary constructor is a heap cell), the cite's `chapter-name` and `citing-chapter` texts, and `chapter-slug` per def. The signature of the failure is worth knowing: `OUT OF MEMORY` with R10 = a legitimate cursor plus 0xA3A3A3A3A3A3A3A3, an allocation whose SIZE was read from memory the desugar byte (163) had filled, caught by the stack check at the next function's entry rather than by any write. When a phase's output starts living in the keep, audit what its INPUT still owns, with the copier that used to protect it as the checklist.
