...
1. **A webserver app in the guios** (val, desk pane + wiring; blu consults
   ...
   fetch). **The parts are `codex/os/net/WebServer.codex` (`cites Net chapter
   WebServer`) and the proven TCP stack. This row said
   `apps/works/WebServer.codex` until 2026-08-28 and that is a DIFFERENT,
   same-named chapter** (val, after Damian said the good one was elsewhere):
   the Net one is a concurrent event loop with accept, per-connection demux,
   content-length reassembly and an idle sweep, pinned by four
   `codex/test/web-mux-*` tests and consumed by ten chapters including
   `Prism.codex:340`; the works one is a router and handler table with no
   socket whose only callers are its own test. The work is the desk pane
   (start/stop, live request log) and the codex:// service; the serve pump is
   already factored, since one `web-mux-loop` iteration is one desk-loop
   iteration. Bed first via codex-vm NAT port-forward; metal rides a
   future sitting. Originates in the works app; register
   `apps/works/works-backlog.md`, this row is the pointer.

   **BLOCKED ON A SCHEDULER, AND DAMIAN RULED THE ANSWER 2026-08-28.** The pane
   cannot serve without giving the DESKTOP network capability -- measured, 42
   functions carry the desk's effect row and none of them has ever carried
   `Network.*`, the Browser pane included, whose desk step is `[Device.Port]`
   and whose real fetch path the desk never calls. Damian: "the desktop is
   merely the admin app, the webserver is a system level concern"; then, on
   finding `codex/os/sched` cannot run a task at all, "we definitely need
   preemptive". So the webserver becomes the first system SERVICE and the pane
   becomes its admin console. Design and stage register:
   `docs/Designs/Active/OS/PreemptiveScheduler.md` (val, 2026-08-28). WORKS-48's
   pane half -- window slot, request-log ring in `ds` 244, state word in 248,
   start/stop -- is unblocked and the same in every road; the serving half is
   stage 5 of that design.
2. **The crazy-boss page: the compiler running in WASM, building itself,
   ...
   **CLOSED 2026-08-27 (fester, plugs 1.91, main 20013), and every part of
   ...
   CLOSED (fester, 2026-08-28), and it was worse than this row said.** The
   row had the exit-4 no-`SIZE:` path writing "neither stderr nor any of the
   output it did see"; measured against the source, it appends every output
   line that is not `WD:`-prefixed, and the reason 19756 got a 5-byte log is
   that in that run there was nothing else. The consequence is sharper than
   opacity: reproduced on demand with `CODEX_VM_DROP_SERIAL_AT`, the path
   exits 4 with no binary and a log holding one ordinary `info CDX4030` line,
   **byte-for-byte the shape of a SUCCESSFUL compile's log**, so the exit code
   is the only thing that disagrees with it. It now names the condition and
   appends what was being discarded: codex-vm's stderr, bounded to its last
   40 lines, and a 40-line tail of the raw output with `WD:`/`HEAP:`/`STACK:`
   left in. On the reproduction the stderr tail carries
   `SERIAL: 92695 guest serial byte(s) DROPPED ... is SHORT`, which is the
   whole diagnosis. Controls: a normal compile is unchanged at exit 0 and 83
   bytes, and a real `CDX2031` still takes the CODEGEN-ERRORS path; neither
   emits the new message. `exit 5` now prints the two sizes it compared
   rather than a bare "Binary size mismatch". Generator and script landed
   together at drift 0. **The riscv plug-source unit NO LONGER FAULTS** (plugs 1.94): it
...
device-written used-ring index, waits for a bed), 9 (DONE root 2026-08-28,
main 20464: every refusal site in the chain armed in
`build/agent-bundle-test.ps1` with wrong-link discrimination; account in
`VerifiedFormatParsing.md` 10.1 row 9), 10 (DONE reek 2026-08-20: `Modbus` is LATENT with no production
...
## The Prism dev environment (Damian, 2026-08-28; MULTI-LANE since the same evening)

**STATE AT 2026-08-28 WRAP (root commanding). This paragraph supersedes
every assignment sentence below it in this section; the paragraphs below
are the day's history, kept for the traps and the acceptance detail.**
LANDED today: stage 0 (20482), stage 1 (20503) + live-review polish
(20549), stage 2's COMPILER HALF (RESOLVE mode + seed `12171275`, 20736,
with Device.Block 20689, the library image 20720, LFN 20684), stage 4 to
test-limit (20580/20604/20610), the ENTIRE native-build configs feature
(20702 UI / 20712 bridge / 20723 write half -- Damian verified at the
browser: built and ran a C# app from the page), Steve PR 98 absorbed
(plugs 2.04, 20740), COMPILER-32 closed (blu; step 3 was gating at
wrap), WORKS-59 fixed (20636) with desk-root-guard in the BVT (20649),
and the PreemptiveScheduler design opened 20716 then CORRECTED 20727
(the kernel ALREADY has preemptive SMP processes, uncalled; the
webserver becomes a spawned process and the desk never gains Network).
OPEN, in order: (1) **stage-2 PAGE WIRING, and Damian's clarified
"source tree" vision rides with it (his words, 2026-08-28 wrap): all
the Codex source files in a nice TREE VIEW with full names, like a
TOOLBOX -- you can see the code, pull it into your project, and build
with it.** The substrate all landed today (LFN, the long-name quire
subdirs in the library image, RESOLVE off the volume); what remains is
the wiring plus the toolbox UI over the library image: browse
quires/chapters, view a chapter read-only, an add-to-project action
that pulls it in as a project file, and the build consuming it.
fester's fresh session (Damian launched it); wire + boundary in the
design's stage-2 convergence section; (2) **PR 98 CLOSED with credit**
(reek, 2026-08-28; absorb was main 20740, plugs 2.04, recorded in
GitHubUpdate53) -- DONE, nothing open; (3) **stage 2c, the in-tab
signer** -- red's next session (design updated 20708); (4) **bench (2f)
LANDED at main 20726** -- 12 of bench/codex's 13 programs are page
examples, vecstride left out for Math.VecArray plus PIT ticks, both
sides timed; NOT open, and it was listed here as unowned after it had
landed; (5) **boards**: riscv/arm64 wasm modules
built and byte-identical, ship=false on the wat2wasm IrIf else-spine
ceiling -- fester's IrIf flattening unblocks it (shelf 20730 proved the
match path innocent, 188 before and after); (6) **stage 5a hosted
arms** -- blu's queue after COMPILER-34; (7) **stage 3 templates +
build tab** -- queues behind the stage-2 wiring; (8) **PreemptiveScheduler
stages 1+2 per the corrected design** -- val's next session. Still
Damian's alone: the API key for stage 4's live round trip, and the
one-command public `compile/` page refresh.

**OPENED TO THE FLEET (Damian, 2026-08-28 evening, via root commanding): the
campaign wants velocity and red is resting, so lanes with room take Prism
side quests while the compiler-bug lane and the Steve catch-ups keep
rolling.** Assignments (each lane reads the design's stage before starting;
coordination through root while red rests): **val** takes stage 4 (the
Claude panel: wire, key handling, agent mode -- independent of the
resolver; CORRECTED 2026-08-28 evening, val caught that stage 3's templates
cannot compile before stage 2's cite resolution exists). **Stage 4 is SPLIT
and 4a is LANDED at main 20580**: the wire, key handling and the SSE reader,
with a headless arm over a canned stream. **The panel UI and the 4b tool
loop landed at main 20604**: the dock (off / chat / agent, key entry,
streamed bubbles, per-session write approval), six tools over the real
project, the manual tool_use/tool_result loop, and the service call behind
ONE injectable transport, which is the seam the arm drives everything else
through. **The stage's stated acceptance is
NOT met and the green arm is not it** -- "a chat round trip streams" needs a
real key and a real billed call, which no lane can supply from a test, and
the planted-error arm SHIMS the compiler, so what it proves is the loop and
the edit rather than that the compiler answers. What is left in stage 4 is
therefore a hand at a browser with a key, not more code. **The request shape is pinned in the
design at 20578 and must not be written from memory**: `budget_tokens` is a
400 on `claude-opus-5`, thinking `display` defaults to `omitted` so a
streaming panel shows a silent pause without asking for the summary,
assistant prefill is a 400, and a refusal arrives as an HTTP 200; **fester** takes
stage 2 (the resolver probe, A-vs-B with criteria written first; IN FLIGHT)
-- fester speaks for the wasm emitters, which is what stage 2 coordinates
with, and its first CL carries red's shelf 20550 (the stage-1 verification
arm); **stage 3 (templates + build tab) queues behind stage 2** for
whichever lane frees first; **reek** keeps the plugs chain (riscv landing,
zig redundant-arm row) and joins Prism after; **blu** stays on compiler
bugs (COMPILER-34 next), which
is itself the queue stage 5a's hosted arms join when it thins. Stage 5a
does not start before Damian's Linux-bed ruling; red resumes wherever red
likes on return, seniority on the campaign unchanged.

Prism becomes a full local HTML/WASM dev environment on the STATIC page:
source tree on disk, multi-file editor, worker compile, cite resolution
in-tab, kernel builds through the existing in-memory pe/img chain,
webpages through the html lens, user-mode `.exe` and Linux executables as
a NEW hosted-runtime backend (stage 5, seed-affecting), and an optional
Claude REPL/agent panel behind a provider interface. Design and stage
register: `apps/prism/design/Active/PrismDevEnvironment.md` (supersedes
`PrismDesign.md`'s served-compiler direction; the essay-repl join is
untouched). Register row: `apps/prism/prism-backlog.md` PRISM-7. Two
rulings the design asks of Damian are listed at its foot; stage order is
CONFIRMED (root, 2026-08-28) and the Linux bed ruling rides to Damian.
**Stages 0, 1, and the live-review polish are LANDED (main 20482, 20503,
20549; Damian's hand was on the polish).** The deployed landing copies at
`apps/landing/web/compile/` deliberately still carry the pre-workspace
page; refreshing the public surface is a one-command call for Damian.
NEXT: stage 2, cite resolution in-tab, opening with the design's
timeboxed A-vs-B resolver probe, criteria written before the probe; the
stage-1 verification arm rides red's shelf 20550 for stage 2's first CL.

**RE-AFFIRMED AND WIDENED by Damian 2026-08-28 (direct, root commanding):
the Prism buildout is the fleet's main campaign of the day; lanes stay on
it and its necessary dependencies.** The build features he named: output
formats, boards, bench, the foreword, the source tree, and long files for
proper quire mapping (fester's Fat16 LFN in flight is that last one's
filename half, Damian-ruled). Added the same day, verbatim intent:
**local transpiler-to-native build configs** -- where a javascript lens
says Run, a lens like c# says Config; the user picks their local compiler
target, saves it to config, runs the final native build, then runs the
output. **With it: a Configs button and an edit panel for the saved
configs**, so saved toolchain configs are managed in the UI rather than
write-once. A static page cannot exec a local toolchain, so the run half
needs a mechanism (an optional local bridge the page works without is the
obvious shape); the feature is dispatched to **reek** (the PRISM-6 lens
surface was reek's), config UI first, bridge probe second, deferring to
red's campaign seniority on return. Day's split where the paragraph above
disagrees: **fester** lands LFN then finishes stage 2; **val** fixes
WORKS-59 then stage 3 when stage 2 lands; **blu** compiler bugs until 5a
unblocks on Damian's Linux-bed ruling (put to him today, since "output
formats" names stage 5); boards and bench have no stage yet and are red's
to fold into the design.

**STAGE 2's COMPILER HALF IS DONE (red, 2026-08-28 evening, main
20736, seed `12171275`).** The `RESOLVE` mode landed: source on stdin,
cites resolved against the mounted volume, the resolved unit answered
as text between `RESOLVE-BEGIN`/`RESOLVE-END`, one `CITE-MISSING
<quire> <name>` line per absent chapter (fester's measured refinement),
sector-count-0 guard. The Device.Block grounding beneath it landed at
20689 (linear memory, the red+fester convergence), fester's library
image at 20720, LFN at 20684; fester proved the whole mechanism end to
end (cites off the volume, 89,882-byte CDX with the library, CDX3007
without). **What remains of stage 2 is the PAGE wiring** -- load the
library image, `disk_reserve`, run RESOLVE once, feed the resolved unit
to the existing compile paths -- which is fester's per the boundary
(their session cycled; the wire and the boundary are in
`PrismDevEnvironment.md`'s stage-2 convergence section). red moves to
stage 2c (the in-tab signer; verifier read done, design updated
20708).

**Three rulings landed 2026-08-28 evening (Damian, direct to root):**
(1) **The stage-5a Linux verification bed: ALL of the options** -- the
narrow WSL exception (verification arms only; R-SHELL amended in
CLAUDE.md), a QEMU Linux guest bed beside it, and 5b's Windows `.exe`
verified natively: *"do all, we are supporting all these options for the
people."* **Stage 5a is UNBLOCKED** and joins blu's queue after
COMPILER-34. (2) **boards = IoT board build targets**: build a Prism
project for the nine HAL board chapters, per-board output beside the
kernel chain. (3) **bench = run our codegen benchmarks and
comparison against any configured output build chains** -- it consumes
the native-build configs feature and queues behind it. WORKS-59 is
FIXED and on main at 20636 (val); the desk-root-guard runner is ruled
INTO the BVT (root, L-NOGATE). Issue 97 is answered on-channel and
registered as COMPILER-34.

**reek's order RE-RULED by Damian direct in-lane, 2026-08-28 (relayed by
reek, register corrected minutes later): (1) native plugs as wasm
modules on the page so people can build BOARD KERNELS in-tab -- **(1)
and (2) ARE DONE 2026-08-28**, and this consolidates the boards
feature's in-tab half under reek, not val; (2) the native-build configs
feature; (3) Steve's PR 98
(zig prelude tree-shake, stacks on landed 95+96) LAST** -- his words:
the zig work "is not the broadest brush right now, we are trying to
paint bigger". This supersedes root's earlier PR-98-first routing.

**LANDED 2026-08-28 (reek), so a reader does not re-do them.** (1) the
native plugs as wasm modules: riscv main 20681, arm64 20697, both wires
proven BYTE-IDENTICAL to bare metal against the serial path; both rows
are `ship = false` because wabt's JS `wat2wasm` cannot assemble
them, which is fester's `WasmEmitter.codex:1061` nesting site with an
agreed acceptance arm (`plugs-backlog.md` 2.03 -- read that before
touching either module). (2) the native-build configs: config UI 20702,
the local bridge 20712, the write half and a working c# preset 20723,
and **Damian confirmed it at the browser by building and running a C#
app from the page**. Stage 2f BENCH landed with it at 20726: twelve of
bench/codex's thirteen programs are page examples under a Bench
category, timed in-tab and over the bridge; `vecstride` is out
because it needs `Math chapter VecArray` and PIT ticks. **What is
left of reek's order is (3), PR 98, and nothing else.**
**val**, after the desk-root-guard BVT wiring, takes the guios
webserver app instead (`apps/works/works-backlog.md` origin; stage 3's
named server template) while stage 3 waits on stage 2.

## The lanes -- RULED by Damian 2026-08-15, re-pointed 2026-08-18

**RE-RULED by Damian 2026-08-28: root commands the fleet; work
assignments and status reports route to root. red is a solo lane on the
hardest problems, first the Prism dev-environment campaign above. The
2026-08-26 paragraph below stands for the other lanes where it does not
disagree. LATER THE SAME DAY: the Prism buildout is the fleet's main
campaign of the day; the RE-SCOPED block in the Prism section above
carries the lane split and supersedes the assignments below for the day.**
...
19845, 26 at 19889. **COMPILER-27 LANDED 2026-08-28 (blu 20488, copy-up main
20490, seed `8769F31E`), so this sentence has now gone stale a SECOND time in
the same way it documents above, and on the row it had just been corrected to
name.** That is the argument for the instruction rather than the list: **the
live order is whatever the backlog shows open, and the register is the only
thing that knows it** -- read it, not this sentence; **val**
makes the guios
...
- **A battery batch can hand every test in it ANOTHER test's output, whole:
  CONTAINED (red, 2026-08-28, red 20450).** The mechanism was the documented
  byte-loss class (`ExaminersAssay.md` "The batch stream can lose bytes",
  first measured 2026-08-16): attribution is positional, so a lost block
  shifts every later block onto the wrong name while the tail's exit 99s
  trigger re-batching and the shifted exit-0 members are kept.
  `test-compile-batch.ps1` now invalidates the WHOLE batch (every member
  exit 99, so phase 1a re-runs all of it) on a dropped-bytes report or a
  stream that ends short of its test count; `test.ps1` captures each batch
  child's stderr into `_batches/*.err` (the DROPPED report used to die in a
  hidden console), and `Get-FailHint` names a FAIL_OUTPUT whose actual is
  byte-for-byte another test's expected as misattribution instead of
  letting it read as codegen. WHICH layer loses the bytes is still unknown;
  the `.err` capture is the instrument that will answer it on the next
  occurrence. Record: `ExaminersAssay.md` same section,
  `docs/PM/SomethingSeenDuringRelease.md` Update 52 entry.
- **`deck-headroom.ps1 -Plugs` crashes on an empty corpus, and the gate
  dance's `p4 sync -f` can empty it. UNOWNED.** (red, 2026-08-28, found
  clearing a red `-Internal` on red 20450.) The plug corpus skips any
  bundle whose newest source mtime is newer than the bundle; a full
  `p4 sync -f` rewrites every source's mtime to now, so all bundles read
  stale, the corpus measures 0 units, and the script dies with "The
  property 'Count' cannot be found" -- the gate then fails for a reason
  that is neither a deck verdict nor the CL's doing. Two halves: make the
  empty corpus a clean named verdict instead of a crash, and decide what
  bundle staleness should mean across a forced sync (submit-time
  comparison against the bundle's build mtime is what recovered the truth
  by hand). Separately observed: 45 of 55 bundles in the red workspace
  genuinely predate the 08-27 plug-wide stdio change, so any lane's
  `-Plugs` run measures 10 units until bundles are rebuilt.

- **VM admission: the check exists at the fan-outs (fester, 2026-08-28).**
  `Get-VmAdmittedSlots` in `vm-config.ps1` answers how many concurrent guests
  the box can hold and is called where a phase fans out, never per guest: a
  per-guest wait deadlocks, because the slots already running hold the memory
  the waiter is waiting for. It ANSWERS A COUNT rather than refusing, so a
  contended box costs wall time instead of a wrong verdict. Wired into
  `deck-headroom` and the battery's phase 1 and phase 2.
  **The budget is measured, and the obvious number was wrong.** `-mem` is
  reserved address space, not footprint: a guest asking 3072 peaked at
  1,093 MB for a small compile and 1,098 MB for a 30-chapter batch slot, so
  budgeting the ask admitted ONE slot where four demonstrably run green and
  would have serialised every battery. 1100 MB per guest with 1024 MB host
  reserve also re-derives Damian's `-Jobs 4` ruling from the other end:
  8 x 1100 overcommits ~6.4 GB free, 4 x 1100 fits. Re-measure on a
  different box (L-COUNT).
  Arms: `deck-headroom -Jobs 4` untouched and OK at 1.33 over 59 units;
  `-Jobs 8` trims to 5, says why, and returns THE SAME verdict, which is the
  point -- the trim protects the answer rather than changing it.
  **On this row's evidence, and the correction is to my own first reading of
  it.** Seven of fester's gate runs died on 2026-08-28 and the causes are
  MIXED, not one thing. At least two are named by their author: kill 4 was an
  `-All` verification run of the scoped `clean-zombies`, and kill 6 a
  sequencing gap where a census, a Renode instance and a pre-token
  verification gate overlapped, the yield having been keyed to the TOKEN
  grant while the gates that needed it ran before the token. `clean-zombies`
  is independently capable of the rest: read at source it killed every
  `codex-vm` on the box, sparing only `-portfwd`.
  **I first wrote that these were sweeps and not contention, and the ablation
  I rested it on was confounded.** The sweep hold removed the sweeper AND the
  concurrent VM load in the same act, so the green gate that followed cannot
  tell them apart. I also argued that host RAM cannot kill the harness's own
  process tree, which was an assumption and not a measurement: on a box
  already 19.5 GB committed against 15.8 GB physical, commit exhaustion
  reaches pwsh as readily as it reaches a guest. Both mechanisms are live,
  they were removed together, and nothing here separates them. What IS
  established without ambiguity is main 20381, where `deck-headroom` at
  `-Jobs 8` read a contended run as "a plug bundle has grown into its deck
  reservation" and named a bundle that compiles clean alone, plus
  `OperatorsManual.md`'s `-Jobs 4` account. Those are the kill-wears-a-
  codegen-costume shape and they carry this item on their own.
  **Left open:** `test-cross-batch`'s run phase dies at 466 subjects while a
  filtered slice runs clean (reek, 2026-08-28), and a Renode instance is a
  VM load the check does not yet see (`OperatorsManual.md`, "A Renode
  instance is a VM load"). Both want the same helper at their own fan-out.
  The battery half is wired but UNPROVEN end to end: `build/test.ps1` refuses
  to run without Damian's approval, so phase 1 and phase 2 are verified by
  the helper's own arms, a clean parse and the refusal path, not by a battery
  run.
  **Corroborating kill census from red's lane (2026-08-28 afternoon),
  CORRECTED the same day, and the discriminating variable was the LAUNCH
  SHAPE, not the box:** a single-guest wasm-plug emission (one codex-vm,
  3 GB) was killed four times, (1) under real contention (five guests,
  1.4 GiB free), (2) inside a stale hand-granted window, (3) behind a
  launch-site admission check that PASSED at free=5.8 GiB guests=0, and
  (4) on a near-idle box -- and every killed run was BORN-BACKGROUND,
  while both foreground runs the same hours survived to honest failures
  (an internal VM timeout under a concurrent gate; a SHORT IR, 12.9M of
  the known 16.5M, under mutual load). The mechanism, caught by fester
  from the session list: fleet-wide SESSION RESTARTS tear down
  background shells, and the teardown reads as a load-independent kill
  at whatever point the run had reached. First attributions (occupancy;
  then clean-zombies) were both wrong for these kills and are recorded
  as the costumes they wore; clean-zombies remains author-named for
  fester's kill 4 only. Logs:
  `build-output/killed-vm-run-20260828-red.log` in red's workspace.
  Separately real from the same runs: the fleet status JSON showed red
  buildState=Idle throughout, so the admission helper cannot see a
  session-launched guest -- the Renode-shaped blind spot this row
  already names, measured from the other side. (This paragraph was
  clobbered twice by merge-down accept-theirs while it lived on a dev
  stream; it lands on MAIN now per the docs-straight-to-main ruling,
  which also removes that failure mode for register edits.)

- **Long filenames work end to end and a booted stick carries them. CLOSED
  2026-08-28, both halves on main. fester.** VFAT long names in `Fat16.codex`
  at main 20684 (read, write, match, list, replace, erase; runs assembled
  across sector and cluster boundaries; checksum verified), and
  `build-img.ps1` plus its generator at main 20720 write chapters under their
  real names into the quire subdirectory `quire-to-dir` resolves, with an
  opt-in `-Library` switch that puts the library quires on the ESP. The
  8.3-plus-`SRC/INDEX.TXT` mangling that made `X86_64CodeGen.codex`
  unfindable on a flown image is gone; `codex/test/img-longname` is the
  runner. Proven together: DISK mode resolved a cite off a built library
  image, an 89,882-byte CDX, against a no-library control answering
  `CDX3007`. **The choice this entry held for Damian -- teach the host writer
  or move the image into `Fat16.codex` -- is answered by 20720 and needs no
  ruling.**
  Note FAT32 does NOT come along for free: long names are the VFAT layer and
  `Fat32.codex` / `Fat32Writer.codex` still have none.
  Known limits, deliberate and pinned by `codex/test/fat16-longname`: a name
  past the format's 255 characters is refused, so is a character that does not
  survive the CCE round trip, and so is a run needing more slots than a
  cluster holds -- 255 characters fits the root, whose sectors are contiguous,
  and not a subdirectory at 512-byte clusters.
...
**One item is WITH STEVE HOWELL and blocks nothing: item 6. Item 7 was
ruled 2026-08-28 (Damian, via root commanding). The five from the
2026-08-27 afternoon walkthrough were ruled or dissolved in one sitting
(Damian with red); each ruling is recorded in the doc that owns it:**
...
4. **ir-fidelity RULED: RELEASE GATE, not `-Internal`** (the BVT-not-full
   ...
   afternoon (red, and fester independently in 20243 -- the two converged,
   sanity-checked clean at head across all 7 cases): the repoint was case-path data (`def:__lam_0/param/0`,
...
5. **Heavy-pane stranding RULED: option D, FIX THE ALLOCATOR.** Buried
   ...
   acceptance arm and its stranded row must drop. **Re-measured 2026-08-27
   at main 20359: that row is +6,480,104 and not the +5,114,480 recorded
   here, and it cannot be staged as written -- closing a window while
   another is open returns a BARE DESKTOP, because `desk-wnd-close-to`
   restores to the desk base mark and hands down `desk-apps-empty`, so
   every other pane's state died with the close (WORKS-57). **FIXED at main
   20387**: that path now goes through the mark stack like the plain close,
   and a pill restore re-enters the pane instead of painting an empty frame.
   Proven against a depot control -- a two-tab Browser survives a close over
   it, where the depot build returned a bare desktop. **THE ARM IS RE-TAKEN,
   2026-08-28 at main 20432 (val), and the allocator work is unblocked.**
   The buried close returns **+1,143,264**: it does not merely fail to
   reclaim, it ALLOCATES while reclaiming nothing, against a control that
   returns -3,500,520 exactly. One buried close strands 5,905,488 bytes.
   Neither +5,114,480 nor +6,480,104 was ever the number to beat (L-COUNT).
   **AND THE ACCEPTANCE CRITERION IS CORRECTED, which is the part that
   changes the work:** `__alloc` is a bump pointer, so reclaiming a buried
   mark cannot LOWER the frontier, and "the stranded row must drop" asks for
   something that will not happen even when option D is perfect -- it would
   report a correct fix as a failure (L-GAP). The row that CAN express hole
   reuse is a reopen after the buried close, today +2,465,912 of fresh
   frontier; under option D that falls toward zero while the stranded row
   stays put. Both are acceptance. (An earlier version of this row blamed
   Browser eviction on the pill raise; wrong, a pill click never reaches
   `-open`. Corrected 2026-08-27.) Table and account in ShellRefinement 6.4.
6. **Free-vs-solved wire marker: OPEN, WITH STEVE HOWELL (Damian,
   2026-08-28), and it BLOCKS NOTHING.** Damian's own read is ADD the
   marker with Steve's zero-sized-default rider, but he has put the
   question to Steve rather than rule it twice, and the final call rides
   Steve's answer -- a "RULED ADD" line stood here for under an hour on
   2026-08-28 and is withdrawn. **No lane implements until it comes
   back**; blu proceeds with COMPILER-27 and the backlog meanwhile. When
   ruled it is seed-affecting, token per CL, priced by the 20327 sweep at
   one lane-day; scope and evidence: COMPILER-32/33, the ir-fidelity
   census (23 of 596 clean programs carry `error` on the wire), issue 94.
7. **COMPILER-27 re-point RULED and LANDED (main 20490, seed `8769F31E`,
   blu 20488, 2026-08-28).** Re-pointed at the deck allocation path (arm at
   `__deck-enter`, disarm at `__deck-exit`); gate green one-pass, the guard
   proven to FIRE by sabotage rather than by green, cost +113,632 bytes
   static with runtime in noise, and item 1's measure-beside obligation
   met. A double-claimed low-memory cell fell out and was fixed (36328 was
   `net-driver-poll-cell`'s; the guard's moved to 36336). Rows closed in
   `codex/compiler/compiler-backlog.md`; this line and item 1 are the
   ruling record only.
...
was ruled DELETED the same afternoon (four dirs found, not the recorded
three); the witness re-run live at head answered OK, both hosts agreeing on
FC5B3D2C0911F075.
...
21. **plugs 1.57: the Rulebook's over-application rule binds every plug that
   ...
   verification the 1.57 row asks for. **CORRECTED 2026-08-28 (reek's
   measurement, root's call): the riscv wiring is INERT** -- the named branch
   wired in gives a byte-identical wire and identical Renode output, while
   the miscompile itself DOES reproduce (1.57 said it did not), so the real
   site is elsewhere and unnamed. Do not re-implement the wiring; reek hunts
   the real site from the reproducer, and the java half stands as ruled.
   Account: `plugs-backlog` 1.57 (reek 20473 also taught the plug runners to
   refuse a FAULT).

### Ruled by Damian, kept as one line while the work is in flight

- **Widen the sem-equiv trigger. DONE (fester, 2026-08-28)**, the hold having
  expired with Update 52. `$tSemantic` is `$tFrontEnd` plus
  `codex/compiler/opening.codex`, and it drives `text-stage1` and `sem-equiv`
  only; `text-stage2` and `text-fixedpoint` stay on `$tFrontEnd`, which is the
  shape ruled below and is forced by sem-equiv consuming the `stage1.codex`
  that only text-stage1 produces. Proven in the real gate, not from a table:
  with `opening.codex` opened, the internal gate printed `plus: ... text-stage1,
  sem-equiv` and `deferred: run-list, text-fixedpoint, text-stage2`. A
  decision table over the shipped trigger lines covers the other arms:
  `CodexEmitter` still fires all four, and build files, docs and other
  compiler chapters still fire none.
  **What it costs, re-measured on the green run rather than carried
  (L-COUNT):** text-stage1 28.6 s, sem-equiv 48.8 s, so the pair is 77.4 s,
  against the ~83 s this row projected from 2026-08-25's 34.4 + 46.2. A
  compiler CL that touches `opening.codex` and nothing else in the front-end
  set pays that and no more; text-stage2 and text-fixedpoint stayed deferred
  in the same run. sem-equiv passed at head with `opening.codex` implicated,
  which is the arm that matters: the widening is not merely wired, it is
  green on the corpus it now guards.
  **Two residues, neither closed by this CL.** Other compiler chapters STILL
  cannot trigger sem-equiv: the ruling picked the cheapest useful widening,
  not the whole class, so `$tCompiler` remains outside it and the next
  instance one chapter over will again be found by a full gate. And the
  mid-body-prose comparator class named below is now reachable more often;
  head is unaffected (a full gate runs sem-equiv green today, so widening
  cannot turn head red), and what it newly exposes is a CL adding prose
  inside a function body. That is R-PROSE's own rule getting a runner where
  the prose is unjustified, and a real cost where the prose is one of
  R-PROSE's three permitted kinds sitting inside a body.
  Original entry, kept because the reasoning is the ruling:
...
| `apps/works/AgentBundle.codex` and `codex/test/apps/agent-bundle-*` | **FREE -- Track D item 9 CLOSED (root, 2026-08-28, main 20464).** Every refusal site in the chain has an arm in `build/agent-bundle-test.ps1`, each bundle honest at every link but its own; no guest code moved. Account: `VerifiedFormatParsing.md` 10.1 row 9 |
...
| `apps/works/GopWeb.codex` (new, the webserver pane) and `ds` cells 244 and 248 | **val, 2026-08-28, WORKS-48.** 244 is a POINTER to the request-log ring, 248 an integer state word (serving flag plus port); both are allocated and written in `desk-run` before the base mark, so no pane close reaches them. The `WebMux` is NOT in a cell -- it is a typed record, so it goes in `DeskApps` beside the other four and is carried through `desk-wnd-close-to` the way `da-browser` already is, with its memory built before the base mark so no restore frees it. `codex/os/net/WebServer.codex` is READ, not changed: the pump is its existing `web-mux-accept`/`-feed`/`-sweep`. 252 stays free |
| `codex/os/kernel/E1000e.codex`, `codex/os/net/**` | blu |
| `codex/os/sched/**` and the preemptive scheduler work | **val, 2026-08-28, `docs/Designs/Active/OS/PreemptiveScheduler.md`.** Damian ruled preemptive after WORKS-48 showed the desk would otherwise need network capability. Was unclaimed. blu keeps `codex/os/net/**`; the scheduler reads that side and does not change it |
...
| `codex/plugs/**` and `codex/plugs/plugs-backlog.md` | **reek, 2026-08-18, the close-out lane** (handed on from val, whose 2026-08-16 claim this row carried until 2026-08-19). The register is the lane. Includes `codex/plugs/zig/**` (ordinary fleet code, Damian 2026-08-18); excludes the entries other lanes hold (named in the lanes table). **`codex/plugs/wasm/**` was split OUT to fester 2026-08-24, Damian-directed, and RETURNED to this row 2026-08-27: the condition the split named was fester landing the fix, and he landed it at main 20013 (register 1.91, the act-tailed self call that made the browser self-compile die 4,805 definitions deep). The wasm plug's emitters and its OOM behaviour stay fester's to speak for.** |
...
**THE COMPILER'S MEMORY CONTRACT (Damian, 2026-08-28, correcting a
mis-recorded cap that stood here for minutes): the FULL SELF-COMPILE must
complete within a 2 GB heap high-water mark, in BOTH text and CDX modes.**
This is a contract on the compiler, not a scheduling policy: a self-compile
needing more is a DEFECT to find and fix, never a reason to grow the
guests. Consequences: compile guests size toward ~2 GB once the compiler
complies, and the phase fits this box at width again; the gate's
self-compile phases, which already report `heap hwm`, gain the <= 2 GB
assertion as the contract's runner (fester, at the fan-outs, with the
admission work). **INVESTIGATION CLOSED (root, 2026-08-28 evening,
Damian-directed): there is NO runaway quadratic and NO compiler memory
regression.** Measured: the CDX-mode self-compile peaks 1,092 MB on the
U52 seed and 1,094 MB at head -- flat across the whole release interval on
identical input, under the contract with room. The "12 GB" was request
arithmetic (4 x `-mem 3072` ceilings; blu's provenance), and compile-BATCH
guests scale LINEARLY with batch size (234 MB at 60 chapters, 2,134 at a
416-chapter slot; fester 20571) -- an appetite that always existed, not one
that grew. What changed since the release was VISIBILITY and BUDGETING:
red's 20450 containment converts any batch byte-loss into a loud
whole-batch red where before it silently misattributed, and admission
budgeted a 30-chapter sample's footprint against 400-chapter slots (fixed
at 20571 with the measured curve). The kill cluster decomposes into those
two plus orphan cascades (closed by the scoped sweep, 20511), one -All
mid-hold (root's, confessed), and a hot six-lane day. Text-mode hwm
remains to be read from the GATE's fast path by fester's assertion runner:
a standalone `-Text` compile measures the serial channel, not the compiler
(40+ min against the gate's 34 s for identical work).

**BATCH YOUR GATES (Damian, 2026-08-28, via root commanding).** Small CLs
land on your DEV stream with targeted tests only -- compile and run what the
change touches; `-Internal` runs once per work ARC or window, never per
one-line CL, and the batch copies up grouped (P-COPY1). This extends the
2026-08-06 many-CL-arc ruling from seed arcs to all work. Seed lands are
unchanged: token and gate at the land. Docs and registers need no gate, as
ever. Until the VM admission check lands, gate windows are coordinated
through the commander so heavyweight builds do not stack on the box.
...
em-dash stays banned. `-Jobs 4` on every parallel harness (Damian
2026-08-27, RAM-bounded, supersedes the 08-02 eight; ExaminersAssay "The
parallelism default"). Do not lower
...
**Do not add a test to the gate or the battery on your own initiative. If you
think one belongs, get red's clearance first.** Damian, 2026-08-21:
*"haphazardly adding tests to the gates slows everyone down. if you really
think it belongs, talk to red and get his clearance."* The cost is not the
test's and not the proposer's, it is every agent's gate run for the rest of
the project, which is why the clearance is the commander's to give rather
than yours to assume. **The rule is about the GATE and the BATTERY and is not
a ban on arms:** measured 2026-08-21, neither `build/build.ps1` nor
`build/test.ps1` references `diag-arm` or `build-diag`, so a
`build/boot/diag-arm.ps1` row is pre-flight rehearsal and costs a gate run
nothing. Check which of the two you are adding to before assuming you need
the clearance, and before assuming you do not.
...
is published, and a bug report or a critique about an external project is
that author's to receive before it is anyone else's to read. It does not go
in the depot at any path while it is unpublished. **Nor does a note saying
one was withheld:** a marker naming what was removed, or where it went, or
that a policy exists, publishes the existence of the critique and advertises
that there is more, which defeats the purpose twice. In a design that has to
explain what our code does, state the target's behaviour as a fact about the
machine we build on rather than as a defect in somebody's document: "TMOD is
the truncating remainder, measured" is fine and "the reference does not
distinguish them" is theirs to be embarrassed by. Watch the framings that
read as neutral and are not -- their own compiler emits, the specification is
insufficient, N of their M examples fail. Perforce CL descriptions are not
mirrored, so they are internal; only files are pushed.
