...
- **A battery batch can hand every test in it ANOTHER test's output, whole.
  UNOWNED.** Found at the Update 52 release step 1 (red, 2026-08-27): 26
  `FAIL_OUTPUT` reds confined to two of eight batches, actuals byte-for-byte
  SWAPPED with other tests' expecteds, under "re-batching 153 tests from
  death-batches" on a hot box; the clean re-run had no re-batching and
  cleared all 26 with the identical compiler, convicting the instrument.
  The suspect is the splitter that turns a batch's serial stream into
  per-test outputs; mechanism NOT established. A misattributed output can
  also SILENTLY PASS when the neighbour's output matches, so this is not
  only a false-red generator -- the battery's verdicts are untrustworthy
  under re-batching until this is fixed. Evidence pointers and the one-line
  swap re-measure: `docs/PM/SomethingSeenDuringRelease.md`, the Update 52
  open entry.
...
**Nothing open as of 2026-08-27 afternoon: all five were ruled or dissolved
