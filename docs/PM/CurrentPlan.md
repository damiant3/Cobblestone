...
|---|---|---|---|
| **blu** | **THE RECEIVE PATH COSTS 120 BYTES A FRAME, DOWN FROM 208 (blu, 2026-09-08, `codex/test/net-recv-heap`, seed `75B414046BEE5208`).** Three landings: the 208 was ATTRIBUTED with no residual (main 24210), the doubled `TcpTransportState` in `transport-process-frame` removed, 208 to 144 (main 24214), the same doubled build removed at `NetIO`s and `Arm64NetIO`s drain paths (main 24218), and the vestigial `recv-buf` field removed from the record, 144 to 120, the record itself 64 bytes to 40 (main 24222). The three surviving stages cost 40 each: `net-process-frame`, `transport-feed-from`, `transport-try-recv`. Every web-mux figure fell by the same 24 independently: a connection is **66,322** bytes, a pooled re-accept **768**. **A CAP ON THE WEB-MUX POOL IS REFUSED, measured:** `drop-delta` moves the heap +144 and never back, so a cap returns nothing and forces a 66,304-byte re-mint per accept it turns away. **The spawned-child `write-file` fault is narrowed to the WRITE path alone** (main 24202): the cap travels, and the child s READ servicer completes, so the lookup, the elevation and the servicer call all work in a spawned process. **NOW: nothing taken.** **OPEN, none blocking:** the `Arm64CodeGen3` host-field-index tables encode `TcpTransportState`s layout by hand and NO arm observes them (a sabotaged index passed every available test), so any future field reorder is a silent arm64 miscompile; `net-io-send-drain` and its arm64 twin cannot be measured end to end without a card; the frame half of the receive-loop repair is still the 14-signature campaign `ProtocolStack.md` describes. | Metal-gated and not startable here: the asde per-step notes (main 22830) stay unproven until an off stage is reliably off; B4 step 6 needs b3; Track Bs NIC-4 successor, NIC-5 and the b3 TCP conversation all advance at sittings. WORKS-16 wants a metal capture, not another headless arm. | `codex/os/kernel/E1000e.codex`, `codex/os/net/**`; WORKS-16, WORKS-62 |
| **val** | **NOW: nothing in flight. Lane clean, level with main, no guest, no token, no shelf.** SHEET-10 step 1 landed (main 24184): `SheetPane` holds the sheet, the focused cell and the edit buffer, and `gsh-tree` renders from one instead of building `gsh-demo` itself, so an edit can survive a repaint. Sabotaged back to `gsh-demo`, `desk-sheet-pane`'s new middle line falls from 1250 to 750 and no other line moves. Sheets is otherwise as it was: a working spreadsheet with a photographed read-only desk pane (SHEET-1/2/3/4/6/7/8/9/11/12). | **NEXT: SHEET-10 step 2**, the `da-sheet : Maybe SheetPane` field on `DeskApps` and its 14 construction sites (13 in `GopDesk`, 1 in `desk-menu-dismiss`), with the existing desk arms as the byte-identical control. Until it lands the desk builds a fresh pane every repaint, so nothing an edit did would survive one. Steps 3 to 5 (the step function, the hit test, the keystrokes) follow, each provable alone; contracts are in `apps/sheets/sheets-backlog.md`. WORKS-60, the 3D pane's bad render target, still needs Damian's own desk hands; ShellRefinement stage 4's menu docking meets it at the same seam. | Damian's batch: WORKS-60, the virtual-desktop wording, WORKS-50's naming, FW-1's three fix options. A start-menu group of seven pushes the laid menu past the taskbar band and nothing clips or scrolls: registered in `ShellRefinement.md`, and it is why the Sheets launcher row sits in Accessories. `ShellRefinement.md` "WHAT IS STILL OPEN IN 6.4"; `PreemptiveScheduler.md` stage 2; WORKS-47/41/44/46/40 |
| **fester** | **NOW: the `diag-arm.ps1` four-arm failure is NOT REPRODUCED at head.** The whole reading is `DiagnosticStick.md`, "What is still open", first bullet. Three full 50-arm runs answer every arm: shipped `6F077EEB` under the current harness, rebuild `37D15C87` on seed `05E254BF`, and that rebuild under a 4-guest BVT. Harness, payload and seed are each refuted by measurement: the twice-failing image carried payload `f2e07084211bb472`, which is the green rebuild's, and kernels `D9CF2404` and `05E254BF` both produce that one payload. LOAD is half measured, `b3-pass` and `nic-kills-msc` green under sustained 4-guest load, `b3-record` and `b3-clockstuck` unread because no safe load instrument exists on a box one DIMM down. **Also in this CL: the staleness-guard repair**, `diag-arm.ps1` keyed on the payload's bundle closure (`bundled-sha256`, recorded by `build-diag.ps1` in `DIAG.RCP`) instead of a timestamp glob over `build/boot/diag` that cannot see the other 87 bundled chapters move. | **LANDED ON MAIN TODAY, all proven, none shelved:** 23830 the app-sweep subject selector (a real blind spot: 3 apps chapters swept nothing, 25 swept fewer); 23877 DATA-W3 (17 chapters nothing had ever compiled, SEVEN red); 24045 STARMAP-8 and the nettool suite (both moved to `codex/test/apps` where `test.ps1` RUNS them, 20 and 35 arms, five red on their first run) plus Phase B stage 1 and COMPILER-68's BVT batch arm; 24054 `sweep-apps.ps1` `-Jobs`; 24114 the seed, `7A36B22409B9B1D8` (DISK mode honours its path). **Registered and unowned:** 43 `*-verify.mjs` graders no script invokes; plugs 1.101 (nine plug run drivers with no `-Kernel`) and 1.102 (the img plug writes one hardcoded 8.3 source name, so no image can hold `codex/test`); STARMAP-10; WORKS-63; the flaky step 2b in `test-disk-compile.ps1`; a load generator with a BOUNDED MEMORY ENVELOPE (the BVT is not one: `-Jobs 4` lasts 28.4s against a 50-minute rehearsal, and looping it takes free RAM to 733 MiB, where a starved guest and a slow one are the same colour). **Phase B stage 2 is blocked on 1.102 and on root's ruling (a), the thin `opening` over a citable compiler, which is seed-affecting and not started.** | **COMPILER-59 and COMPILER-60 are PARKED and measured, not abandoned:** `ir-fidelity -Disagree` counts a def disagreeing with itself and reads 3 sites over 2 programs of 615; the lambda-parameter-span change moves 2 of the 3 and REGRESSES `typeclass-poly`'s `convert`, and the shared-span reading is refuted, both digests in `docs/Designs/Active/Compiler/LambdaParamSpans.md`. The name-the-kernel class is closed: `test-self-verify`, `check-generated-scripts` and `test-cross` all take `-Kernel` (2026-09-07). What the Build register still carries unowned is that `test-cross` BOOTS RENODE whenever the binary exists, guarded only by the operator remembering. A8 desk build loop when VT-x metal is available; Renode out; `deck-headroom`; WORKS-24 rides a sitting |
| **reek** | **NOW: nothing taken; clean, no red gate.** The PipelineModel migration is Damian's direct direction of 2026-09-08 and is the lane: he approved the record shape, then directed the migration through the remaining generators. **TWENTY-SIX of the 58 generators are on the model and on MAIN, every one `match / 0 drift`, and NO shipped `build/*.ps1` byte changed by any of it** -- say that in every copy-up, it is what lets a release owner skip re-running them. Latest: `check-cdx-registry match 213 / 0 drift`, reek 24255, main 24257. Census: 26 migrated, 22 flat, 10 blocked by burr 4; of the 22 flat TWO can never be graded, so 20 are takeable. **THE ONE THING WAITING ON DAMIAN, a ruling not a review: the burr-4 shape.** `pl-stages` is flat, so a script whose NUMBERED SECTIONS sit inside control flow cannot keep them; the blocked ten need a group node carrying THREE things, a repeat condition (2 foreach, 2 while), a guard condition (1 if) and a handler stage (5 try/finally). **Of the four scripts whose stage structure is worth the most, only `CompileScript` is blocked**; `build.ps1`, `test.ps1` and `bvt` were carried on this row and on the design as blocked, and all three are FLAT (measured at head 2026-09-08: their assemblies are plain `[ScSequence gNN, ScBlank, ...]` lists and every numbered section, 10 and 6 and 12, is referenced straight from it). All three are takeable without the ruling. A second ruling was offered with it: the three vocabulary gaps, each hit twice (a COMPUTED exit code, a stage whose output is a FUNCTION, a resource driven by a PARAMETER), recommended as one `PoComputed`, one `PkFunction` and widening the three Integer resource constructors. Both are on `docs/Designs/Active/Build/PipelineModel.md`. | **NEXT for whoever takes this: the next flat generator, one per CL**, taken from the census table on `PipelineModel.md` rather than by grepping, which has been wrong three times (L-CENSUS). Recipe: `p4 edit codex/build/<name>Script.codex`, migrate (cite Shell chapter Pipeline, declare artifacts, `sN` becomes `sN-body` plus `sN : PipeStage`, assembly becomes a `Pipeline`), then `build/check-generated-scripts.ps1 -Only <emitted-name>`, ONE guest, which must read `match / 0 drift`. Each stage body takes the leading `ScBlank` the old flat assembly held between sections, or the emitted script comes out short by one blank line per boundary. **MERGE DOWN BEFORE EVERY `-Only` RUN**: a proof under a superseded seed is void, and `plug-build` had to be re-run when the depot seed moved twice under it. **AND STASH ANY OPEN EDIT BEFORE A MERGE-DOWN**: `p4 submit` on the default changelist sweeps it into the merge CL, which put an unverified generator on `//Codex/reek` at 24034 (P-OPENMERGE). | Plugs close-out lane; WORKS-9 metal-gated at red's sitting; `ShellDslReadability.md`, which is the SAME campaign as the pipeline model (a raw node hides a name AND a refusal from every reader but the one holding the text); `tools/codex-vm.c`. gpushow live-compile is ON MAIN (23608, 23609, on Damian's ruling relayed by root); it is not published to the public site, and publishing is his call. Blocked on Damian: SPARK-4. Registered, unchased: COMPILER-46, COMPILER-39, COMPILER-48. |
| **red** | **NOW: nothing taken; awaiting dispatch (tree clean, no red gate).** Landed 2026-09-08 evening: COMPILER-69 (main 24205, CDX2097 warns on a `let` bound to a partial application that nothing reads; fires on `desk-cursor-arrow`#2 u1+u2, silent on the fix and on the compiler's own source; BVT 143/0, fixed point green) and COMPILER-70 (main 24230, sine and cosine folded to an octant, `real-cos 0.0` exactly 1.0, worst error 5.63e-8 to 9.77e-15 against python, `tr-tol` 1e-7 to 1e-12 which the old chapter fails on 18 cells; NOT seed-affecting, the compiler unit is byte-identical either way). Registered COMPILER-71 (`aexpr-mentions-name` answers False for a name used in 11 of the 20 AExpr variants). Seed on main: `75B414046BEE5208` (#774). | **NEXT: awaiting root's dispatch.** The kbd and mscalign step-2 ladder lifts stay HELD by root until the last hardware sitting flies. Then the compiler register in its order. | Releases, personally and end to end; `apps/works/GopBoot.codex`, `GopWizard.codex`, `apps/guios/**`; the 4.3 seed hash check runs BEFORE build-complete. |
| **root** | **NOW (handed off 2026-09-08 14:00 at 70 measured; resume by `/commander-init`, pulse 600 s while Damian is present):** commander. UPDATE 56 PUBLISHED by blu at 12:33, commit `6cd2ca1b` on GitHub master and GitLab main, seed `D9CF240465C3D0BC`; account `GitHubUpdate56.md`. MAIN OPEN. Head seed `75B414046BEE5208` (#774, 24205, COMPILER-69), a 2-member batch proven on it by root. No red gate known. **In flight at handoff:** fester's load control (its rebuilt diag image's 50 arms beside a BVT fan-out; the defect did not reproduce on a quiet box, payload identical to blu's failing image; then the staleness guard in `diag-arm.ps1` ~364-382 keyed on the bundle closure); blu's vestigial recv-buf removal (receive loop 208 to 144 B/frame landed 24214/24218); val's SHEET-10 step 3 shelved 24216 pending an on-glass capture (a serial pointer probe `desk-pointer-bed` diagnoses a click that opens no menu against three kernels); red's COMPILER-70 (the cosine, issue 122). reek handed off at 70.4 (24189), 25 of 58 generators on the pipeline model, awaiting relaunch. **Lanes at handoff, measured 13:53:** blu 34, fester 16, red 23, reek 71.5, val 35. **Steve:** receipts posted on all eleven ingested PRs (release skill Step 8, 24160); issues reconciled 13:20 (109, 113, 102 closed as shipped; 110, 115, 120, 125, 126 replied with their rows; 122 registered as COMPILER-70, 24166; 94 open by design); mail answered. **Damian's decisions:** the release-cadence remedy (a nightly unattended full gate, or the L-NOGATE cite-graph lane runner); PipelineModel burr 4 (repeat, guard and handler, three shapes priced on the design page). | The pipeline model campaign is reek's (`PipelineModel.md`, 23627). fester's stocked queue after the flight starts at `pair-generic` (23364). Inventory 2026-09-08: about 125 startable units outside the four main registers, but seven register lines proved stale at head in one night, so every dispatch is verified at head first. | `DiagnosticStick.md` composition; `ComplianceEvidence.md`; `HardwareAbstractionLayer.md` question 5 blocked on a board crypto manual; OracleCloudArm64 deferred; `build/boot/diag/**` (released to red for the two stage lifts) |
