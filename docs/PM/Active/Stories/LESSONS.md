...
| L-UNCALLED | A path nothing calls is a path nothing has tested. Grep for callers before trusting that a capability exists. | TheKeyboardWasNeverSilent, ProseHasNoRunner | partial: `deck-headroom.ps1` refuses a phase-table constant nothing in `codex/compiler` reads |
...
| L-NOGATE | A test that is in no lane's gate goes red at head and stays red until a release finds it, and a static instrument cannot observe a runtime defect. Ask not "is it green" but "could this instrument observe the failure I am risking". | GitHubUpdate49; val 2026-08-27 `codex/test/desk-parse`; blu 2026-08-28 COMPILER-32 | `build/cite-gate.ps1`, which walks the cite graph BACKWARDS from the changed files and RUNS every `codex/test` chapter it reaches, reporting the runnable count apart from the compile-only one. It does not close the lesson: a test reaching a change through no cite is not selected, and the compiler is assembled by glob |
...
| L-EXITCODE | **`$LASTEXITCODE` after invoking a `.ps1` DIRECTLY is the last EXTERNAL command run inside that script, not the script's verdict.** A script that ends by calling another tool and then printing its own success line reads as a FAILURE to a lane that called it in-session, and as a success to `pwsh -File`, which is what a real caller uses. Measured 2026-09-09 (red): a two-line probe ending in `Write-Host` after an external `exit 1` answers 0 through `pwsh -File` and 1 through a direct call. Read a script's verdict from what it PRINTS, or invoke it as a child process; and before reporting a tool as broken on an exit code, reproduce the code the way its real callers get it. | red 2026-09-09, a false "install-seed.ps1 exits 1 on success" finding published and retracted the same hour; `build/install-seed.ps1` has no defect | none (candidate: a review question, "was this exit code observed the way the tool's real callers observe it?") |
| L-UNRESOLVED | **A type inspected BEFORE resolution matches no arm, and the fallback answers something plausible instead of refusing.** The shape has now bitten twice in this compiler, both times on a recorded type that was still a variable at the moment a `when` looked at it. Ask of any `when` over a type: is this resolved here, and what does the `otherwise` arm CLAIM when it is not? | `lower-lambda` recording the expected rather than the resolved type (IndependentRechecker.md, red 2026-08-10); `infer-comparison` typing a vector comparison in application position as Boolean, so `if two 5.0 == two 5.0 then 1 else 0` compiled, ran and answered 0 for two identical vectors (red 2026-09-09, main 25061) | partial: `codex/test/errors/vec-repr-mix` and the `deep-resolve` in `infer-comparison` cover the vector case; nothing grades the general shape |
| L-CLEANMERGE | **A merge that resolves cleanly can still be wrong: two lanes adding the SAME record merge into two copies, and no conflict is raised because neither edit touched the other's lines.** Run the checker that owns the file after every merge that touched it, and before the copy-up, not only after your own edit. | red 2026-09-23: red cataloged `apps/prism/wasm-equiv.ps1` while fester cataloged it in main 26556; red's merge-down auto-resolved both entries in, and copy-up 26559 put a Duplicate ID on main, so `build/checks/tool-catalog.ps1` exited 1 at head until main 26561. The check had been run, green, on red's edit BEFORE the merge. | `build/checks/tool-catalog.ps1` catches this file's case when it is run after the merge; nothing runs it there |
| L-BEFORE | A `before` control must predate the whole CAMPAIGN under suspicion, not its latest unit. A seed taken just before the last landing of a multi-landing change still carries the earlier landings, so it clears the change of an effect the change caused. Bisect over the campaign's whole window when the first control disagrees with the expectation. | blu 2026-09-23: `sheet-chain` cleared of COMPILER-86 against seed main 26362 and reported so to root; a seed bisect then named 26048, COMPILER-86's own stage 1 | none (candidate: a review question, "does the control predate the FIRST landing of the thing it clears?") |
## How to use it
