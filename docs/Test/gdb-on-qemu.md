...
1. **`tools/bs3-qemu-trace.sh [sample]`** — runs stage1.elf under QEMU
   **TCG** (no KVM) with `-d in_asm -D /tmp/bs3-qemu-trace.log`. Captures
   every translated block. Use this to find out which addresses are
   actually executed.
2. **`tools/bs3-gdb-probe.sh <hex-address> [sample]`** — runs stage1.elf
   under QEMU **KVM** with gdbstub, sets a hardware breakpoint at your
   address, reports register values when hit. Fast.
...
```bash
wsl bash tools/bs3-qemu-trace.sh
grep -c '^0x001e0534:' /tmp/bs3-qemu-trace.log
```
...
```bash
wsl bash tools/bs3-gdb-probe.sh 0x1e0534
```
...
| `-serial stdio` | kernel's `BINARY\n<src>\x04` input, text output |
...
Always trace first, probe second. Cross-checking between
`bs3-qemu-trace.sh` (what runs) and `bs3-gdb-probe.sh` (what state) is
the fastest path to ground truth.

## References

- `tools/bs3-gdb-probe.sh` — address + sample → register dump
- `tools/bs3-qemu-trace.sh` — sample → execution trace
