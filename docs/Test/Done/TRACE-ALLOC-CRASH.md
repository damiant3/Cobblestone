...
`trace-max-entries = 30000` in `codex/Emit/X86_64Boot.codex`; the buggy
`0x108D40` value appears nowhere. Seed `seed/Codex.cdx` + `Codex.map`
rebuilt 2026-05-29 04:30 (after the fix), so the corrected immediates are
baked in. "Seed rebuild pending" below is satisfied. Do not re-investigate.

---

## Status (original): ROOT CAUSE FOUND (2026-05-25)
