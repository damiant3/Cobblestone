...
## Resolution (2026-05-29, reek)

**The bottleneck was the host↔guest transport, not the guest's text emit.**

The 2026-04-30 investigation measured text serial throughput at ~1.1K B/s
(180x slower than binary) over **dual TCP serial sockets under QEMU/KVM/WHPX**.
Every byte written by `emit-print-text-loop` had to clear a real UART
THR-empty poll whose timing was governed by the emulated serial port and a
TCP round-trip. That is what made text emit hang for hundreds of seconds.

The build harness now drives codex-vm with `-input <file> -output <file>`
(`build/compile.ps1`, `build.ps1::Invoke-BuildText`): input is preloaded into
guest memory at the serial ring buffer (0x500000) before boot, and output is
captured from guest UART writes into a file. **No TCP sockets, no serial-port
baud timing.** codex-vm signals THR-empty immediately, so the per-byte poll in
`emit-serial-wait-thr` (X86_64IO.codex) never stalls.

### Empirical proof (2026-05-29)

Full compiler source (`build/output/Codex.codex`, 1,229,160 B) compiled
through the canonical signed seed (`seed/Codex.cdx`) directly via codex-vm,
2048 MB, headless:

| Mode | Elapsed | Output bytes |
|------|---------|--------------|
| TEXT | 10.4s   | 1,167,681    |
| CDX  | 20.3s   | 2,438,913    |

Text emit is ~2x **faster** than CDX (it skips codegen + binary assembly), not
180x slower. Output is complete (matches today's gate `stage1.codex` of
1,166,798 B once WD/HEAP/STACK status lines are filtered).

The **text fixed-point gate currently passes**: today's gate run produced
`stage1.codex` === `stage2.codex` (byte-identical, SHA-256
201BFF91…D0D09A8E, 1,166,798 B each). `build.ps1` phases `text-stage1`,
`sem-equiv`, `text-stage2`, `text-fixedpoint` are all green.
...
**Original investigation below (Cam, 2026-04-30):**
