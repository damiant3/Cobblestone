...
| `LeanAndCodex.md` | lean-lang.org, a dependent type theory with a small trusted kernel, tactics, and formalised mathematics | The gap that matters is trust architecture and termination, not tactics or Mathlib. Lean claims from their published docs, Codex claims measured with paths. |
| `AgdaAndCodex.md` | agda/agda, a dependently typed language and interactive proof assistant written in Haskell | Agda's literate mode discards the prose before type checking, the exact inverse of CPL. Also their erasure modality, and what our `assume` lacks that `--safe` gives them. Nothing verified against their code. |
| `IdrisAndCodex.md` | idris-lang/Idris2, a self-hosted dependently typed language built on Quantitative Type Theory | The nearest neighbour we have: self-hosted, genuinely linear, bootstrapped from committed generated Scheme rather than a byte-identical fixed point. QTT gets erasure and linearity from one mechanism where we use two. Nothing verified against their code. |
