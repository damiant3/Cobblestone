# Build plugs/php/build-output/php-plug.cdx
[CmdletBinding()]
param([switch]$Force)
...
$OutFile   = Join-Path $OutDir 'php-plug.cdx'
...
Add-Chapter -Path (Join-Path $PlugDir 'PhpEmitter.codex')
Add-Chapter -Path (Join-Path $PlugDir 'PhpPlug.codex')
...
Write-Host "[php-plug] bundled $($preLines.Count + $lines.Count) lines, $($body.Length) bytes"
...
try {
    ...
    Write-Host "[php-plug] OK: $OutFile ($binSize bytes)"
    exit 0
} finally {
