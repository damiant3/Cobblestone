# Build plugs/lua/build-output/lua-plug.cdx
[CmdletBinding()]
param([switch]$Force)
...
$OutFile   = Join-Path $OutDir 'lua-plug.cdx'
...
Add-Chapter -Path (Join-Path $PlugDir 'LuaEmitter.codex')
Add-Chapter -Path (Join-Path $PlugDir 'LuaPlug.codex')
...
Write-Host "[lua-plug] bundled $($preLines.Count + $lines.Count) lines, $($body.Length) bytes"
...
try {
    ...
    Write-Host "[lua-plug] OK: $OutFile ($binSize bytes)"
    exit 0
} finally {
