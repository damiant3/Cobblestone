...
$OutFile   = Join-Path $OutDir 'ruby-plug.cdx'
...
Add-Chapter -Path (Join-Path $PlugDir 'RubyEmitter.codex')
Add-Chapter -Path (Join-Path $PlugDir 'RubyPlug.codex')
...
Write-Host "[ruby-plug] bundled $($preLines.Count + $lines.Count) lines, $($body.Length) bytes"
...
try {
    ...
    Write-Host "[ruby-plug] OK: $OutFile ($binSize bytes)"
    exit 0
} finally {
