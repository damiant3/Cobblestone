...
$OutFile   = Join-Path $OutDir 'scheme-plug.cdx'
...
Add-Chapter -Path (Join-Path $PlugDir 'SchemeEmitter.codex')
Add-Chapter -Path (Join-Path $PlugDir 'SchemePlug.codex')
...
Write-Host "[scheme-plug] bundled $($preLines.Count + $lines.Count) lines, $($body.Length) bytes"
...
try {
    ...
    Write-Host "[scheme-plug] OK: $OutFile ($binSize bytes)"
    exit 0
} finally {
