# Run the Scheme plug over a Codex source file via TCP.
[CmdletBinding()]
param(
...
$PlugCdx  = Join-Path $PlugDir 'build-output\scheme-plug.cdx'
...
if (-not (Test-Path -PathType Leaf $PlugCdx)) {
    [Console]::Error.WriteLine("MISSING: $PlugCdx -- run plugs/scheme/build.ps1 first")
    exit 2
}
...
# -- Phase 2: IR text -> Scheme via plug -------------------------------
...
try {
    ...
    $listener = [System.Net.Sockets.TcpListener]::new([System.Net.IPAddress]::Loopback, 9138)
    $listener.Start()
    ...
    Write-Host "[scheme-plug] OK: $Out ($($resp.Count) bytes)"
} finally {
