# Build plugs/vue/build-output/vue-plug.cdx
[CmdletBinding()]
param([switch]$Force)
...
$OutFile   = Join-Path $OutDir 'vue-plug.cdx'
...
Add-Chapter -Path (Join-Path $PlugDir 'VueEmitter.codex')
Add-Chapter -Path (Join-Path $PlugDir 'VuePlug.codex')
...
Write-Host "[vue-plug] bundled $($preLines.Count + $lines.Count) lines, $($body.Length) bytes"
...
try {
    ...
    Write-Host "[vue-plug] OK: $OutFile ($binSize bytes)"
    exit 0
} finally {
