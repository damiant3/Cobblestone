...
$PlugCdx  = Join-Path $PlugDir 'build-output\cobol-plug.cdx'
...
if (-not (Test-Path -PathType Leaf $PlugCdx)) {
    [Console]::Error.WriteLine("MISSING: $PlugCdx -- run plugs/cobol/build.ps1 first")
    exit 2
}
...
try {
    ...
    $listener.Stop()
    Write-Host "[cobol-run] Plug connected"
    ...
    $tcpStream.Flush()
    Write-Host "[cobol-run] Sent IR ($($irBytes.Length) bytes)"
    ...
    Write-Host "[cobol-run] OK: $Out ($($outText.Length) chars)"
    ...
    if ($serialDrain.Length -gt 0) { Write-Host "[cobol-run] Serial: $serialDrain" }
    exit 0
} finally {
    ...
    if ($run) {
        ...
        if ($run.StderrFile -and (Test-Path $run.StderrFile)) {
            ...
            if ($stderr.Length -gt 0) {
                [Console]::Error.WriteLine("[cobol-run] VM stderr:")
                [Console]::Error.WriteLine($stderr)
                if ($stderr -match 'RIP=0x([0-9a-fA-F]+)') {
                    $crashRip = [Convert]::ToInt64($matches[1], 16)
                    $mapPath = Join-Path $PlugDir 'build-output\cobol-plug.map'
