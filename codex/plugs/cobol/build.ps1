# Build plugs/cobol/build-output/cobol-plug.cdx
...
$OutFile   = Join-Path $OutDir 'cobol-plug.cdx'
...
Add-Chapter -Path (Join-Path $PlugDir 'CobolEmitter.codex')
Add-Chapter -Path (Join-Path $PlugDir 'CobolPlug.codex')
...
Write-Host "[cobol-plug] bundled $($preLines.Count + $lines.Count) lines, $($body.Length) bytes"
...
try {
    ...
    Write-Host "[cobol-plug] OK: $OutFile ($binSize bytes)"
    exit 0
} finally {
