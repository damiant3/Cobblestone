...
$OutFile   = Join-Path $OutDir 'swift-plug.cdx'
...
Add-Chapter -Path (Join-Path $PlugDir 'SwiftEmitter.codex')
Add-Chapter -Path (Join-Path $PlugDir 'SwiftPlug.codex')
...
Write-Host "[swift-plug] bundled $($preLines.Count + $lines.Count) lines, $($body.Length) bytes"
...
try {
    ...
    Write-Host "[swift-plug] OK: $OutFile ($binSize bytes)"
    exit 0
} finally {
