# Run the Swift plug over a Codex source file via TCP.
[CmdletBinding()]
param(
...
$PlugCdx  = Join-Path $PlugDir 'build-output\swift-plug.cdx'
...
if (-not (Test-Path -PathType Leaf $PlugCdx)) {
    [Console]::Error.WriteLine("MISSING: $PlugCdx -- run plugs/swift/build.ps1 first")
    exit 2
}
...
# -- Phase 2: IR text -> Swift via plug -------------------------------
...
try {
    ...
    $listener = [System.Net.Sockets.TcpListener]::new([System.Net.IPAddress]::Loopback, 9140)
    $listener.Start()
    ...
    Write-Host "[swift-plug] OK: $Out ($($resp.Count) bytes)"
} finally {
