...
$OutFile   = Join-Path $OutDir 'julia-plug.cdx'
...
Add-Chapter -Path (Join-Path $PlugDir 'JuliaEmitter.codex')
Add-Chapter -Path (Join-Path $PlugDir 'JuliaPlug.codex')
...
Write-Host "[julia-plug] bundled $($preLines.Count + $lines.Count) lines, $($body.Length) bytes"
...
try {
    ...
    Write-Host "[julia-plug] OK: $OutFile ($binSize bytes)"
    exit 0
} finally {
