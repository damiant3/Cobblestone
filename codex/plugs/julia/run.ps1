# Run the Julia plug over a Codex source file via TCP.
[CmdletBinding()]
param(
...
$PlugCdx  = Join-Path $PlugDir 'build-output\julia-plug.cdx'
...
if (-not (Test-Path -PathType Leaf $PlugCdx)) {
    [Console]::Error.WriteLine("MISSING: $PlugCdx -- run plugs/julia/build.ps1 first")
    exit 2
}
...
# -- Phase 2: IR text -> Julia via plug -------------------------------
...
try {
    ...
    $listener = [System.Net.Sockets.TcpListener]::new([System.Net.IPAddress]::Loopback, 9121)
    $listener.Start()
    ...
    Write-Host "[julia-plug] OK: $Out ($($resp.Count) bytes)"
} finally {
