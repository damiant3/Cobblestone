# Build plugs/clojure/build-output/clojure-plug.cdx
[CmdletBinding()]
param([switch]$Force)
...
$OutFile   = Join-Path $OutDir 'clojure-plug.cdx'
...
Add-Chapter -Path (Join-Path $PlugDir 'ClojureEmitter.codex')
Add-Chapter -Path (Join-Path $PlugDir 'ClojurePlug.codex')
...
Write-Host "[clojure-plug] bundled $($preLines.Count + $lines.Count) lines, $($body.Length) bytes"
...
try {
    ...
    Write-Host "[clojure-plug] OK: $OutFile ($binSize bytes)"
    exit 0
} finally {
