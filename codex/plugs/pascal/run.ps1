# Run the Pascal plug over a Codex source file via TCP.
[CmdletBinding()]
param(
...
$PlugCdx  = Join-Path $PlugDir 'build-output\pascal-plug.cdx'
...
if (-not (Test-Path -PathType Leaf $PlugCdx)) {
    [Console]::Error.WriteLine("MISSING: $PlugCdx -- run plugs/pascal/build.ps1 first")
    exit 2
}
...
# -- Phase 2: IR text -> Pascal via plug -------------------------------
...
try {
    ...
    $listener = [System.Net.Sockets.TcpListener]::new([System.Net.IPAddress]::Loopback, 9127)
    $listener.Start()
    ...
    Write-Host "[pascal-plug] OK: $Out ($($resp.Count) bytes)"
} finally {
