# Build plugs/pascal/build-output/pascal-plug.cdx
[CmdletBinding()]
param([switch]$Force)
...
$OutFile   = Join-Path $OutDir 'pascal-plug.cdx'
...
Add-Chapter -Path (Join-Path $PlugDir 'PascalEmitter.codex')
Add-Chapter -Path (Join-Path $PlugDir 'PascalPlug.codex')
...
Write-Host "[pascal-plug] bundled $($preLines.Count + $lines.Count) lines, $($body.Length) bytes"
...
try {
    ...
    Write-Host "[pascal-plug] OK: $OutFile ($binSize bytes)"
    exit 0
} finally {
