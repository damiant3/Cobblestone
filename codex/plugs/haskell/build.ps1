# Build plugs/haskell/build-output/haskell-plug.cdx
[CmdletBinding()]
param([switch]$Force)
...
$OutFile   = Join-Path $OutDir 'haskell-plug.cdx'
...
Add-Chapter -Path (Join-Path $PlugDir 'HaskellEmitter.codex')
Add-Chapter -Path (Join-Path $PlugDir 'HaskellPlug.codex')
...
Write-Host "[haskell-plug] bundled $($preLines.Count + $lines.Count) lines, $($body.Length) bytes"
...
try {
    ...
    Write-Host "[haskell-plug] OK: $OutFile ($binSize bytes)"
    exit 0
} finally {
