...
$PlugCdx  = Join-Path $PlugDir 'build-output\haskell-plug.cdx'
...
if (-not (Test-Path -PathType Leaf $PlugCdx)) {
    [Console]::Error.WriteLine("MISSING: $PlugCdx -- run plugs/haskell/build.ps1 first")
    exit 2
}
...
# -- Phase 2: IR text -> Haskell via plug -------------------------------
...
try {
    ...
    $listener = [System.Net.Sockets.TcpListener]::new([System.Net.IPAddress]::Loopback, 9117)
    $listener.Start()
    ...
    Write-Host "[haskell-plug] OK: $Out ($($resp.Count) bytes)"
} finally {
