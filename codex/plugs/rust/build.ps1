# Build plugs/rust/build-output/rust-plug.cdx
...
$OutFile   = Join-Path $OutDir 'rust-plug.cdx'
...
Add-Chapter -Path (Join-Path $PlugDir 'RustEmitter.codex')
Add-Chapter -Path (Join-Path $PlugDir 'RustPlug.codex')
...
Write-Host "[rust-plug] bundled $($preLines.Count + $lines.Count) lines, $($body.Length) bytes"
...
try {
    ...
    Write-Host "[rust-plug] OK: $OutFile ($binSize bytes)"
    exit 0
} finally {
