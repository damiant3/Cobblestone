# Run the Nim plug over a Codex source file via TCP.
[CmdletBinding()]
param(
...
$PlugCdx  = Join-Path $PlugDir 'build-output\nim-plug.cdx'
...
if (-not (Test-Path -PathType Leaf $PlugCdx)) {
    [Console]::Error.WriteLine("MISSING: $PlugCdx -- run plugs/nim/build.ps1 first")
    exit 2
}
...
try {
    ...
    $listener = [System.Net.Sockets.TcpListener]::new([System.Net.IPAddress]::Loopback, 9124)
    $listener.Start()
    ...
    Write-Host "[nim-plug] OK: $Out ($($resp.Count) bytes)"
} finally {
