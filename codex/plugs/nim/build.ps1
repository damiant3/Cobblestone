# Build plugs/nim/build-output/nim-plug.cdx
[CmdletBinding()]
param([switch]$Force)
...
$OutFile   = Join-Path $OutDir 'nim-plug.cdx'
...
Add-Chapter -Path (Join-Path $PlugDir 'NimEmitter.codex')
Add-Chapter -Path (Join-Path $PlugDir 'NimPlug.codex')
...
Write-Host "[nim-plug] bundled $($preLines.Count + $lines.Count) lines, $($body.Length) bytes"
...
try {
    ...
    Write-Host "[nim-plug] OK: $OutFile ($binSize bytes)"
    exit 0
} finally {
