...
try {
    ...
    if ($serialDrain.Length -gt 0) { Write-Host "[python-run] Serial: $serialDrain" }
    exit 0
} finally {
    ...
    if ($run) {
        ...
        if ($run.StderrFile -and (Test-Path $run.StderrFile)) {
            ...
            if ($stderr.Length -gt 0) {
                [Console]::Error.WriteLine("[python-run] VM stderr:")
                [Console]::Error.WriteLine($stderr)
                if ($stderr -match 'RIP=0x([0-9a-fA-F]+)') {
                    $crashRip = [Convert]::ToInt64($matches[1], 16)
                    $mapPath = Join-Path $PlugDir 'build-output\python-plug.map'
