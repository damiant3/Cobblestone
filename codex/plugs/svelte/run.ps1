# Run the Svelte plug over a Codex source file via TCP.
[CmdletBinding()]
param(
...
$PlugCdx  = Join-Path $PlugDir 'build-output\svelte-plug.cdx'
...
if (-not (Test-Path -PathType Leaf $PlugCdx)) {
    [Console]::Error.WriteLine("MISSING: $PlugCdx -- run plugs/svelte/build.ps1 first")
    exit 2
}
...
# -- Phase 2: IR text -> Svelte via plug -------------------------------
...
try {
    ...
    $listener = [System.Net.Sockets.TcpListener]::new([System.Net.IPAddress]::Loopback, 9139)
    $listener.Start()
    ...
    Write-Host "[svelte-plug] OK: $Out ($($resp.Count) bytes)"
} finally {
