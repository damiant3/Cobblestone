# Build plugs/svelte/build-output/svelte-plug.cdx
[CmdletBinding()]
param([switch]$Force)
...
$OutFile   = Join-Path $OutDir 'svelte-plug.cdx'
...
Add-Chapter -Path (Join-Path $PlugDir 'SvelteEmitter.codex')
Add-Chapter -Path (Join-Path $PlugDir 'SveltePlug.codex')
...
Write-Host "[svelte-plug] bundled $($preLines.Count + $lines.Count) lines, $($body.Length) bytes"
...
try {
    ...
    Write-Host "[svelte-plug] OK: $OutFile ($binSize bytes)"
    exit 0
} finally {
