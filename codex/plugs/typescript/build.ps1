# Build plugs/typescript/build-output/typescript-plug.cdx
[CmdletBinding()]
param([switch]$Force)
...
$OutFile   = Join-Path $OutDir 'typescript-plug.cdx'
...
Add-Chapter -Path (Join-Path $PlugDir 'TypeScriptEmitter.codex')
Add-Chapter -Path (Join-Path $PlugDir 'TypeScriptPlug.codex')
...
Write-Host "[typescript-plug] bundled $($preLines.Count + $lines.Count) lines, $($body.Length) bytes"
...
try {
    ...
    Write-Host "[typescript-plug] OK: $OutFile ($binSize bytes)"
    exit 0
} finally {
