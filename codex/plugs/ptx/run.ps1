# Run PTX plug: source -> IR-CCE -> plug CDX -> PTX
[CmdletBinding()]
...
Write-Host "[ptx-run] IR: $((Get-Item $IrFile).Length) bytes (CCE)"
...
[System.IO.File]::WriteAllText($Out, $ptx, [System.Text.UTF8Encoding]::new($false))
Write-Host "[ptx-plug] OK: $Out ($($ptx.Length) chars)"
