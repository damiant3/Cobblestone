# Build plugs/flutter/build-output/flutter-plug.cdx
[CmdletBinding()]
param([switch]$Force)
...
$OutFile   = Join-Path $OutDir 'flutter-plug.cdx'
...
Add-Chapter -Path (Join-Path $PlugDir 'FlutterEmitter.codex')
Add-Chapter -Path (Join-Path $PlugDir 'FlutterPlug.codex')
...
Write-Host "[flutter-plug] bundled $($preLines.Count + $lines.Count) lines, $($body.Length) bytes"
...
try {
    ...
    Write-Host "[flutter-plug] OK: $OutFile ($binSize bytes)"
    exit 0
} finally {
