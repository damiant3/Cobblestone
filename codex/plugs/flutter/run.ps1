# Run the Flutter plug over a Codex source file via TCP.
[CmdletBinding()]
param(
...
$PlugCdx  = Join-Path $PlugDir 'build-output\flutter-plug.cdx'
...
if (-not (Test-Path -PathType Leaf $PlugCdx)) {
    [Console]::Error.WriteLine("MISSING: $PlugCdx -- run plugs/flutter/build.ps1 first")
    exit 2
}
...
# -- Phase 2: IR text -> Flutter Dart via plug ----------------------
...
try {
    ...
    $listener = [System.Net.Sockets.TcpListener]::new([System.Net.IPAddress]::Loopback, 9112)
    $listener.Start()
    ...
    Write-Host "[flutter-plug] OK: $Out ($($resp.Count) bytes)"
} finally {
