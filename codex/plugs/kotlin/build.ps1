# Build plugs/kotlin/build-output/kotlin-plug.cdx
[CmdletBinding()]
param([switch]$Force)
...
$OutFile   = Join-Path $OutDir 'kotlin-plug.cdx'
...
Add-Chapter -Path (Join-Path $PlugDir 'KotlinEmitter.codex')
Add-Chapter -Path (Join-Path $PlugDir 'KotlinPlug.codex')
...
Write-Host "[kotlin-plug] bundled $($preLines.Count + $lines.Count) lines, $($body.Length) bytes"
...
try {
    ...
    Write-Host "[kotlin-plug] OK: $OutFile ($binSize bytes)"
    exit 0
} finally {
