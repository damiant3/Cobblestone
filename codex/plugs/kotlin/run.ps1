# Run the Kotlin plug over a Codex source file via TCP.
[CmdletBinding()]
param(
...
$PlugCdx  = Join-Path $PlugDir 'build-output\kotlin-plug.cdx'
...
if (-not (Test-Path -PathType Leaf $PlugCdx)) {
    [Console]::Error.WriteLine("MISSING: $PlugCdx -- run plugs/kotlin/build.ps1 first")
    exit 2
}
...
# -- Phase 2: IR text -> Kotlin via plug -------------------------------
...
try {
    ...
    $listener = [System.Net.Sockets.TcpListener]::new([System.Net.IPAddress]::Loopback, 9122)
    $listener.Start()
    ...
    Write-Host "[kotlin-plug] OK: $Out ($($resp.Count) bytes)"
} finally {
