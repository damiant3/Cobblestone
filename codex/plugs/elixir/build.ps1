...
$OutFile   = Join-Path $OutDir 'elixir-plug.cdx'
...
Add-Chapter -Path (Join-Path $PlugDir 'ElixirEmitter.codex')
Add-Chapter -Path (Join-Path $PlugDir 'ElixirPlug.codex')
...
Write-Host "[elixir-plug] bundled $($preLines.Count + $lines.Count) lines, $($body.Length) bytes"
...
try {
    ...
    Write-Host "[elixir-plug] OK: $OutFile ($binSize bytes)"
    exit 0
} finally {
