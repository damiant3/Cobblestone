# Build plugs/maui/build-output/maui-plug.cdx
[CmdletBinding()]
param([switch]$Force)
...
$OutFile   = Join-Path $OutDir 'maui-plug.cdx'
...
Add-Chapter -Path (Join-Path $PlugDir 'MauiEmitter.codex')
Add-Chapter -Path (Join-Path $PlugDir 'MauiPlug.codex')
...
Write-Host "[maui-plug] bundled $($preLines.Count + $lines.Count) lines, $($body.Length) bytes"
...
try {
    ...
    Write-Host "[maui-plug] OK: $OutFile ($binSize bytes)"
    exit 0
} finally {
