# Build plugs/d/build-output/d-plug.cdx
[CmdletBinding()]
param([switch]$Force)
...
$OutFile   = Join-Path $OutDir 'd-plug.cdx'
...
Add-Chapter -Path (Join-Path $PlugDir 'DEmitter.codex')
Add-Chapter -Path (Join-Path $PlugDir 'DPlug.codex')
...
Write-Host "[d-plug] bundled $($preLines.Count + $lines.Count) lines, $($body.Length) bytes"
...
try {
    ...
    Write-Host "[d-plug] OK: $OutFile ($binSize bytes)"
    exit 0
} finally {
