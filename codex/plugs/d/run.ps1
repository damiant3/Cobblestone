# Run the D plug over a Codex source file via TCP.
[CmdletBinding()]
param(
...
$PlugCdx  = Join-Path $PlugDir 'build-output\d-plug.cdx'
...
if (-not (Test-Path -PathType Leaf $PlugCdx)) {
    [Console]::Error.WriteLine("MISSING: $PlugCdx -- run plugs/d/build.ps1 first")
    exit 2
}
...
try {
    ...
    Write-Host "[d-plug] OK: $Out ($($resp.Count) bytes)"
} finally {
