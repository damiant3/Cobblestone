...
$PlugCdx  = Join-Path $PlugDir 'build-output\zig-plug.cdx'
...
if (-not (Test-Path -PathType Leaf $PlugCdx)) {
    [Console]::Error.WriteLine("MISSING: $PlugCdx -- run plugs/zig/build.ps1 first")
    exit 2
}
...
try {
    ...
    $listener = [System.Net.Sockets.TcpListener]::new([System.Net.IPAddress]::Loopback, 9145)
    $listener.Start()
    ...
    Write-Host "[zig-plug] OK: $Out ($($resp.Count) bytes)"
} finally {
