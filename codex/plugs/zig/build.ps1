...
$OutFile   = Join-Path $OutDir 'zig-plug.cdx'
...
Add-Chapter -Path (Join-Path $PlugDir 'ZigEmitter.codex')
Add-Chapter -Path (Join-Path $PlugDir 'ZigPlug.codex')
...
Write-Host "[zig-plug] bundled $($preLines.Count + $lines.Count) lines, $($body.Length) bytes"
...
try {
    ...
    Write-Host "[zig-plug] OK: $OutFile ($binSize bytes)"
    exit 0
} finally {
