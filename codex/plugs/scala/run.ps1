# Run the Scala plug over a Codex source file via TCP.
[CmdletBinding()]
param(
...
$PlugCdx  = Join-Path $PlugDir 'build-output\scala-plug.cdx'
...
if (-not (Test-Path -PathType Leaf $PlugCdx)) {
    [Console]::Error.WriteLine("MISSING: $PlugCdx -- run plugs/scala/build.ps1 first")
    exit 2
}
...
# -- Phase 2: IR text -> Scala via plug -------------------------------
...
try {
    ...
    $listener = [System.Net.Sockets.TcpListener]::new([System.Net.IPAddress]::Loopback, 9137)
    $listener.Start()
    ...
    Write-Host "[scala-plug] OK: $Out ($($resp.Count) bytes)"
} finally {
