...
$OutFile   = Join-Path $OutDir 'scala-plug.cdx'
...
Add-Chapter -Path (Join-Path $PlugDir 'ScalaEmitter.codex')
Add-Chapter -Path (Join-Path $PlugDir 'ScalaPlug.codex')
...
Write-Host "[scala-plug] bundled $($preLines.Count + $lines.Count) lines, $($body.Length) bytes"
...
try {
    ...
    Write-Host "[scala-plug] OK: $OutFile ($binSize bytes)"
    exit 0
} finally {
