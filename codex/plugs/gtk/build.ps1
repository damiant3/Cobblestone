# Build plugs/gtk/build-output/gtk-plug.cdx
[CmdletBinding()]
param([switch]$Force)
...
$OutFile   = Join-Path $OutDir 'gtk-plug.cdx'
...
Add-Chapter -Path (Join-Path $PlugDir 'GtkEmitter.codex')
Add-Chapter -Path (Join-Path $PlugDir 'GtkPlug.codex')
...
Write-Host "[gtk-plug] bundled $($preLines.Count + $lines.Count) lines, $($body.Length) bytes"
...
try {
    ...
    Write-Host "[gtk-plug] OK: $OutFile ($binSize bytes)"
    exit 0
} finally {
