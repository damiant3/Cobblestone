# Run the React plug over a Codex source file via TCP.
[CmdletBinding()]
param(
...
$PlugCdx  = Join-Path $PlugDir 'build-output\react-plug.cdx'
...
if (-not (Test-Path -PathType Leaf $PlugCdx)) {
    [Console]::Error.WriteLine("MISSING: $PlugCdx -- run plugs/react/build.ps1 first")
    exit 2
}
...
# -- Phase 2: IR text -> React via plug -----------------------------
...
try {
    ...
    $listener = [System.Net.Sockets.TcpListener]::new([System.Net.IPAddress]::Loopback, 9133)
    $listener.Start()
    ...
    Write-Host "[react-plug] OK: $Out ($($resp.Count) bytes)"
} finally {
