# Build plugs/react/build-output/react-plug.cdx
[CmdletBinding()]
param([switch]$Force)
...
$OutFile   = Join-Path $OutDir 'react-plug.cdx'
...
Add-Chapter -Path (Join-Path $PlugDir 'ReactEmitter.codex')
Add-Chapter -Path (Join-Path $PlugDir 'ReactPlug.codex')
...
Write-Host "[react-plug] bundled $($preLines.Count + $lines.Count) lines, $($body.Length) bytes"
...
try {
    ...
    Write-Host "[react-plug] OK: $OutFile ($binSize bytes)"
    exit 0
} finally {
