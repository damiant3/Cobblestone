# Run the Perl plug over a Codex source file via TCP.
[CmdletBinding()]
param(
...
$PlugCdx  = Join-Path $PlugDir 'build-output\perl-plug.cdx'
...
if (-not (Test-Path -PathType Leaf $PlugCdx)) {
    [Console]::Error.WriteLine("MISSING: $PlugCdx -- run plugs/perl/build.ps1 first")
    exit 2
}
...
try {
    ...
    $listener = [System.Net.Sockets.TcpListener]::new([System.Net.IPAddress]::Loopback, 9129)
    $listener.Start()
    ...
    Write-Host "[perl-plug] OK: $Out ($($resp.Count) bytes)"
} finally {
