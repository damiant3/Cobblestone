# Build plugs/perl/build-output/perl-plug.cdx
[CmdletBinding()]
param([switch]$Force)
...
$OutFile   = Join-Path $OutDir 'perl-plug.cdx'
...
Add-Chapter -Path (Join-Path $PlugDir 'PerlEmitter.codex')
Add-Chapter -Path (Join-Path $PlugDir 'PerlPlug.codex')
...
Write-Host "[perl-plug] bundled $($preLines.Count + $lines.Count) lines, $($body.Length) bytes"
...
try {
    ...
    Write-Host "[perl-plug] OK: $OutFile ($binSize bytes)"
    exit 0
} finally {
