# Run the Electron plug over a Codex source file via TCP.
[CmdletBinding()]
param(
...
$PlugCdx  = Join-Path $PlugDir 'build-output\electron-plug.cdx'
...
if (-not (Test-Path -PathType Leaf $PlugCdx)) {
    [Console]::Error.WriteLine("MISSING: $PlugCdx -- run plugs/electron/build.ps1 first")
    exit 2
}
...
# -- Phase 2: IR text -> Electron via plug -------------------------------
...
try {
    ...
    $listener = [System.Net.Sockets.TcpListener]::new([System.Net.IPAddress]::Loopback, 9109)
    $listener.Start()
    ...
    Write-Host "[electron-plug] OK: $Out ($($resp.Count) bytes)"
} finally {
