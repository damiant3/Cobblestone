# Build plugs/electron/build-output/electron-plug.cdx
[CmdletBinding()]
param([switch]$Force)
...
$OutFile   = Join-Path $OutDir 'electron-plug.cdx'
...
Add-Chapter -Path (Join-Path $PlugDir 'ElectronEmitter.codex')
Add-Chapter -Path (Join-Path $PlugDir 'ElectronPlug.codex')
...
Write-Host "[electron-plug] bundled $($preLines.Count + $lines.Count) lines, $($body.Length) bytes"
...
try {
    ...
    Write-Host "[electron-plug] OK: $OutFile ($binSize bytes)"
    exit 0
} finally {
