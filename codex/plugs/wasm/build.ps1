...
$OutFile   = Join-Path $OutDir 'wasm-plug.cdx'
...
Add-Chapter -Path (Join-Path $PlugDir 'WasmEmitter.codex')
Add-Chapter -Path (Join-Path $PlugDir 'WasmPlug.codex')
...
Write-Host "[wasm-plug] bundled $($preLines.Count + $lines.Count) lines, $($body.Length) bytes"
...
try {
    ...
    Write-Host "[wasm-plug] OK: $OutFile ($binSize bytes)"
    exit 0
} finally {
