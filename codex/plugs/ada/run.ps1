...
$PlugCdx  = Join-Path $PlugDir 'build-output\ada-plug.cdx'
...
if (-not (Test-Path -PathType Leaf $PlugCdx)) {
    [Console]::Error.WriteLine("MISSING: $PlugCdx -- run plugs/ada/build.ps1 first")
    exit 2
}
...
try {
    ...
    $listener.Stop()
    Write-Host "[ada-run] Plug connected"
    ...
    $tcpStream.Flush()
    Write-Host "[ada-run] Sent IR ($($irBytes.Length) bytes)"
    ...
    Write-Host "[ada-run] OK: $Out ($($outText.Length) chars)"
    ...
    if ($serialDrain.Length -gt 0) { Write-Host "[ada-run] Serial: $serialDrain" }
    exit 0
} finally {
    ...
    if ($run) {
        ...
        if ($run.StderrFile -and (Test-Path $run.StderrFile)) {
            ...
            if ($stderr.Length -gt 0) {
                [Console]::Error.WriteLine("[ada-run] VM stderr:")
                [Console]::Error.WriteLine($stderr)
                if ($stderr -match 'RIP=0x([0-9a-fA-F]+)') {
                    $crashRip = [Convert]::ToInt64($matches[1], 16)
                    $mapPath = Join-Path $PlugDir 'build-output\ada-plug.map'
