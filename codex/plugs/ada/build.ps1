# Build plugs/ada/build-output/ada-plug.cdx
...
$OutFile   = Join-Path $OutDir 'ada-plug.cdx'
...
Add-Chapter -Path (Join-Path $PlugDir 'AdaEmitter.codex')
Add-Chapter -Path (Join-Path $PlugDir 'AdaPlug.codex')
...
Write-Host "[ada-plug] bundled $($preLines.Count + $lines.Count) lines, $($body.Length) bytes"
...
try {
    ...
    Write-Host "[ada-plug] OK: $OutFile ($binSize bytes)"
    exit 0
} finally {
