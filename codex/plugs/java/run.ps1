# Run the Java plug over a Codex source file via TCP.
[CmdletBinding()]
param(
...
$PlugCdx  = Join-Path $PlugDir 'build-output\java-plug.cdx'
...
if (-not (Test-Path -PathType Leaf $PlugCdx)) {
    [Console]::Error.WriteLine("MISSING: $PlugCdx -- run plugs/java/build.ps1 first")
    exit 2
}
...
try {
    ...
    $listener = [System.Net.Sockets.TcpListener]::new([System.Net.IPAddress]::Loopback, 9119)
    $listener.Start()
    ...
    Write-Host "[java-plug] OK: $Out ($($resp.Count) bytes)"
} finally {
