# Build plugs/java/build-output/java-plug.cdx
[CmdletBinding()]
param([switch]$Force)
...
$OutFile   = Join-Path $OutDir 'java-plug.cdx'
...
Add-Chapter -Path (Join-Path $PlugDir 'JavaEmitter.codex')
Add-Chapter -Path (Join-Path $PlugDir 'JavaPlug.codex')
...
Write-Host "[java-plug] bundled $($preLines.Count + $lines.Count) lines, $($body.Length) bytes"
...
try {
    ...
    Write-Host "[java-plug] OK: $OutFile ($binSize bytes)"
    exit 0
} finally {
