# Build plugs/angular/build-output/angular-plug.cdx
[CmdletBinding()]
param([switch]$Force)
...
$OutFile   = Join-Path $OutDir 'angular-plug.cdx'
...
Add-Chapter -Path (Join-Path $PlugDir 'AngularEmitter.codex')
Add-Chapter -Path (Join-Path $PlugDir 'AngularPlug.codex')
...
Write-Host "[angular-plug] bundled $($preLines.Count + $lines.Count) lines, $($body.Length) bytes"
...
try {
    ...
    Write-Host "[angular-plug] OK: $OutFile ($binSize bytes)"
    exit 0
} finally {
