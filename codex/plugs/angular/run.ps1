# Run the Angular plug over a Codex source file via TCP.
[CmdletBinding()]
param(
...
$PlugCdx  = Join-Path $PlugDir 'build-output\angular-plug.cdx'
...
if (-not (Test-Path -PathType Leaf $PlugCdx)) {
    [Console]::Error.WriteLine("MISSING: $PlugCdx -- run plugs/angular/build.ps1 first")
    exit 2
}
...
# -- Phase 2: IR text -> Angular via plug -------------------------------
...
try {
    ...
    $listener = [System.Net.Sockets.TcpListener]::new([System.Net.IPAddress]::Loopback, 9102)
    $listener.Start()
    ...
    Write-Host "[angular-plug] OK: $Out ($($resp.Count) bytes)"
} finally {
