...
$OutFile   = Join-Path $OutDir 'wpf-plug.cdx'
...
Add-Chapter -Path (Join-Path $PlugDir 'WpfEmitter.codex')
Add-Chapter -Path (Join-Path $PlugDir 'WpfPlug.codex')
...
Write-Host "[wpf-plug] bundled $($preLines.Count + $lines.Count) lines, $($body.Length) bytes"
...
try {
    ...
    Write-Host "[wpf-plug] OK: $OutFile ($binSize bytes)"
    exit 0
} finally {
