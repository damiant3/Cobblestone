# Build plugs/fortran/build-output/fortran-plug.cdx
...
$OutFile   = Join-Path $OutDir 'fortran-plug.cdx'
...
Add-Chapter -Path (Join-Path $PlugDir 'FortranEmitter.codex')
Add-Chapter -Path (Join-Path $PlugDir 'FortranPlug.codex')
...
Write-Host "[fortran-plug] bundled $($preLines.Count + $lines.Count) lines, $($body.Length) bytes"
...
try {
    ...
    Write-Host "[fortran-plug] OK: $OutFile ($binSize bytes)"
    exit 0
} finally {
