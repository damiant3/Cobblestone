# Build plugs/qt/build-output/qt-plug.cdx
[CmdletBinding()]
param([switch]$Force)
...
$OutFile   = Join-Path $OutDir 'qt-plug.cdx'
...
Add-Chapter -Path (Join-Path $PlugDir 'QtEmitter.codex')
Add-Chapter -Path (Join-Path $PlugDir 'QtPlug.codex')
...
Write-Host "[qt-plug] bundled $($preLines.Count + $lines.Count) lines, $($body.Length) bytes"
...
try {
    ...
    Write-Host "[qt-plug] OK: $OutFile ($binSize bytes)"
    exit 0
} finally {
