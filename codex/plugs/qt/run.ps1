# Run the Qt plug over a Codex source file via TCP.
[CmdletBinding()]
param(
...
$PlugCdx  = Join-Path $PlugDir 'build-output\qt-plug.cdx'
...
if (-not (Test-Path -PathType Leaf $PlugCdx)) {
    [Console]::Error.WriteLine("MISSING: $PlugCdx -- run plugs/qt/build.ps1 first")
    exit 2
}
...
try {
    ...
    $listener = [System.Net.Sockets.TcpListener]::new([System.Net.IPAddress]::Loopback, 9132)
    $listener.Start()
    ...
    Write-Host "[qt-plug] OK: $Out ($($resp.Count) bytes)"
} finally {
