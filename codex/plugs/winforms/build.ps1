...
$OutFile   = Join-Path $OutDir 'winforms-plug.cdx'
...
Add-Chapter -Path (Join-Path $PlugDir 'WinFormsEmitter.codex')
Add-Chapter -Path (Join-Path $PlugDir 'WinFormsPlug.codex')
...
Write-Host "[winforms-plug] bundled $($preLines.Count + $lines.Count) lines, $($body.Length) bytes"
...
try {
    ...
    Write-Host "[winforms-plug] OK: $OutFile ($binSize bytes)"
    exit 0
} finally {
