# Run the WinForms plug over a Codex source file via TCP.
[CmdletBinding()]
param(
...
$PlugCdx  = Join-Path $PlugDir 'build-output\winforms-plug.cdx'
...
if (-not (Test-Path -PathType Leaf $PlugCdx)) {
    [Console]::Error.WriteLine("MISSING: $PlugCdx -- run plugs/winforms/build.ps1 first")
    exit 2
}
...
# -- Phase 2: IR text -> WinForms via plug --------------------------
$stderrFile = [System.IO.Path]::GetTempFileName()
    ...
    Write-Host "[winforms-plug] OK: $Out ($($resp.Count) bytes)"
} finally {
