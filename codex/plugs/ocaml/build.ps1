...
$OutFile   = Join-Path $OutDir 'ocaml-plug.cdx'
...
Add-Chapter -Path (Join-Path $PlugDir 'OCamlEmitter.codex')
Add-Chapter -Path (Join-Path $PlugDir 'OCamlPlug.codex')
...
Write-Host "[ocaml-plug] bundled $($preLines.Count + $lines.Count) lines, $($body.Length) bytes"
...
try {
    ...
    Write-Host "[ocaml-plug] OK: $OutFile ($binSize bytes)"
    exit 0
} finally {
