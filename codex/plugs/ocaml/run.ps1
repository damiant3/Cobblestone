# Run the OCaml plug over a Codex source file via TCP.
[CmdletBinding()]
param(
...
$PlugCdx  = Join-Path $PlugDir 'build-output\ocaml-plug.cdx'
...
if (-not (Test-Path -PathType Leaf $PlugCdx)) {
    [Console]::Error.WriteLine("MISSING: $PlugCdx -- run plugs/ocaml/build.ps1 first")
    exit 2
}
...
# -- Phase 2: IR text -> OCaml via plug -------------------------------
...
try {
    ...
    $listener = [System.Net.Sockets.TcpListener]::new([System.Net.IPAddress]::Loopback, 9126)
    $listener.Start()
    ...
    Write-Host "[ocaml-plug] OK: $Out ($($resp.Count) bytes)"
} finally {
