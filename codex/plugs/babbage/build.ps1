# Build plugs/babbage/build-output/babbage-plug.cdx
...
$OutFile   = Join-Path $OutDir 'babbage-plug.cdx'
...
Add-Chapter -Path (Join-Path $PlugDir 'BabbageEmitter.codex')
Add-Chapter -Path (Join-Path $PlugDir 'BabbagePlug.codex')
...
Write-Host "[babbage-plug] bundled $($preLines.Count + $lines.Count) lines, $($body.Length) bytes"
...
try {
    ...
    Write-Host "[babbage-plug] OK: $OutFile ($binSize bytes)"
    exit 0
} finally {
