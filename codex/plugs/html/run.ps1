# Run the HTML plug over a Codex source file via TCP.
[CmdletBinding()]
param(
...
if (-not (Test-Path -PathType Leaf $PlugCdx)) {
    [Console]::Error.WriteLine("MISSING: $PlugCdx -- run plugs/html/build.ps1 first")
    exit 2
}
...
try {
    ...
    Write-Host "[html-plug] OK: $Out ($($resp.Count) bytes)"
} finally {
