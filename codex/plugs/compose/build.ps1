# Build plugs/compose/build-output/compose-plug.cdx
[CmdletBinding()]
param([switch]$Force)
...
$OutFile   = Join-Path $OutDir 'compose-plug.cdx'
...
Add-Chapter -Path (Join-Path $PlugDir 'ComposeEmitter.codex')
Add-Chapter -Path (Join-Path $PlugDir 'ComposePlug.codex')
...
Write-Host "[compose-plug] bundled $($preLines.Count + $lines.Count) lines, $($body.Length) bytes"
...
try {
    ...
    Write-Host "[compose-plug] OK: $OutFile ($binSize bytes)"
    exit 0
} finally {
