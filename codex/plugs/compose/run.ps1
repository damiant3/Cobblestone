# Run the Compose plug over a Codex source file via TCP.
[CmdletBinding()]
param(
...
$PlugCdx  = Join-Path $PlugDir 'build-output\compose-plug.cdx'
...
if (-not (Test-Path -PathType Leaf $PlugCdx)) {
    [Console]::Error.WriteLine("MISSING: $PlugCdx -- run plugs/compose/build.ps1 first")
    exit 2
}
...
# -- Phase 2: IR text -> Compose Kotlin via plug --------------------
...
try {
    ...
    $listener = [System.Net.Sockets.TcpListener]::new([System.Net.IPAddress]::Loopback, 9106)
    $listener.Start()
    ...
    Write-Host "[compose-plug] OK: $Out ($($resp.Count) bytes)"
} finally {
