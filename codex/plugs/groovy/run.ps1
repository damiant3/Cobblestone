# Run the Groovy plug over a Codex source file via TCP.
[CmdletBinding()]
param(
...
$PlugCdx  = Join-Path $PlugDir 'build-output\groovy-plug.cdx'
...
if (-not (Test-Path -PathType Leaf $PlugCdx)) {
    [Console]::Error.WriteLine("MISSING: $PlugCdx -- run plugs/groovy/build.ps1 first")
    exit 2
}
...
# -- Phase 2: IR text -> Groovy via plug -------------------------------
...
try {
    ...
    $listener = [System.Net.Sockets.TcpListener]::new([System.Net.IPAddress]::Loopback, 9115)
    $listener.Start()
    ...
    Write-Host "[groovy-plug] OK: $Out ($($resp.Count) bytes)"
} finally {
