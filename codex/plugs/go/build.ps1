# Build plugs/go/build-output/go-plug.cdx
[CmdletBinding()]
param([switch]$Force)
...
$OutFile   = Join-Path $OutDir 'go-plug.cdx'
...
Add-Chapter -Path (Join-Path $PlugDir 'GoEmitter.codex')
Add-Chapter -Path (Join-Path $PlugDir 'GoPlug.codex')
...
Write-Host "[go-plug] bundled $($preLines.Count + $lines.Count) lines, $($body.Length) bytes"
...
try {
    ...
    Write-Host "[go-plug] OK: $OutFile ($binSize bytes)"
    exit 0
} finally {
