...
# Bundles: ByteHelpers + PlugChain + ElfWriter + DwarfWriter + ElfPlug
[CmdletBinding()]
param([switch]$Force)
...
$OutFile   = Join-Path $OutDir 'elf-plug.cdx'
...
Add-Chapter -Path (Join-Path $PlugDir 'ElfWriter.codex')
Add-Chapter -Path (Join-Path $PlugDir 'DwarfWriter.codex')
Add-Chapter -Path (Join-Path $PlugDir 'ElfPlug.codex')
...
Write-Host "[elf-plug] bundled $($preLines.Count + $lines.Count) lines, $($body.Length) bytes"

# ── Compile via seed CDX ─────────────────────────────────────────────
$run = Start-VmRun -Kernel $Stage0 -ConnectTimeoutSec 60 -MemMB 4096
if (-not $run) { [Console]::Error.WriteLine("FAIL: VM did not start"); exit 4 }
try {
    ...
    Write-Host "[elf-plug] OK: $OutFile ($binSize bytes)"
    exit 0
} finally {
