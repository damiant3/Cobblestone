# Build plugs/objc/build-output/objc-plug.cdx
[CmdletBinding()]
param([switch]$Force)
...
$OutFile   = Join-Path $OutDir 'objc-plug.cdx'
...
Add-Chapter -Path (Join-Path $PlugDir 'ObjCEmitter.codex')
Add-Chapter -Path (Join-Path $PlugDir 'ObjCPlug.codex')
...
Write-Host "[objc-plug] bundled $($preLines.Count + $lines.Count) lines, $($body.Length) bytes"
...
try {
    ...
    Write-Host "[objc-plug] OK: $OutFile ($binSize bytes)"
    exit 0
} finally {
