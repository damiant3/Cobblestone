...
$OutFile   = Join-Path $OutDir 'swiftui-plug.cdx'
...
Add-Chapter -Path (Join-Path $PlugDir 'SwiftUIEmitter.codex')
Add-Chapter -Path (Join-Path $PlugDir 'SwiftUIPlug.codex')
...
Write-Host "[swiftui-plug] bundled $($preLines.Count + $lines.Count) lines, $($body.Length) bytes"
...
try {
    ...
    Write-Host "[swiftui-plug] OK: $OutFile ($binSize bytes)"
    exit 0
} finally {
