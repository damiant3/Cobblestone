# Run the SwiftUI plug over a Codex source file via TCP.
[CmdletBinding()]
param(
...
$PlugCdx  = Join-Path $PlugDir 'build-output\swiftui-plug.cdx'
...
if (-not (Test-Path -PathType Leaf $PlugCdx)) {
    [Console]::Error.WriteLine("MISSING: $PlugCdx -- run plugs/swiftui/build.ps1 first")
    exit 2
}
...
# -- Phase 2: IR text -> SwiftUI via plug ---------------------------
...
try {
    ...
    $listener = [System.Net.Sockets.TcpListener]::new([System.Net.IPAddress]::Loopback, 9141)
    $listener.Start()
    ...
    Write-Host "[swiftui-plug] OK: $Out ($($resp.Count) bytes)"
} finally {
