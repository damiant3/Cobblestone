# Foreword Engine -- open capabilities
...
**ENG-1 was here and the migration is declined.** WorldGen's hash ends on a
fold, and its own row said so: it is not broken, and the entry was duplication
rather than defect. Migrating changes world output and forces every expectation
downstream to be re-recorded, and buys one shared definition in place of a
private copy. Damian declined that trade on 2026-07-27. Its consumer was NOT
measured, so if world generation is ever suspected on other grounds this is an
open question again and this paragraph is not evidence about it.