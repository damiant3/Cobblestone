...
| COMPILER-104 | **Under OVMF a PS/2 keystroke never reaches the x86-64 kernel's IRQ1 handler.** | `build/boot/diag-arm.ps1 -Only ovmf-cad-ps2` (q35 with its i8042, no USB keyboard, QEMU `sendkey ctrl-alt-delete` after END) finds QEMU still running, on a diag image built by the depot seed and by the Ctrl-Alt-Del seed alike (2026-09-25). The same image resets in codex-vm on the PS/2 chord (`cad-summary-ps2`), so `emit-kbd-chord` works whenever IRQ1 fires; under OVMF either the i8042 or the IRQ1 delivery the kernel sets up (`emit-kbd-init`, `emit-pic-init` in `Emit/X86_64Boot.codex`) is not what firmware leaves behind, and no arm had sent a PS/2 key under OVMF before. The arm is out of the default rehearsal until this closes; under OVMF, Ctrl-Alt-Del is rehearsed over a USB keyboard only (`ovmf-cad`), and no real hardware has been measured. Unowned (root 2026-09-25). |
