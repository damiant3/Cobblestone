...
#   <Codex source.codex>
#     │
#     ▼  codex.build/sample-compile-selfhost.ps1 -Ir
...
. (Join-Path $PSScriptRoot '..' '..' 'codex.build' 'qemu-config.ps1')
...
$compileScript = Join-Path $Repo 'codex.build\sample-compile-selfhost.ps1'
...
$run = Start-QemuRun -Kernel $PlugCdx -ConnectTimeoutSec 30 -MemMB 1024
if (-not $run) {
