...
if (-not (Test-Path -PathType Leaf $Stage0)) {
    [Console]::Error.WriteLine("MISSING: $Stage0 — run codex.build/pingpong-self.ps1 first to build the self-host")
    exit 2
}
...
$run = Start-QemuRun -Kernel $Stage0 -ConnectTimeoutSec 30 -MemMB 1024
if (-not $run) {
