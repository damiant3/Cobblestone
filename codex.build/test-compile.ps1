...
# compiler (Codex.cdx) in QEMU and feeding the source over a
# socket-chardev serial port. Default output is CDX; pass -Elf for ELF,
# or -Ir for IR text emission to stdout (captured between IR-BEGIN /
# IR-END sentinels and written to $Out as UTF-8 text).
#
...
$QuireDirs = @{ 'Foreword' = 'codex.foreword'; 'Kernel' = 'codex.kernel'; 'OS' = 'codex.os'; 'Works' = 'codex.works'; 'Trust' = 'codex.os.trust'; 'Net' = 'codex.os.net'; 'Verify' = 'codex.os.verify'; 'Replay' = 'codex.os.replay'; 'Sched' = 'codex.os.sched'; 'Observe' = 'codex.os.observe'; 'Game' = 'codex.foreword.game'; 'Signal' = 'codex.foreword.signal'; 'Compress' = 'codex.foreword.compress'; 'Encode' = 'codex.foreword.encode'; 'Math' = 'codex.foreword.math'; 'Sim' = 'codex.foreword.sim'; 'AI' = 'codex.foreword.ai'; 'UI' = 'codex.foreword.ui'; 'Dev' = 'codex.os.dev' }
...
try {
    ...
    # Launch QEMU + connect (4 retry attempts on bind/listen failure).
    $run = Start-QemuRun -Kernel $Stage0 -ConnectTimeoutSec 30 -MemMB 2048 -PCore $PCore
    if (-not $run) {
        "FAIL: QEMU did not listen after 4 attempts" | Set-Content -Path $Log -Encoding UTF8
        exit 3
    }
    $conn = $run.Conn
    ...
    $mode = if ($Ir) { "IR" } elseif ($Img) { "IMG" } elseif ($Uefi) { "UEFI" } elseif ($Efi) { "EFI" } elseif ($Elf) { "ELF QEMU-11.0.0" } else { "CDX" }
