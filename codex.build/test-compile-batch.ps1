# Batch compiler: boots one QEMU, compiles multiple sources through the
...
#         -PCore     P-core index for QEMU affinity
#
...
try {
    ...
    if (-not $run) {
        Write-Error "QEMU failed to start"
        exit 3
    }
    ...
    for ($i = 0; $i -lt $sources.Count; $i++) {
        ...
        try {
            ...
            # Compiler crashed. Restart QEMU for remaining items.
            Close-Qemu -Conn $run.Conn -Process $run.Process
            $run = Start-QemuRun -Kernel $Stage0 -ConnectTimeoutSec 30 -MemMB 2048 -PCore $PCore
            if (-not $run) {
                Write-Error "QEMU restart failed at item $i"
                exit 3
            }
            if (-not (Read-QemuReady -Conn $run.Conn -TimeoutSec 30)) {
