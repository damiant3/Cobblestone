...
#
# Phase 1: batch-compile all non-skipped tests through persistent QEMU
#           instances (one per job slot, repl-loop reuse).
# Phase 2: run compiled tests that have .expected files (individual QEMU
#           per test, parallel).
#
# Usage:
#   codex.build/test.ps1 [-CodexCdx FP] [-Jobs N] [-ErrorsOnly | -NoErrors]
#   codex.build/test.ps1 -FW      # also include codex.test\forewords\*.codex
#   codex.build/test.ps1 -Apps    # also include codex.test\apps\*.codex
#   codex.build/test.ps1 -All     # all of the above
#   codex.build/test.ps1 -All -Slow  # include slow tests too
#   codex.build/test.ps1 -Fatal     # include fatal tests (GPF/exception demos)
#
...
#   codex*.test\foo.fatal     — skipped unless -Fatal is passed (kills QEMU at runtime)
#
...
if ($Scope -eq 'both' -or $Scope -eq 'positive') {
    Get-ChildItem -Path 'codex.test\*.codex' -File -ErrorAction SilentlyContinue | ForEach-Object { $tests.Add($_.FullName) }
    if ($Apps -or $All) {
        Get-ChildItem -Path 'codex.test\apps\*.codex' -File -ErrorAction SilentlyContinue | ForEach-Object { $tests.Add($_.FullName) }
    }
    if ($FW -or $All) {
        Get-ChildItem -Path 'codex.test\forewords\*.codex' -File -ErrorAction SilentlyContinue | ForEach-Object { $tests.Add($_.FullName) }
    }
}
if ($Scope -eq 'both' -or $Scope -eq 'errors') {
    Get-ChildItem -Path 'codex.test\errors\*.codex' -File -ErrorAction SilentlyContinue | ForEach-Object { $tests.Add($_.FullName) }
}
...
# ═══════════════════════════════════════════════════════════════════════════
# Phase 1: Batch compile — one QEMU per job slot
# ═══════════════════════════════════════════════════════════════════════════
...
# ═══════════════════════════════════════════════════════════════════════════
# Phase 2: Run tests with .expected files (individual QEMU per test)
# ═══════════════════════════════════════════════════════════════════════════
