...
$OutDir    = Join-Path $Repo 'codex.build\output'
...
function Invoke-BuildText {
    ...
    if (-not $run) { Write-Host ''; Write-Host 'FAIL: QEMU did not start'; return $false }
    try {
        ...
        $stream.WriteByte(4)
        $stream.Flush()
        $allBytes = [System.Collections.Generic.List[byte]]::new(2097152)
        ...
        while ((Get-Date) -lt $deadline) {
            ...
            $readTimeout = if ($gotOutput) { 10000 } else { [math]::Min(180000, $remainMs) }
            if ($stream.CanTimeout) { $stream.ReadTimeout = $readTimeout }
            try { $n = $stream.Read($readBuf, 0, $readBuf.Length) } catch { break }
            if ($n -le 0) { break }
            for ($bi = 0; $bi -lt $n; $bi++) { $allBytes.Add($readBuf[$bi]) }
            if ($allBytes.Count -gt 100000) { $gotOutput = $true }
        }
        $raw = [System.Text.Encoding]::UTF8.GetString($allBytes.ToArray())
        $lines = $raw -split "`n"
        $textLines = [System.Collections.Generic.List[string]]::new()
        foreach ($rl in $lines) {
            $line = $rl.TrimEnd("`r")
            if (-not $line.StartsWith('WD:')) { $textLines.Add($line) }
        }
...
& pwsh -NoProfile -File $Concat -CodexDir (Join-Path $Repo 'codex') -OutFile $CodexSrc
...
"@
    ...
    $signLines = Get-Content $signOut
    $pubBytes = $signLines[0].Split(',') | ForEach-Object { [byte]$_ }
    $sigBytes = $signLines[1].Split(',') | ForEach-Object { [byte]$_ }
...
# ── canary
$canarySrc      = Join-Path $Repo 'codex.test\hello.codex'
$canaryExpected = Join-Path $Repo 'codex.test\hello.expected'
$canaryCdx      = Join-Path $OutDir 'canary-hello.cdx'
...
if ($LASTEXITCODE -ne 0) {
    Write-Host 'FAIL: canary compile — SUT cannot compile hello.codex'
